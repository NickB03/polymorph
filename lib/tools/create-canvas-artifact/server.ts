...
import type { CreateCanvasArtifactOutput } from './schema'
...
export function createCanvasArtifactTool(ctx: CanvasToolContext) {
  return tool({
    ...
    execute: async ({ title, files }) => {
      ...
      // Emit generating status immediately so the client knows the tool is executing.
...
export const serverTool = createCanvasArtifactTool
