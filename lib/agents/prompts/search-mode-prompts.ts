...
/**
 * Constructs the system prompt used for the assistant's fast, efficiency-focused (quick) mode.
 *
 * The returned prompt contains efficiency guidelines, early-stop criteria, language and approach directives,
 * mandatory search and fetch rules, strict citation formatting, fetch tool behavior (including PDF handling),
 * display tool guidance, and an explicit note about video/image search availability that is included only when a
 * general search provider is available.
 *
 * @returns The full quick-mode system prompt string used to guide search, fetching, citation, and display tool behavior.
 */
...
/**
 * Builds and returns the adaptive-mode system prompt used to guide an AI assistant
 * that has real-time web search, content retrieval, task management, and clarifying-question tools.
 *
 * The prompt defines efficiency guidelines, early-stop criteria, a strategy for assessing
 * query complexity, mandatory search and citation rules, detailed tool usage patterns
 * (search, fetch, todoWrite, ask_question), display tool ordering and formatting requirements,
 * and the required output/markdown structure for responses.
 *
 * @returns The complete prompt string for adaptive mode, suitable for use as a system/instruction message.
 */
