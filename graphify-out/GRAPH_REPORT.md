# Graph Report - vana-v2  (2026-07-21)

## Corpus Check
- 988 files · ~490,247 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 16544 nodes · 22454 edges · 1087 communities (1048 shown, 39 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 161 edges (avg confidence: 0.81)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `2bc4f55f`
- Run `git rev-parse HEAD` and compare to check if the graph is stale.
- Run `graphify update .` after code changes (no API cost).

## Community Hubs (Navigation)
- [[_COMMUNITY_Project Root|Project Root]]
- [[_COMMUNITY_Utilities|Utilities]]
- [[_COMMUNITY_Components|Components]]
- [[_COMMUNITY_Database & Schema|Database & Schema]]
- [[_COMMUNITY_Components|Components]]
- [[_COMMUNITY_Rate Limiting|Rate Limiting]]
- [[_COMMUNITY_Server Actions|Server Actions]]
- [[_COMMUNITY_Charts|Charts]]
- [[_COMMUNITY_React Hooks|React Hooks]]
- [[_COMMUNITY_Components|Components]]
- [[_COMMUNITY_Streaming|Streaming]]
- [[_COMMUNITY_Tools|Tools]]
- [[_COMMUNITY_UI Primitives|UI Primitives]]
- [[_COMMUNITY_Canvas Engine|Canvas Engine]]
- [[_COMMUNITY_Evals Service|Evals Service]]
- [[_COMMUNITY_Canvas UI|Canvas UI]]
- [[_COMMUNITY_Tools|Tools]]
- [[_COMMUNITY_Agents|Agents]]
- [[_COMMUNITY_Evals Library|Evals Library]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_Evals UI|Evals UI]]
- [[_COMMUNITY_Components|Components]]
- [[_COMMUNITY_Server Actions|Server Actions]]
- [[_COMMUNITY_UI Primitives|UI Primitives]]
- [[_COMMUNITY_Evals Library|Evals Library]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_Tool UI|Tool UI]]
- [[_COMMUNITY_Canvas UI|Canvas UI]]
- [[_COMMUNITY_App Routes|App Routes]]
- [[_COMMUNITY_React Hooks|React Hooks]]
- [[_COMMUNITY_Evals Service|Evals Service]]
- [[_COMMUNITY_Canvas Engine|Canvas Engine]]
- [[_COMMUNITY_Tool UI|Tool UI]]
- [[_COMMUNITY_Canvas Engine|Canvas Engine]]
- [[_COMMUNITY_Tool UI|Tool UI]]
- [[_COMMUNITY_Tool UI|Tool UI]]
- [[_COMMUNITY_Tool UI|Tool UI]]
- [[_COMMUNITY_Components|Components]]
- [[_COMMUNITY_Evals Library|Evals Library]]
- [[_COMMUNITY_Evals Service|Evals Service]]
- [[_COMMUNITY_Charts|Charts]]
- [[_COMMUNITY_Charts|Charts]]
- [[_COMMUNITY_Evals Service|Evals Service]]
- [[_COMMUNITY_Tool UI|Tool UI]]
- [[_COMMUNITY_Agents|Agents]]
- [[_COMMUNITY_Evals UI|Evals UI]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_Tools|Tools]]
- [[_COMMUNITY_Components|Components]]
- [[_COMMUNITY_Evals Service|Evals Service]]
- [[_COMMUNITY_Tool UI|Tool UI]]
- [[_COMMUNITY_Components|Components]]
- [[_COMMUNITY_Components|Components]]
- [[_COMMUNITY_Evals Service|Evals Service]]
- [[_COMMUNITY_Tools|Tools]]
- [[_COMMUNITY_Agents|Agents]]
- [[_COMMUNITY_Tool UI|Tool UI]]
- [[_COMMUNITY_Tools|Tools]]
- [[_COMMUNITY_Tool UI|Tool UI]]
- [[_COMMUNITY_Tool UI|Tool UI]]
- [[_COMMUNITY_Activity UI|Activity UI]]
- [[_COMMUNITY_Agents|Agents]]
- [[_COMMUNITY_lib|lib]]
- [[_COMMUNITY_Types|Types]]
- [[_COMMUNITY_Evals UI|Evals UI]]
- [[_COMMUNITY_Tools|Tools]]
- [[_COMMUNITY_Evals Service|Evals Service]]
- [[_COMMUNITY_Project Root|Project Root]]
- [[_COMMUNITY_Tools|Tools]]
- [[_COMMUNITY_UI Primitives|UI Primitives]]
- [[_COMMUNITY_UI Primitives|UI Primitives]]
- [[_COMMUNITY_Tool UI|Tool UI]]
- [[_COMMUNITY_Tools|Tools]]
- [[_COMMUNITY_Canvas Engine|Canvas Engine]]
- [[_COMMUNITY_Tool UI|Tool UI]]
- [[_COMMUNITY_Tools|Tools]]
- [[_COMMUNITY_UI Primitives|UI Primitives]]
- [[_COMMUNITY_Components|Components]]
- [[_COMMUNITY_Tools|Tools]]
- [[_COMMUNITY_Scripts|Scripts]]
- [[_COMMUNITY_Evals Service|Evals Service]]
- [[_COMMUNITY_UI Primitives|UI Primitives]]
- [[_COMMUNITY_Tool UI|Tool UI]]
- [[_COMMUNITY_Tools|Tools]]
- [[_COMMUNITY_Evals Service|Evals Service]]
- [[_COMMUNITY_Tool UI|Tool UI]]
- [[_COMMUNITY_Project Root|Project Root]]
- [[_COMMUNITY_Agents|Agents]]
- [[_COMMUNITY_Agents|Agents]]
- [[_COMMUNITY_Project Root|Project Root]]
- [[_COMMUNITY_Tool UI|Tool UI]]
- [[_COMMUNITY_Tool UI|Tool UI]]
- [[_COMMUNITY_Tool UI|Tool UI]]
- [[_COMMUNITY_Evals Service|Evals Service]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_Project Root|Project Root]]
- [[_COMMUNITY_Configuration|Configuration]]
- [[_COMMUNITY_Tools|Tools]]
- [[_COMMUNITY_lib|lib]]
- [[_COMMUNITY_lib|lib]]
- [[_COMMUNITY_Canvas Engine|Canvas Engine]]
- [[_COMMUNITY_Tool UI|Tool UI]]
- [[_COMMUNITY_Tools|Tools]]
- [[_COMMUNITY_Tool UI|Tool UI]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_Evals Service|Evals Service]]
- [[_COMMUNITY_Tools|Tools]]
- [[_COMMUNITY_lib|lib]]
- [[_COMMUNITY_Tool UI|Tool UI]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_Tools|Tools]]
- [[_COMMUNITY_Evals UI|Evals UI]]
- [[_COMMUNITY_UI Primitives|UI Primitives]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_Auth|Auth]]
- [[_COMMUNITY_App Routes|App Routes]]
- [[_COMMUNITY_UI Primitives|UI Primitives]]
- [[_COMMUNITY_Tool UI|Tool UI]]
- [[_COMMUNITY_Tools|Tools]]
- [[_COMMUNITY_Tools|Tools]]
- [[_COMMUNITY_Tools|Tools]]
- [[_COMMUNITY_tests|tests]]
- [[_COMMUNITY_lib|lib]]
- [[_COMMUNITY_Scripts|Scripts]]
- [[_COMMUNITY_Tools|Tools]]
- [[_COMMUNITY_Tools|Tools]]
- [[_COMMUNITY_Tools|Tools]]
- [[_COMMUNITY_Evals Service|Evals Service]]
- [[_COMMUNITY_Utilities|Utilities]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_Project Root|Project Root]]
- [[_COMMUNITY_Tools|Tools]]
- [[_COMMUNITY_Evals Service|Evals Service]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_Streaming|Streaming]]
- [[_COMMUNITY_Types|Types]]
- [[_COMMUNITY_Utilities|Utilities]]
- [[_COMMUNITY_Auth|Auth]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_lib|lib]]
- [[_COMMUNITY_lib|lib]]
- [[_COMMUNITY_Tools|Tools]]
- [[_COMMUNITY_Tool UI|Tool UI]]
- [[_COMMUNITY_Components|Components]]
- [[_COMMUNITY_Tools|Tools]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_Tool UI|Tool UI]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_lib|lib]]
- [[_COMMUNITY_Tools|Tools]]
- [[_COMMUNITY_App Routes|App Routes]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_Project Root|Project Root]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_App Routes|App Routes]]
- [[_COMMUNITY_UI Primitives|UI Primitives]]
- [[_COMMUNITY_App Routes|App Routes]]
- [[_COMMUNITY_Components|Components]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_Tool UI|Tool UI]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_Evals Service|Evals Service]]
- [[_COMMUNITY_App Routes|App Routes]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_Canvas UI|Canvas UI]]
- [[_COMMUNITY_Tool UI|Tool UI]]
- [[_COMMUNITY_Evals Service|Evals Service]]
- [[_COMMUNITY_Tools|Tools]]
- [[_COMMUNITY_Tools|Tools]]
- [[_COMMUNITY_Streaming|Streaming]]
- [[_COMMUNITY_Tools|Tools]]
- [[_COMMUNITY_Tools|Tools]]
- [[_COMMUNITY_Tools|Tools]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_Evals Service|Evals Service]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_Project Root|Project Root]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_Scripts|Scripts]]
- [[_COMMUNITY_Project Root|Project Root]]
- [[_COMMUNITY_Project Root|Project Root]]
- [[_COMMUNITY_Database & Schema|Database & Schema]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_Tool UI|Tool UI]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_Evals Library|Evals Library]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_lib|lib]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_Tool UI|Tool UI]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_unnamed|]]
- [[_COMMUNITY_Community 243|Community 243]]
- [[_COMMUNITY_Community 244|Community 244]]
- [[_COMMUNITY_Community 245|Community 245]]
- [[_COMMUNITY_Community 246|Community 246]]
- [[_COMMUNITY_Community 247|Community 247]]
- [[_COMMUNITY_Community 248|Community 248]]
- [[_COMMUNITY_Community 249|Community 249]]
- [[_COMMUNITY_Community 250|Community 250]]
- [[_COMMUNITY_Community 251|Community 251]]
- [[_COMMUNITY_Community 252|Community 252]]
- [[_COMMUNITY_Community 253|Community 253]]
- [[_COMMUNITY_Community 254|Community 254]]
- [[_COMMUNITY_Community 255|Community 255]]
- [[_COMMUNITY_Community 256|Community 256]]
- [[_COMMUNITY_Community 257|Community 257]]
- [[_COMMUNITY_Community 258|Community 258]]
- [[_COMMUNITY_Community 259|Community 259]]
- [[_COMMUNITY_Community 260|Community 260]]
- [[_COMMUNITY_Community 261|Community 261]]
- [[_COMMUNITY_Community 262|Community 262]]
- [[_COMMUNITY_Community 263|Community 263]]
- [[_COMMUNITY_Community 264|Community 264]]
- [[_COMMUNITY_Community 265|Community 265]]
- [[_COMMUNITY_Community 266|Community 266]]
- [[_COMMUNITY_Community 267|Community 267]]
- [[_COMMUNITY_Community 268|Community 268]]
- [[_COMMUNITY_Community 269|Community 269]]
- [[_COMMUNITY_Community 270|Community 270]]
- [[_COMMUNITY_Community 271|Community 271]]
- [[_COMMUNITY_Community 272|Community 272]]
- [[_COMMUNITY_Community 273|Community 273]]
- [[_COMMUNITY_Community 274|Community 274]]
- [[_COMMUNITY_Community 275|Community 275]]
- [[_COMMUNITY_Community 276|Community 276]]
- [[_COMMUNITY_Community 277|Community 277]]
- [[_COMMUNITY_Community 278|Community 278]]
- [[_COMMUNITY_Community 279|Community 279]]
- [[_COMMUNITY_Community 280|Community 280]]
- [[_COMMUNITY_Community 281|Community 281]]
- [[_COMMUNITY_Community 282|Community 282]]
- [[_COMMUNITY_Community 283|Community 283]]
- [[_COMMUNITY_Community 284|Community 284]]
- [[_COMMUNITY_Community 285|Community 285]]
- [[_COMMUNITY_Community 286|Community 286]]
- [[_COMMUNITY_Community 287|Community 287]]
- [[_COMMUNITY_Community 288|Community 288]]
- [[_COMMUNITY_Community 289|Community 289]]
- [[_COMMUNITY_Community 290|Community 290]]
- [[_COMMUNITY_Community 291|Community 291]]
- [[_COMMUNITY_Community 292|Community 292]]
- [[_COMMUNITY_Community 293|Community 293]]
- [[_COMMUNITY_Community 294|Community 294]]
- [[_COMMUNITY_Community 295|Community 295]]
- [[_COMMUNITY_Community 296|Community 296]]
- [[_COMMUNITY_Community 297|Community 297]]
- [[_COMMUNITY_Community 298|Community 298]]
- [[_COMMUNITY_Community 299|Community 299]]
- [[_COMMUNITY_Community 300|Community 300]]
- [[_COMMUNITY_Community 301|Community 301]]
- [[_COMMUNITY_Community 302|Community 302]]
- [[_COMMUNITY_Community 303|Community 303]]
- [[_COMMUNITY_Community 304|Community 304]]
- [[_COMMUNITY_Community 305|Community 305]]
- [[_COMMUNITY_Community 306|Community 306]]
- [[_COMMUNITY_Community 307|Community 307]]
- [[_COMMUNITY_Community 308|Community 308]]
- [[_COMMUNITY_Community 309|Community 309]]
- [[_COMMUNITY_Community 310|Community 310]]
- [[_COMMUNITY_Community 311|Community 311]]
- [[_COMMUNITY_Community 312|Community 312]]
- [[_COMMUNITY_Community 313|Community 313]]
- [[_COMMUNITY_Community 314|Community 314]]
- [[_COMMUNITY_Community 315|Community 315]]
- [[_COMMUNITY_Community 316|Community 316]]
- [[_COMMUNITY_Community 317|Community 317]]
- [[_COMMUNITY_Community 318|Community 318]]
- [[_COMMUNITY_Community 319|Community 319]]
- [[_COMMUNITY_Community 320|Community 320]]
- [[_COMMUNITY_Community 321|Community 321]]
- [[_COMMUNITY_Community 322|Community 322]]
- [[_COMMUNITY_Community 323|Community 323]]
- [[_COMMUNITY_Community 324|Community 324]]
- [[_COMMUNITY_Community 325|Community 325]]
- [[_COMMUNITY_Community 326|Community 326]]
- [[_COMMUNITY_Community 327|Community 327]]
- [[_COMMUNITY_Community 328|Community 328]]
- [[_COMMUNITY_Community 329|Community 329]]
- [[_COMMUNITY_Community 330|Community 330]]
- [[_COMMUNITY_Community 331|Community 331]]
- [[_COMMUNITY_Community 332|Community 332]]
- [[_COMMUNITY_Community 333|Community 333]]
- [[_COMMUNITY_Community 334|Community 334]]
- [[_COMMUNITY_Community 335|Community 335]]
- [[_COMMUNITY_Community 336|Community 336]]
- [[_COMMUNITY_Community 337|Community 337]]
- [[_COMMUNITY_Community 338|Community 338]]
- [[_COMMUNITY_Community 339|Community 339]]
- [[_COMMUNITY_Community 340|Community 340]]
- [[_COMMUNITY_Community 341|Community 341]]
- [[_COMMUNITY_Community 342|Community 342]]
- [[_COMMUNITY_Community 343|Community 343]]
- [[_COMMUNITY_Community 344|Community 344]]
- [[_COMMUNITY_Community 345|Community 345]]
- [[_COMMUNITY_Community 346|Community 346]]
- [[_COMMUNITY_Community 347|Community 347]]
- [[_COMMUNITY_Community 348|Community 348]]
- [[_COMMUNITY_Community 349|Community 349]]
- [[_COMMUNITY_Community 350|Community 350]]
- [[_COMMUNITY_Community 351|Community 351]]
- [[_COMMUNITY_Community 352|Community 352]]
- [[_COMMUNITY_Community 353|Community 353]]
- [[_COMMUNITY_Community 354|Community 354]]
- [[_COMMUNITY_Community 355|Community 355]]
- [[_COMMUNITY_Community 356|Community 356]]
- [[_COMMUNITY_Community 357|Community 357]]
- [[_COMMUNITY_Community 358|Community 358]]
- [[_COMMUNITY_Community 359|Community 359]]
- [[_COMMUNITY_Community 360|Community 360]]
- [[_COMMUNITY_Community 361|Community 361]]
- [[_COMMUNITY_Community 362|Community 362]]
- [[_COMMUNITY_Community 363|Community 363]]
- [[_COMMUNITY_Community 364|Community 364]]
- [[_COMMUNITY_Community 365|Community 365]]
- [[_COMMUNITY_Community 366|Community 366]]
- [[_COMMUNITY_Community 367|Community 367]]
- [[_COMMUNITY_Community 368|Community 368]]
- [[_COMMUNITY_Community 369|Community 369]]
- [[_COMMUNITY_Community 370|Community 370]]
- [[_COMMUNITY_Community 371|Community 371]]
- [[_COMMUNITY_Community 372|Community 372]]
- [[_COMMUNITY_Community 373|Community 373]]
- [[_COMMUNITY_Community 374|Community 374]]
- [[_COMMUNITY_Community 375|Community 375]]
- [[_COMMUNITY_Community 376|Community 376]]
- [[_COMMUNITY_Community 377|Community 377]]
- [[_COMMUNITY_Community 378|Community 378]]
- [[_COMMUNITY_Community 379|Community 379]]
- [[_COMMUNITY_Community 380|Community 380]]
- [[_COMMUNITY_Community 381|Community 381]]
- [[_COMMUNITY_Community 382|Community 382]]
- [[_COMMUNITY_Community 383|Community 383]]
- [[_COMMUNITY_Community 384|Community 384]]
- [[_COMMUNITY_Community 385|Community 385]]
- [[_COMMUNITY_Community 386|Community 386]]
- [[_COMMUNITY_Community 387|Community 387]]
- [[_COMMUNITY_Community 388|Community 388]]
- [[_COMMUNITY_Community 389|Community 389]]
- [[_COMMUNITY_Community 390|Community 390]]
- [[_COMMUNITY_Community 391|Community 391]]
- [[_COMMUNITY_Community 392|Community 392]]
- [[_COMMUNITY_Community 393|Community 393]]
- [[_COMMUNITY_Community 394|Community 394]]
- [[_COMMUNITY_Community 395|Community 395]]
- [[_COMMUNITY_Community 396|Community 396]]
- [[_COMMUNITY_Community 397|Community 397]]
- [[_COMMUNITY_Community 398|Community 398]]
- [[_COMMUNITY_Community 399|Community 399]]
- [[_COMMUNITY_Community 400|Community 400]]
- [[_COMMUNITY_Community 401|Community 401]]
- [[_COMMUNITY_Community 402|Community 402]]
- [[_COMMUNITY_Community 403|Community 403]]
- [[_COMMUNITY_Community 404|Community 404]]
- [[_COMMUNITY_Community 405|Community 405]]
- [[_COMMUNITY_Community 406|Community 406]]
- [[_COMMUNITY_Community 407|Community 407]]
- [[_COMMUNITY_Community 408|Community 408]]
- [[_COMMUNITY_Community 409|Community 409]]
- [[_COMMUNITY_Community 410|Community 410]]
- [[_COMMUNITY_Community 411|Community 411]]
- [[_COMMUNITY_Community 412|Community 412]]
- [[_COMMUNITY_Community 413|Community 413]]
- [[_COMMUNITY_Community 414|Community 414]]
- [[_COMMUNITY_Community 415|Community 415]]
- [[_COMMUNITY_Community 416|Community 416]]
- [[_COMMUNITY_Community 417|Community 417]]
- [[_COMMUNITY_Community 418|Community 418]]
- [[_COMMUNITY_Community 419|Community 419]]
- [[_COMMUNITY_Community 420|Community 420]]
- [[_COMMUNITY_Community 421|Community 421]]
- [[_COMMUNITY_Community 422|Community 422]]
- [[_COMMUNITY_Community 423|Community 423]]
- [[_COMMUNITY_Community 424|Community 424]]
- [[_COMMUNITY_Community 425|Community 425]]
- [[_COMMUNITY_Community 426|Community 426]]
- [[_COMMUNITY_Community 427|Community 427]]
- [[_COMMUNITY_Community 428|Community 428]]
- [[_COMMUNITY_Community 429|Community 429]]
- [[_COMMUNITY_Community 430|Community 430]]
- [[_COMMUNITY_Community 431|Community 431]]
- [[_COMMUNITY_Community 432|Community 432]]
- [[_COMMUNITY_Community 433|Community 433]]
- [[_COMMUNITY_Community 434|Community 434]]
- [[_COMMUNITY_Community 435|Community 435]]
- [[_COMMUNITY_Community 436|Community 436]]
- [[_COMMUNITY_Community 437|Community 437]]
- [[_COMMUNITY_Community 438|Community 438]]
- [[_COMMUNITY_Community 439|Community 439]]
- [[_COMMUNITY_Community 440|Community 440]]
- [[_COMMUNITY_Community 441|Community 441]]
- [[_COMMUNITY_Community 442|Community 442]]
- [[_COMMUNITY_Community 443|Community 443]]
- [[_COMMUNITY_Community 444|Community 444]]
- [[_COMMUNITY_Community 445|Community 445]]
- [[_COMMUNITY_Community 446|Community 446]]
- [[_COMMUNITY_Community 447|Community 447]]
- [[_COMMUNITY_Community 448|Community 448]]
- [[_COMMUNITY_Community 449|Community 449]]
- [[_COMMUNITY_Community 450|Community 450]]
- [[_COMMUNITY_Community 451|Community 451]]
- [[_COMMUNITY_Community 452|Community 452]]
- [[_COMMUNITY_Community 453|Community 453]]
- [[_COMMUNITY_Community 454|Community 454]]
- [[_COMMUNITY_Community 455|Community 455]]
- [[_COMMUNITY_Community 456|Community 456]]
- [[_COMMUNITY_Community 457|Community 457]]
- [[_COMMUNITY_Community 458|Community 458]]
- [[_COMMUNITY_Community 459|Community 459]]
- [[_COMMUNITY_Community 460|Community 460]]
- [[_COMMUNITY_Community 461|Community 461]]
- [[_COMMUNITY_Community 462|Community 462]]
- [[_COMMUNITY_Community 463|Community 463]]
- [[_COMMUNITY_Community 464|Community 464]]
- [[_COMMUNITY_Community 465|Community 465]]
- [[_COMMUNITY_Community 466|Community 466]]
- [[_COMMUNITY_Community 467|Community 467]]
- [[_COMMUNITY_Community 468|Community 468]]
- [[_COMMUNITY_Community 469|Community 469]]
- [[_COMMUNITY_Community 470|Community 470]]
- [[_COMMUNITY_Community 471|Community 471]]
- [[_COMMUNITY_Community 472|Community 472]]
- [[_COMMUNITY_Community 473|Community 473]]
- [[_COMMUNITY_Community 474|Community 474]]
- [[_COMMUNITY_Community 475|Community 475]]
- [[_COMMUNITY_Community 476|Community 476]]
- [[_COMMUNITY_Community 477|Community 477]]
- [[_COMMUNITY_Community 478|Community 478]]
- [[_COMMUNITY_Community 479|Community 479]]
- [[_COMMUNITY_Community 480|Community 480]]
- [[_COMMUNITY_Community 481|Community 481]]
- [[_COMMUNITY_Community 482|Community 482]]
- [[_COMMUNITY_Community 483|Community 483]]
- [[_COMMUNITY_Community 484|Community 484]]
- [[_COMMUNITY_Community 485|Community 485]]
- [[_COMMUNITY_Community 486|Community 486]]
- [[_COMMUNITY_Community 487|Community 487]]
- [[_COMMUNITY_Community 488|Community 488]]
- [[_COMMUNITY_Community 489|Community 489]]
- [[_COMMUNITY_Community 490|Community 490]]
- [[_COMMUNITY_Community 491|Community 491]]
- [[_COMMUNITY_Community 492|Community 492]]
- [[_COMMUNITY_Community 493|Community 493]]
- [[_COMMUNITY_Community 494|Community 494]]
- [[_COMMUNITY_Community 495|Community 495]]
- [[_COMMUNITY_Community 496|Community 496]]
- [[_COMMUNITY_Community 497|Community 497]]
- [[_COMMUNITY_Community 498|Community 498]]
- [[_COMMUNITY_Community 499|Community 499]]
- [[_COMMUNITY_Community 500|Community 500]]
- [[_COMMUNITY_Community 501|Community 501]]
- [[_COMMUNITY_Community 502|Community 502]]
- [[_COMMUNITY_Community 503|Community 503]]
- [[_COMMUNITY_Community 504|Community 504]]
- [[_COMMUNITY_Community 505|Community 505]]
- [[_COMMUNITY_Community 506|Community 506]]
- [[_COMMUNITY_Community 507|Community 507]]
- [[_COMMUNITY_Community 508|Community 508]]
- [[_COMMUNITY_Community 509|Community 509]]
- [[_COMMUNITY_Community 510|Community 510]]
- [[_COMMUNITY_Community 511|Community 511]]
- [[_COMMUNITY_Community 512|Community 512]]
- [[_COMMUNITY_Community 513|Community 513]]
- [[_COMMUNITY_Community 514|Community 514]]
- [[_COMMUNITY_Community 515|Community 515]]
- [[_COMMUNITY_Community 516|Community 516]]
- [[_COMMUNITY_Community 517|Community 517]]
- [[_COMMUNITY_Community 518|Community 518]]
- [[_COMMUNITY_Community 519|Community 519]]
- [[_COMMUNITY_Community 520|Community 520]]
- [[_COMMUNITY_Community 521|Community 521]]
- [[_COMMUNITY_Community 522|Community 522]]
- [[_COMMUNITY_Community 523|Community 523]]
- [[_COMMUNITY_Community 524|Community 524]]
- [[_COMMUNITY_Community 525|Community 525]]
- [[_COMMUNITY_Community 526|Community 526]]
- [[_COMMUNITY_Community 527|Community 527]]
- [[_COMMUNITY_Community 528|Community 528]]
- [[_COMMUNITY_Community 529|Community 529]]
- [[_COMMUNITY_Community 530|Community 530]]
- [[_COMMUNITY_Community 531|Community 531]]
- [[_COMMUNITY_Community 532|Community 532]]
- [[_COMMUNITY_Community 533|Community 533]]
- [[_COMMUNITY_Community 534|Community 534]]
- [[_COMMUNITY_Community 535|Community 535]]
- [[_COMMUNITY_Community 536|Community 536]]
- [[_COMMUNITY_Community 537|Community 537]]
- [[_COMMUNITY_Community 538|Community 538]]
- [[_COMMUNITY_Community 539|Community 539]]
- [[_COMMUNITY_Community 540|Community 540]]
- [[_COMMUNITY_Community 541|Community 541]]
- [[_COMMUNITY_Community 542|Community 542]]
- [[_COMMUNITY_Community 543|Community 543]]
- [[_COMMUNITY_Community 544|Community 544]]
- [[_COMMUNITY_Community 545|Community 545]]
- [[_COMMUNITY_Community 546|Community 546]]
- [[_COMMUNITY_Community 547|Community 547]]
- [[_COMMUNITY_Community 548|Community 548]]
- [[_COMMUNITY_Community 549|Community 549]]
- [[_COMMUNITY_Community 550|Community 550]]
- [[_COMMUNITY_Community 551|Community 551]]
- [[_COMMUNITY_Community 552|Community 552]]
- [[_COMMUNITY_Community 553|Community 553]]
- [[_COMMUNITY_Community 554|Community 554]]
- [[_COMMUNITY_Community 555|Community 555]]
- [[_COMMUNITY_Community 556|Community 556]]
- [[_COMMUNITY_Community 557|Community 557]]
- [[_COMMUNITY_Community 558|Community 558]]
- [[_COMMUNITY_Community 559|Community 559]]
- [[_COMMUNITY_Community 560|Community 560]]
- [[_COMMUNITY_Community 561|Community 561]]
- [[_COMMUNITY_Community 562|Community 562]]
- [[_COMMUNITY_Community 563|Community 563]]
- [[_COMMUNITY_Community 564|Community 564]]
- [[_COMMUNITY_Community 565|Community 565]]
- [[_COMMUNITY_Community 566|Community 566]]
- [[_COMMUNITY_Community 567|Community 567]]
- [[_COMMUNITY_Community 568|Community 568]]
- [[_COMMUNITY_Community 569|Community 569]]
- [[_COMMUNITY_Community 570|Community 570]]
- [[_COMMUNITY_Community 571|Community 571]]
- [[_COMMUNITY_Community 572|Community 572]]
- [[_COMMUNITY_Community 573|Community 573]]
- [[_COMMUNITY_Community 574|Community 574]]
- [[_COMMUNITY_Community 575|Community 575]]
- [[_COMMUNITY_Community 576|Community 576]]
- [[_COMMUNITY_Community 577|Community 577]]
- [[_COMMUNITY_Community 578|Community 578]]
- [[_COMMUNITY_Community 579|Community 579]]
- [[_COMMUNITY_Community 580|Community 580]]
- [[_COMMUNITY_Community 581|Community 581]]
- [[_COMMUNITY_Community 582|Community 582]]
- [[_COMMUNITY_Community 583|Community 583]]
- [[_COMMUNITY_Community 584|Community 584]]
- [[_COMMUNITY_Community 585|Community 585]]
- [[_COMMUNITY_Community 586|Community 586]]
- [[_COMMUNITY_Community 587|Community 587]]
- [[_COMMUNITY_Community 588|Community 588]]
- [[_COMMUNITY_Community 589|Community 589]]
- [[_COMMUNITY_Community 590|Community 590]]
- [[_COMMUNITY_Community 591|Community 591]]
- [[_COMMUNITY_Community 592|Community 592]]
- [[_COMMUNITY_Community 593|Community 593]]
- [[_COMMUNITY_Community 594|Community 594]]
- [[_COMMUNITY_Community 595|Community 595]]
- [[_COMMUNITY_Community 596|Community 596]]
- [[_COMMUNITY_Community 597|Community 597]]
- [[_COMMUNITY_Community 598|Community 598]]
- [[_COMMUNITY_Community 599|Community 599]]
- [[_COMMUNITY_Community 600|Community 600]]
- [[_COMMUNITY_Community 601|Community 601]]
- [[_COMMUNITY_Community 602|Community 602]]
- [[_COMMUNITY_Community 603|Community 603]]
- [[_COMMUNITY_Community 604|Community 604]]
- [[_COMMUNITY_Community 605|Community 605]]
- [[_COMMUNITY_Community 606|Community 606]]
- [[_COMMUNITY_Community 607|Community 607]]
- [[_COMMUNITY_Community 608|Community 608]]
- [[_COMMUNITY_Community 609|Community 609]]
- [[_COMMUNITY_Community 610|Community 610]]
- [[_COMMUNITY_Community 611|Community 611]]
- [[_COMMUNITY_Community 612|Community 612]]
- [[_COMMUNITY_Community 613|Community 613]]
- [[_COMMUNITY_Community 614|Community 614]]
- [[_COMMUNITY_Community 615|Community 615]]
- [[_COMMUNITY_Community 616|Community 616]]
- [[_COMMUNITY_Community 617|Community 617]]
- [[_COMMUNITY_Community 618|Community 618]]
- [[_COMMUNITY_Community 619|Community 619]]
- [[_COMMUNITY_Community 620|Community 620]]
- [[_COMMUNITY_Community 621|Community 621]]
- [[_COMMUNITY_Community 622|Community 622]]
- [[_COMMUNITY_Community 623|Community 623]]
- [[_COMMUNITY_Community 624|Community 624]]
- [[_COMMUNITY_Community 625|Community 625]]
- [[_COMMUNITY_Community 626|Community 626]]
- [[_COMMUNITY_Community 627|Community 627]]
- [[_COMMUNITY_Community 628|Community 628]]
- [[_COMMUNITY_Community 629|Community 629]]
- [[_COMMUNITY_Community 630|Community 630]]
- [[_COMMUNITY_Community 631|Community 631]]
- [[_COMMUNITY_Community 632|Community 632]]
- [[_COMMUNITY_Community 633|Community 633]]
- [[_COMMUNITY_Community 634|Community 634]]
- [[_COMMUNITY_Community 635|Community 635]]
- [[_COMMUNITY_Community 636|Community 636]]
- [[_COMMUNITY_Community 637|Community 637]]
- [[_COMMUNITY_Community 638|Community 638]]
- [[_COMMUNITY_Community 639|Community 639]]
- [[_COMMUNITY_Community 640|Community 640]]
- [[_COMMUNITY_Community 641|Community 641]]
- [[_COMMUNITY_Community 642|Community 642]]
- [[_COMMUNITY_Community 643|Community 643]]
- [[_COMMUNITY_Community 644|Community 644]]
- [[_COMMUNITY_Community 645|Community 645]]
- [[_COMMUNITY_Community 646|Community 646]]
- [[_COMMUNITY_Community 647|Community 647]]
- [[_COMMUNITY_Community 648|Community 648]]
- [[_COMMUNITY_Community 649|Community 649]]
- [[_COMMUNITY_Community 650|Community 650]]
- [[_COMMUNITY_Community 651|Community 651]]
- [[_COMMUNITY_Community 652|Community 652]]
- [[_COMMUNITY_Community 653|Community 653]]
- [[_COMMUNITY_Community 654|Community 654]]
- [[_COMMUNITY_Community 655|Community 655]]
- [[_COMMUNITY_Community 656|Community 656]]
- [[_COMMUNITY_Community 657|Community 657]]
- [[_COMMUNITY_Community 658|Community 658]]
- [[_COMMUNITY_Community 659|Community 659]]
- [[_COMMUNITY_Community 660|Community 660]]
- [[_COMMUNITY_Community 661|Community 661]]
- [[_COMMUNITY_Community 662|Community 662]]
- [[_COMMUNITY_Community 663|Community 663]]
- [[_COMMUNITY_Community 664|Community 664]]
- [[_COMMUNITY_Community 665|Community 665]]
- [[_COMMUNITY_Community 666|Community 666]]
- [[_COMMUNITY_Community 667|Community 667]]
- [[_COMMUNITY_Community 668|Community 668]]
- [[_COMMUNITY_Community 669|Community 669]]
- [[_COMMUNITY_Community 670|Community 670]]
- [[_COMMUNITY_Community 671|Community 671]]
- [[_COMMUNITY_Community 672|Community 672]]
- [[_COMMUNITY_Community 673|Community 673]]
- [[_COMMUNITY_Community 674|Community 674]]
- [[_COMMUNITY_Community 675|Community 675]]
- [[_COMMUNITY_Community 676|Community 676]]
- [[_COMMUNITY_Community 677|Community 677]]
- [[_COMMUNITY_Community 678|Community 678]]
- [[_COMMUNITY_Community 679|Community 679]]
- [[_COMMUNITY_Community 680|Community 680]]
- [[_COMMUNITY_Community 681|Community 681]]
- [[_COMMUNITY_Community 682|Community 682]]
- [[_COMMUNITY_Community 683|Community 683]]
- [[_COMMUNITY_Community 684|Community 684]]
- [[_COMMUNITY_Community 685|Community 685]]
- [[_COMMUNITY_Community 686|Community 686]]
- [[_COMMUNITY_Community 687|Community 687]]
- [[_COMMUNITY_Community 688|Community 688]]
- [[_COMMUNITY_Community 689|Community 689]]
- [[_COMMUNITY_Community 690|Community 690]]
- [[_COMMUNITY_Community 691|Community 691]]
- [[_COMMUNITY_Community 692|Community 692]]
- [[_COMMUNITY_Community 693|Community 693]]
- [[_COMMUNITY_Community 694|Community 694]]
- [[_COMMUNITY_Community 695|Community 695]]
- [[_COMMUNITY_Community 696|Community 696]]
- [[_COMMUNITY_Community 697|Community 697]]
- [[_COMMUNITY_Community 698|Community 698]]
- [[_COMMUNITY_Community 699|Community 699]]
- [[_COMMUNITY_Community 700|Community 700]]
- [[_COMMUNITY_Community 701|Community 701]]
- [[_COMMUNITY_Community 702|Community 702]]
- [[_COMMUNITY_Community 703|Community 703]]
- [[_COMMUNITY_Community 704|Community 704]]
- [[_COMMUNITY_Community 705|Community 705]]
- [[_COMMUNITY_Community 706|Community 706]]
- [[_COMMUNITY_Community 707|Community 707]]
- [[_COMMUNITY_Community 708|Community 708]]
- [[_COMMUNITY_Community 709|Community 709]]
- [[_COMMUNITY_Community 710|Community 710]]
- [[_COMMUNITY_Community 711|Community 711]]
- [[_COMMUNITY_Community 712|Community 712]]
- [[_COMMUNITY_Community 713|Community 713]]
- [[_COMMUNITY_Community 714|Community 714]]
- [[_COMMUNITY_Community 715|Community 715]]
- [[_COMMUNITY_Community 716|Community 716]]
- [[_COMMUNITY_Community 717|Community 717]]
- [[_COMMUNITY_Community 718|Community 718]]
- [[_COMMUNITY_Community 719|Community 719]]
- [[_COMMUNITY_Community 720|Community 720]]
- [[_COMMUNITY_Community 721|Community 721]]
- [[_COMMUNITY_Community 722|Community 722]]
- [[_COMMUNITY_Community 723|Community 723]]
- [[_COMMUNITY_Community 724|Community 724]]
- [[_COMMUNITY_Community 725|Community 725]]
- [[_COMMUNITY_Community 726|Community 726]]
- [[_COMMUNITY_Community 727|Community 727]]
- [[_COMMUNITY_Community 728|Community 728]]
- [[_COMMUNITY_Community 729|Community 729]]
- [[_COMMUNITY_Community 730|Community 730]]
- [[_COMMUNITY_Community 731|Community 731]]
- [[_COMMUNITY_Community 732|Community 732]]
- [[_COMMUNITY_Community 733|Community 733]]
- [[_COMMUNITY_Community 734|Community 734]]
- [[_COMMUNITY_Community 735|Community 735]]
- [[_COMMUNITY_Community 736|Community 736]]
- [[_COMMUNITY_Community 737|Community 737]]
- [[_COMMUNITY_Community 738|Community 738]]
- [[_COMMUNITY_Community 739|Community 739]]
- [[_COMMUNITY_Community 740|Community 740]]
- [[_COMMUNITY_Community 741|Community 741]]
- [[_COMMUNITY_Community 742|Community 742]]
- [[_COMMUNITY_Community 743|Community 743]]
- [[_COMMUNITY_Community 744|Community 744]]
- [[_COMMUNITY_Community 745|Community 745]]
- [[_COMMUNITY_Community 746|Community 746]]
- [[_COMMUNITY_Community 747|Community 747]]
- [[_COMMUNITY_Community 748|Community 748]]
- [[_COMMUNITY_Community 749|Community 749]]
- [[_COMMUNITY_Community 750|Community 750]]
- [[_COMMUNITY_Community 751|Community 751]]
- [[_COMMUNITY_Community 752|Community 752]]
- [[_COMMUNITY_Community 753|Community 753]]
- [[_COMMUNITY_Community 754|Community 754]]
- [[_COMMUNITY_Community 755|Community 755]]
- [[_COMMUNITY_Community 756|Community 756]]
- [[_COMMUNITY_Community 757|Community 757]]
- [[_COMMUNITY_Community 758|Community 758]]
- [[_COMMUNITY_Community 759|Community 759]]
- [[_COMMUNITY_Community 760|Community 760]]
- [[_COMMUNITY_Community 761|Community 761]]
- [[_COMMUNITY_Community 762|Community 762]]
- [[_COMMUNITY_Community 763|Community 763]]
- [[_COMMUNITY_Community 764|Community 764]]
- [[_COMMUNITY_Community 765|Community 765]]
- [[_COMMUNITY_Community 766|Community 766]]
- [[_COMMUNITY_Community 767|Community 767]]
- [[_COMMUNITY_Community 768|Community 768]]
- [[_COMMUNITY_Community 769|Community 769]]
- [[_COMMUNITY_Community 770|Community 770]]
- [[_COMMUNITY_Community 771|Community 771]]
- [[_COMMUNITY_Community 772|Community 772]]
- [[_COMMUNITY_Community 773|Community 773]]
- [[_COMMUNITY_Community 774|Community 774]]
- [[_COMMUNITY_Community 775|Community 775]]
- [[_COMMUNITY_Community 776|Community 776]]
- [[_COMMUNITY_Community 777|Community 777]]
- [[_COMMUNITY_Community 778|Community 778]]
- [[_COMMUNITY_Community 779|Community 779]]
- [[_COMMUNITY_Community 780|Community 780]]
- [[_COMMUNITY_Community 781|Community 781]]
- [[_COMMUNITY_Community 782|Community 782]]
- [[_COMMUNITY_Community 783|Community 783]]
- [[_COMMUNITY_Community 784|Community 784]]
- [[_COMMUNITY_Community 785|Community 785]]
- [[_COMMUNITY_Community 786|Community 786]]
- [[_COMMUNITY_Community 787|Community 787]]
- [[_COMMUNITY_Community 788|Community 788]]
- [[_COMMUNITY_Community 789|Community 789]]
- [[_COMMUNITY_Community 790|Community 790]]
- [[_COMMUNITY_Community 791|Community 791]]
- [[_COMMUNITY_Community 792|Community 792]]
- [[_COMMUNITY_Community 793|Community 793]]
- [[_COMMUNITY_Community 794|Community 794]]
- [[_COMMUNITY_Community 795|Community 795]]
- [[_COMMUNITY_Community 796|Community 796]]
- [[_COMMUNITY_Community 797|Community 797]]
- [[_COMMUNITY_Community 798|Community 798]]
- [[_COMMUNITY_Community 799|Community 799]]
- [[_COMMUNITY_Community 800|Community 800]]
- [[_COMMUNITY_Community 801|Community 801]]
- [[_COMMUNITY_Community 802|Community 802]]
- [[_COMMUNITY_Community 803|Community 803]]
- [[_COMMUNITY_Community 804|Community 804]]
- [[_COMMUNITY_Community 805|Community 805]]
- [[_COMMUNITY_Community 806|Community 806]]
- [[_COMMUNITY_Community 807|Community 807]]
- [[_COMMUNITY_Community 808|Community 808]]
- [[_COMMUNITY_Community 809|Community 809]]
- [[_COMMUNITY_Community 810|Community 810]]
- [[_COMMUNITY_Community 811|Community 811]]
- [[_COMMUNITY_Community 812|Community 812]]
- [[_COMMUNITY_Community 813|Community 813]]
- [[_COMMUNITY_Community 814|Community 814]]
- [[_COMMUNITY_Community 815|Community 815]]
- [[_COMMUNITY_Community 816|Community 816]]
- [[_COMMUNITY_Community 817|Community 817]]
- [[_COMMUNITY_Community 818|Community 818]]
- [[_COMMUNITY_Community 819|Community 819]]
- [[_COMMUNITY_Community 820|Community 820]]
- [[_COMMUNITY_Community 821|Community 821]]
- [[_COMMUNITY_Community 822|Community 822]]
- [[_COMMUNITY_Community 823|Community 823]]
- [[_COMMUNITY_Community 824|Community 824]]
- [[_COMMUNITY_Community 825|Community 825]]
- [[_COMMUNITY_Community 826|Community 826]]
- [[_COMMUNITY_Community 827|Community 827]]
- [[_COMMUNITY_Community 828|Community 828]]
- [[_COMMUNITY_Community 829|Community 829]]
- [[_COMMUNITY_Community 830|Community 830]]
- [[_COMMUNITY_Community 831|Community 831]]
- [[_COMMUNITY_Community 832|Community 832]]
- [[_COMMUNITY_Community 833|Community 833]]
- [[_COMMUNITY_Community 834|Community 834]]
- [[_COMMUNITY_Community 835|Community 835]]
- [[_COMMUNITY_Community 836|Community 836]]
- [[_COMMUNITY_Community 837|Community 837]]
- [[_COMMUNITY_Community 838|Community 838]]
- [[_COMMUNITY_Community 839|Community 839]]
- [[_COMMUNITY_Community 840|Community 840]]
- [[_COMMUNITY_Community 841|Community 841]]
- [[_COMMUNITY_Community 842|Community 842]]
- [[_COMMUNITY_Community 843|Community 843]]
- [[_COMMUNITY_Community 844|Community 844]]
- [[_COMMUNITY_Community 845|Community 845]]
- [[_COMMUNITY_Community 846|Community 846]]
- [[_COMMUNITY_Community 847|Community 847]]
- [[_COMMUNITY_Community 848|Community 848]]
- [[_COMMUNITY_Community 849|Community 849]]
- [[_COMMUNITY_Community 850|Community 850]]
- [[_COMMUNITY_Community 851|Community 851]]
- [[_COMMUNITY_Community 852|Community 852]]
- [[_COMMUNITY_Community 853|Community 853]]
- [[_COMMUNITY_Community 854|Community 854]]
- [[_COMMUNITY_Community 855|Community 855]]
- [[_COMMUNITY_Community 856|Community 856]]
- [[_COMMUNITY_Community 857|Community 857]]
- [[_COMMUNITY_Community 858|Community 858]]
- [[_COMMUNITY_Community 859|Community 859]]
- [[_COMMUNITY_Community 860|Community 860]]
- [[_COMMUNITY_Community 861|Community 861]]
- [[_COMMUNITY_Community 862|Community 862]]
- [[_COMMUNITY_Community 863|Community 863]]
- [[_COMMUNITY_Community 864|Community 864]]
- [[_COMMUNITY_Community 865|Community 865]]
- [[_COMMUNITY_Community 866|Community 866]]
- [[_COMMUNITY_Community 867|Community 867]]
- [[_COMMUNITY_Community 868|Community 868]]
- [[_COMMUNITY_Community 869|Community 869]]
- [[_COMMUNITY_Community 870|Community 870]]
- [[_COMMUNITY_Community 871|Community 871]]
- [[_COMMUNITY_Community 872|Community 872]]
- [[_COMMUNITY_Community 873|Community 873]]
- [[_COMMUNITY_Community 874|Community 874]]
- [[_COMMUNITY_Community 875|Community 875]]
- [[_COMMUNITY_Community 876|Community 876]]
- [[_COMMUNITY_Community 877|Community 877]]
- [[_COMMUNITY_Community 878|Community 878]]
- [[_COMMUNITY_Community 879|Community 879]]
- [[_COMMUNITY_Community 880|Community 880]]
- [[_COMMUNITY_Community 881|Community 881]]
- [[_COMMUNITY_Community 882|Community 882]]
- [[_COMMUNITY_Community 883|Community 883]]
- [[_COMMUNITY_Community 884|Community 884]]
- [[_COMMUNITY_Community 885|Community 885]]
- [[_COMMUNITY_Community 886|Community 886]]
- [[_COMMUNITY_Community 887|Community 887]]
- [[_COMMUNITY_Community 888|Community 888]]
- [[_COMMUNITY_Community 889|Community 889]]
- [[_COMMUNITY_Community 890|Community 890]]
- [[_COMMUNITY_Community 891|Community 891]]
- [[_COMMUNITY_Community 892|Community 892]]
- [[_COMMUNITY_Community 893|Community 893]]
- [[_COMMUNITY_Community 894|Community 894]]
- [[_COMMUNITY_Community 895|Community 895]]
- [[_COMMUNITY_Community 896|Community 896]]
- [[_COMMUNITY_Community 897|Community 897]]
- [[_COMMUNITY_Community 898|Community 898]]
- [[_COMMUNITY_Community 899|Community 899]]
- [[_COMMUNITY_Community 900|Community 900]]
- [[_COMMUNITY_Community 901|Community 901]]
- [[_COMMUNITY_Community 902|Community 902]]
- [[_COMMUNITY_Community 903|Community 903]]
- [[_COMMUNITY_Community 904|Community 904]]
- [[_COMMUNITY_Community 905|Community 905]]
- [[_COMMUNITY_Community 906|Community 906]]
- [[_COMMUNITY_Community 907|Community 907]]
- [[_COMMUNITY_Community 908|Community 908]]
- [[_COMMUNITY_Community 909|Community 909]]
- [[_COMMUNITY_Community 910|Community 910]]
- [[_COMMUNITY_Community 911|Community 911]]
- [[_COMMUNITY_Community 912|Community 912]]
- [[_COMMUNITY_Community 913|Community 913]]
- [[_COMMUNITY_Community 914|Community 914]]
- [[_COMMUNITY_Community 915|Community 915]]
- [[_COMMUNITY_Community 916|Community 916]]
- [[_COMMUNITY_Community 917|Community 917]]
- [[_COMMUNITY_Community 918|Community 918]]
- [[_COMMUNITY_Community 919|Community 919]]
- [[_COMMUNITY_Community 920|Community 920]]
- [[_COMMUNITY_Community 921|Community 921]]
- [[_COMMUNITY_Community 922|Community 922]]
- [[_COMMUNITY_Community 923|Community 923]]
- [[_COMMUNITY_Community 924|Community 924]]
- [[_COMMUNITY_Community 925|Community 925]]
- [[_COMMUNITY_Community 926|Community 926]]
- [[_COMMUNITY_Community 927|Community 927]]
- [[_COMMUNITY_Community 928|Community 928]]
- [[_COMMUNITY_Community 929|Community 929]]
- [[_COMMUNITY_Community 930|Community 930]]
- [[_COMMUNITY_Community 931|Community 931]]
- [[_COMMUNITY_Community 932|Community 932]]
- [[_COMMUNITY_Community 933|Community 933]]
- [[_COMMUNITY_Community 934|Community 934]]
- [[_COMMUNITY_Community 935|Community 935]]
- [[_COMMUNITY_Community 936|Community 936]]
- [[_COMMUNITY_Community 937|Community 937]]
- [[_COMMUNITY_Community 938|Community 938]]
- [[_COMMUNITY_Community 939|Community 939]]
- [[_COMMUNITY_Community 940|Community 940]]
- [[_COMMUNITY_Community 941|Community 941]]
- [[_COMMUNITY_Community 942|Community 942]]
- [[_COMMUNITY_Community 943|Community 943]]
- [[_COMMUNITY_Community 944|Community 944]]
- [[_COMMUNITY_Community 945|Community 945]]
- [[_COMMUNITY_Community 946|Community 946]]
- [[_COMMUNITY_Community 947|Community 947]]
- [[_COMMUNITY_Community 948|Community 948]]
- [[_COMMUNITY_Community 949|Community 949]]
- [[_COMMUNITY_Community 950|Community 950]]
- [[_COMMUNITY_Community 951|Community 951]]
- [[_COMMUNITY_Community 952|Community 952]]
- [[_COMMUNITY_Community 953|Community 953]]
- [[_COMMUNITY_Community 954|Community 954]]
- [[_COMMUNITY_Community 955|Community 955]]
- [[_COMMUNITY_Community 956|Community 956]]
- [[_COMMUNITY_Community 957|Community 957]]
- [[_COMMUNITY_Community 958|Community 958]]
- [[_COMMUNITY_Community 959|Community 959]]
- [[_COMMUNITY_Community 960|Community 960]]
- [[_COMMUNITY_Community 961|Community 961]]
- [[_COMMUNITY_Community 962|Community 962]]
- [[_COMMUNITY_Community 963|Community 963]]
- [[_COMMUNITY_Community 964|Community 964]]
- [[_COMMUNITY_Community 965|Community 965]]
- [[_COMMUNITY_Community 966|Community 966]]
- [[_COMMUNITY_Community 967|Community 967]]
- [[_COMMUNITY_Community 968|Community 968]]
- [[_COMMUNITY_Community 969|Community 969]]
- [[_COMMUNITY_Community 970|Community 970]]
- [[_COMMUNITY_Community 971|Community 971]]
- [[_COMMUNITY_Community 972|Community 972]]
- [[_COMMUNITY_Community 973|Community 973]]
- [[_COMMUNITY_Community 974|Community 974]]
- [[_COMMUNITY_Community 975|Community 975]]
- [[_COMMUNITY_Community 976|Community 976]]
- [[_COMMUNITY_Community 977|Community 977]]
- [[_COMMUNITY_Community 978|Community 978]]
- [[_COMMUNITY_Community 979|Community 979]]
- [[_COMMUNITY_Community 980|Community 980]]
- [[_COMMUNITY_Community 981|Community 981]]
- [[_COMMUNITY_Community 982|Community 982]]
- [[_COMMUNITY_Community 983|Community 983]]
- [[_COMMUNITY_Community 984|Community 984]]
- [[_COMMUNITY_Community 985|Community 985]]
- [[_COMMUNITY_Community 986|Community 986]]
- [[_COMMUNITY_Community 987|Community 987]]
- [[_COMMUNITY_Community 988|Community 988]]
- [[_COMMUNITY_Community 989|Community 989]]
- [[_COMMUNITY_Community 990|Community 990]]
- [[_COMMUNITY_Community 991|Community 991]]
- [[_COMMUNITY_Community 992|Community 992]]
- [[_COMMUNITY_Community 993|Community 993]]
- [[_COMMUNITY_Community 994|Community 994]]
- [[_COMMUNITY_Community 995|Community 995]]
- [[_COMMUNITY_Community 996|Community 996]]
- [[_COMMUNITY_Community 997|Community 997]]
- [[_COMMUNITY_Community 998|Community 998]]
- [[_COMMUNITY_Community 999|Community 999]]
- [[_COMMUNITY_Community 1000|Community 1000]]
- [[_COMMUNITY_Community 1001|Community 1001]]
- [[_COMMUNITY_Community 1002|Community 1002]]
- [[_COMMUNITY_Community 1003|Community 1003]]
- [[_COMMUNITY_Community 1004|Community 1004]]
- [[_COMMUNITY_Community 1005|Community 1005]]
- [[_COMMUNITY_Community 1006|Community 1006]]
- [[_COMMUNITY_Community 1007|Community 1007]]
- [[_COMMUNITY_Community 1008|Community 1008]]
- [[_COMMUNITY_Community 1009|Community 1009]]
- [[_COMMUNITY_Community 1010|Community 1010]]
- [[_COMMUNITY_Community 1011|Community 1011]]
- [[_COMMUNITY_Community 1012|Community 1012]]
- [[_COMMUNITY_Community 1013|Community 1013]]
- [[_COMMUNITY_Community 1014|Community 1014]]
- [[_COMMUNITY_Community 1015|Community 1015]]
- [[_COMMUNITY_Community 1016|Community 1016]]
- [[_COMMUNITY_Community 1017|Community 1017]]
- [[_COMMUNITY_Community 1018|Community 1018]]
- [[_COMMUNITY_Community 1019|Community 1019]]
- [[_COMMUNITY_Community 1020|Community 1020]]
- [[_COMMUNITY_Community 1021|Community 1021]]
- [[_COMMUNITY_Community 1022|Community 1022]]
- [[_COMMUNITY_Community 1023|Community 1023]]
- [[_COMMUNITY_Community 1024|Community 1024]]
- [[_COMMUNITY_Community 1025|Community 1025]]
- [[_COMMUNITY_Community 1026|Community 1026]]
- [[_COMMUNITY_Community 1027|Community 1027]]
- [[_COMMUNITY_Community 1028|Community 1028]]
- [[_COMMUNITY_Community 1029|Community 1029]]
- [[_COMMUNITY_Community 1030|Community 1030]]
- [[_COMMUNITY_Community 1031|Community 1031]]
- [[_COMMUNITY_Community 1032|Community 1032]]
- [[_COMMUNITY_Community 1033|Community 1033]]
- [[_COMMUNITY_Community 1034|Community 1034]]
- [[_COMMUNITY_Community 1035|Community 1035]]
- [[_COMMUNITY_Community 1036|Community 1036]]
- [[_COMMUNITY_Community 1037|Community 1037]]
- [[_COMMUNITY_Community 1038|Community 1038]]
- [[_COMMUNITY_Community 1039|Community 1039]]
- [[_COMMUNITY_Community 1040|Community 1040]]
- [[_COMMUNITY_Community 1041|Community 1041]]
- [[_COMMUNITY_Community 1042|Community 1042]]
- [[_COMMUNITY_Community 1043|Community 1043]]
- [[_COMMUNITY_Community 1044|Community 1044]]
- [[_COMMUNITY_Community 1045|Community 1045]]
- [[_COMMUNITY_Community 1046|Community 1046]]
- [[_COMMUNITY_Community 1047|Community 1047]]
- [[_COMMUNITY_Community 1048|Community 1048]]
- [[_COMMUNITY_Community 1049|Community 1049]]
- [[_COMMUNITY_Community 1050|Community 1050]]
- [[_COMMUNITY_Community 1051|Community 1051]]
- [[_COMMUNITY_Community 1052|Community 1052]]
- [[_COMMUNITY_Community 1053|Community 1053]]
- [[_COMMUNITY_Community 1054|Community 1054]]
- [[_COMMUNITY_Community 1055|Community 1055]]
- [[_COMMUNITY_Community 1056|Community 1056]]
- [[_COMMUNITY_Community 1057|Community 1057]]
- [[_COMMUNITY_Community 1058|Community 1058]]
- [[_COMMUNITY_Community 1059|Community 1059]]
- [[_COMMUNITY_Community 1060|Community 1060]]
- [[_COMMUNITY_Community 1061|Community 1061]]
- [[_COMMUNITY_Community 1062|Community 1062]]
- [[_COMMUNITY_Community 1063|Community 1063]]
- [[_COMMUNITY_Community 1064|Community 1064]]
- [[_COMMUNITY_Community 1065|Community 1065]]
- [[_COMMUNITY_Community 1066|Community 1066]]
- [[_COMMUNITY_Community 1067|Community 1067]]
- [[_COMMUNITY_Community 1068|Community 1068]]
- [[_COMMUNITY_Community 1069|Community 1069]]
- [[_COMMUNITY_Community 1070|Community 1070]]
- [[_COMMUNITY_Community 1071|Community 1071]]
- [[_COMMUNITY_Community 1072|Community 1072]]
- [[_COMMUNITY_Community 1073|Community 1073]]
- [[_COMMUNITY_Community 1074|Community 1074]]
- [[_COMMUNITY_Community 1075|Community 1075]]
- [[_COMMUNITY_Community 1076|Community 1076]]
- [[_COMMUNITY_Community 1077|Community 1077]]
- [[_COMMUNITY_Community 1078|Community 1078]]
- [[_COMMUNITY_Community 1079|Community 1079]]
- [[_COMMUNITY_Community 1080|Community 1080]]
- [[_COMMUNITY_Community 1082|Community 1082]]
- [[_COMMUNITY_Community 1083|Community 1083]]
- [[_COMMUNITY_Community 1084|Community 1084]]
- [[_COMMUNITY_Community 1085|Community 1085]]
- [[_COMMUNITY_Community 1086|Community 1086]]

## God Nodes (most connected - your core abstractions)
1. `cn()` - 212 edges
2. `UIMessage` - 66 edges
3. `getCurrentUserId()` - 45 edges
4. `columns` - 41 edges
5. `columns` - 41 edges
6. `columns` - 41 edges
7. `columns` - 41 edges
8. `columns` - 41 edges
9. `columns` - 41 edges
10. `columns` - 41 edges

## Surprising Connections (you probably didn't know these)
- `Generative UI` --conceptually_related_to--> `Generative UI Components Catalog`  [INFERRED]
  README.md → docs/architecture/GENERATIVE-UI-COMPONENTS.md
- `RootLayout()` --calls--> `cn()`  [EXTRACTED]
  app/layout.tsx → lib/utils/index.ts
- `ActivityFetchItemProps` --references--> `ToolPart`  [EXTRACTED]
  components/activity/activity-fetch-item.tsx → lib/types/ai.ts
- `ActivitySearchItemProps` --references--> `ToolPart`  [EXTRACTED]
  components/activity/activity-search-item.tsx → lib/types/ai.ts
- `DiagnosticItem()` --calls--> `cn()`  [EXTRACTED]
  components/canvas/canvas-diagnostics-panel.tsx → lib/utils/index.ts

## Import Cycles
- None detected.

## Hyperedges (group relationships)
- **Tool UI serializable display-contract schemas** — callout_schema_serializablecalloutschema, chart_schema_chartpropsschema, citation_schema_serializablecitationschema, data_table_schema_serializabledatatableschema [INFERRED 0.85]
- **Geo-map renderer sub-modules forming the Leaflet map** — geo_map_geo_map_engine_geomapengine, geo_map_geo_map_overlays_geomapoverlays, geo_map_geo_map_icons_resolvemarkericon, geo_map_adapter_contract [INFERRED 0.85]
- **Components tracking step progress** — plan_plan_planroot, progress_tracker_progress_tracker_progresstrackerlive, tool_ui_step_progress_pattern [INFERRED 0.75]
- **Registry-driven tool result renderer catalogs dispatch by tool name** — tool_ui_renderer_catalog_tooluirendererentries, tool_ui_interactive_renderer_catalog_interactivetoolrendererentries, tool_ui_registry_additionalresultrendererentries [INFERRED 0.85]
- **Tool renderers wrap output in ToolErrorBoundary** — tool_ui_tool_error_boundary_toolerrorboundary, tool_ui_renderer_catalog_tooluirendererentries, tool_ui_registry_additionalresultrendererentries [INFERRED 0.85]
- **Per-tool serializable schemas implementing the Tool UI contract** — geo_map_schema_serializablegeomapschema, link_preview_schema_serializablelinkpreviewschema, generate_image_schema_serializablegenerateimageschema [INFERRED 0.80]
- **Tool UI schemas implementing the shared adapter contract** — plan_schema_serializableplanschemacontract, progress_tracker_schema_serializableprogresstrackerschemacontract, question_wizard_schema_contract, option_list_schema_serializableoptionlistschemacontract, shared_contract_tooluicontract [INFERRED 0.85]
- **Shared media presentation/security helpers** — media_sanitize_href_sanitizehref, media_safe_navigation_resolvesafenavigationhref, media_aspect_ratio_getratioclass, media_format_utils_formatduration, media_overlay_gradient_overlay_gradient [INFERRED 0.75]
- **Tool UI base schema shared identity/receipt/role** — shared_schema_tooluiidschema, shared_schema_tooluireceiptschema, shared_schema_tooluiroleschema, timeline_schema_serializabletimelineschema [INFERRED 0.75]
- **Per-mode chat agent definitions built through the shared factory** — chat_build_createbuildagentdefinition, chat_research_createresearchagentdefinition, chat_search_createsearchagentdefinition, chat_factory_createconfiguredchatagent [INFERRED 0.85]
- **Chat agent resolution and dispatch flow** — chat_route_handler_handlechatagentroute, chat_registry_resolvechatagentid, chat_registry_createchatagentbyid [INFERRED 0.85]
- **Competitor research evidence-gathering pipeline** — specialists_competitor_research_createcompetitorresearchtool, specialists_competitor_research_gathercompetitorevidence, specialists_competitor_research_buildcompetitorresearchoutput [INFERRED 0.85]
- **Canvas allowed-packages derivation chain** — vendor_chunk_defs_vendor_chunk_defs, canvas_allowed_packages_canvas_allowed_packages, canvas_allowed_packages_isallowedcanvasimport [INFERRED 0.75]
- **Canvas compile pipeline: validate -> bundle -> tailwind -> assemble** — validation_validate_canvas_source_validatecanvassource, compiler_compile_canvas_artifact_createvirtualplugin, compiler_build_tailwind_css_buildtailwindcss, compiler_assemble_canvas_html_assemblecanvashtml [EXTRACTED 0.85]
- **Canvas source pre-processing chain** — pre_processors_run_pre_processors_runpreprocessors, pre_processors_fix_missing_default_export_fixmissingdefaultexport, pre_processors_fix_hallucinated_imports_fixhallucinatedimports [EXTRACTED 0.85]
- **Canvas HTML serving auth and rendering flow** — canvas_serve_canvas_html_servecanvashtml, canvas_service_exportcanvasartifacthtml, canvas_inject_viewport_fit_injectviewportfitstyles [INFERRED 0.75]
- **Canvas artifact create/update/restore service flow** — canvas_service_createcanvasartifactfromsource, canvas_service_updatecanvasartifactdraftfromsource, canvas_service_restorecanvasartifactversion, compiler_compile_canvas_artifact_compilecanvasartifact [INFERRED 0.75]
- **Model resolution accessors share the cached ModelsConfig via getModelsConfig** — config_model_types_getmodelformodeandtype, config_model_types_getrelatedquestionsmodel, config_model_types_gettrendingsuggestionsmodel, config_load_models_config_getmodelsconfig [EXTRACTED 0.85]
- **User-scoped DB actions route through withRLS/withOptionalRLS to set app.current_user_id GUC** — db_with_rls_withrls, db_with_rls_withoptionalrls, db_actions_createchat, db_actions_getchats, db_schema_chats [EXTRACTED 0.85]
- **Suite dashboard getters build snapshots via shared select/build helpers** — evals_queries_getcapabilitydashboard, evals_queries_getregressiondashboard, evals_queries_gettrafficmonitordashboard, evals_queries_getevalsdashboard [INFERRED 0.85]
- **Per-feature rate limiters implement the Redis-with-memory-fallback contract** — rate_limit_per_minute_limiter_checkperminutelimit, rate_limit_chat_limits_checkoverallchatlimit, rate_limit_guest_limit_checkguestlimit [INFERRED 0.85]
- **Canvas/voice/feedback limiters delegate to the per-minute limiter primitive** — rate_limit_canvas_limits_checkandenforcecanvaslimit, rate_limit_voice_limits_checkandenforcevoicelimit, rate_limit_feedback_limits_checkfeedbacklimit [INFERRED 0.85]
- **prepare -> agent execute -> persist chat stream lifecycle** — streaming_create_chat_stream_response_createchatstreamresponse, helpers_prepare_messages_preparemessages, helpers_persist_stream_results_persiststreamresults [INFERRED 0.85]
- **UIMessage -> model message transform pipeline** — helpers_strip_reasoning_parts_stripreasoningparts, helpers_inline_file_urls_inlinefileurls, helpers_prepare_messages_preparemessages [INFERRED 0.75]
- **chat / ephemeral / eval stream entry points** — streaming_create_chat_stream_response_createchatstreamresponse, streaming_create_ephemeral_chat_stream_response_createephemeralchatstreamresponse, streaming_eval_chat_runner_runevalchat [INFERRED 0.85]
- **UIMessage generic type backbone** — types_ai_uimessage, types_ai_uimessagemetadata, types_ai_uidatatypes, types_ai_uitools [EXTRACTED 0.85]
- **Canvas data-part payload contract** — types_ai_canvasartifactdata, types_ai_canvasartifactstatusdata, types_ai_canvasartifacteventdata, types_ai_canvasdiagnosticsdata [EXTRACTED 0.85]
- **Dynamic tool part state variants** — types_dynamic_tools_dynamictoolpartinputstreaming, types_dynamic_tools_dynamictoolpartinputavailable, types_dynamic_tools_dynamictoolpartoutputavailable, types_dynamic_tools_dynamictoolpartoutputerror [EXTRACTED 0.85]
- **Agent Artifact Port Provenance** — agent_artifact_readme_port, agent_artifact_upstream_license_noncommercial, agent_artifact_readme_adapter, generative_ui_adding_tools_community_sources [EXTRACTED 1.00]
- **AI Provider Registry Mechanism** — ai_provider_registry_create_provider_registry, ai_provider_registry_get_model, ai_provider_registry_is_provider_enabled [EXTRACTED 1.00]
- **Generative UI Display Component Family** — plan_readme_plan, data_table_readme_data_table, chart_readme_chart, citation_readme_citation, timeline_readme_timeline, callout_readme_callout, option_list_readme_option_list [EXTRACTED 1.00]
- **Generative UI Render Pipeline** — generative_ui_display_tools_display_tools, generative_ui_message_rendering_render_message, generative_ui_registry_adapters_try_render_by_name, generative_ui_registry_adapters_define_contract [EXTRACTED 0.90]
- **Compose-First Geo Tool Chain** — geo_tools_geocode_address, geo_tools_get_directions, geo_tools_get_isochrone, geo_tools_display_geo_map_contract [EXTRACTED 0.85]
- **Chat Agent Dispatch by Mode** — overview_agent_pipeline_chat_registry, overview_agent_pipeline_search_agent, overview_agent_pipeline_research_agent, overview_agent_pipeline_build_agent, overview_agent_pipeline_toolset [EXTRACTED 0.90]
- **Spatial Helper Tools Group** — research_agent_core_tools_geocode_address, research_agent_core_tools_get_directions, research_agent_core_tools_get_isochrone, research_agent_core_tools_get_static_map_image [EXTRACTED 1.00]
- **Chat Request Pipeline Flow** — overview_system_chat_route, research_agent_pipeline_select_model, research_agent_pipeline_handle_chat_agent_route, research_agent_pipeline_create_chat_stream, research_agent_pipeline_tool_loop_agent [EXTRACTED 0.95]
- **Search Provider Implementations** — research_agent_search_providers_brave, research_agent_search_providers_tavily, research_agent_search_providers_exa, research_agent_search_providers_searxng, research_agent_search_providers_firecrawl [EXTRACTED 1.00]
- **Message Preparation Transform Pipeline** — streaming_message_preparation_stripreasoningparts, streaming_message_preparation_converttomodelmessages, streaming_message_preparation_prunemessages, streaming_message_preparation_truncatemessages [EXTRACTED 0.90]
- **Chat Request Streaming Lifecycle** — streaming_request_dispatch_chat_route, streaming_request_dispatch_handlechatagentroute, streaming_request_dispatch_createchatstreamresponse, streaming_agent_finalization_preparemessages, streaming_agent_finalization_toolloopagent, streaming_agent_finalization_persiststreamresults [EXTRACTED 0.90]
- **Tool-Selection Failure Taxonomy** — tool_selection_failure_modes_correct_tool, tool_selection_failure_modes_wrong_tool, tool_selection_failure_modes_missing_tool, tool_selection_failure_modes_not_required [EXTRACTED 0.90]
- **Phoenix Persistence Acid Test** — phoenix_operations_phoenix_service, phoenix_operations_railway_volume, phoenix_operations_persistence_verification, phoenix_operations_api_key_rotation [EXTRACTED 0.85]
- **Evals Cron Pipeline (run, sample, failure modes)** — api_evals_limits_evals_run, api_evals_limits_sampler, day_2_operations_polymorph_evals_cron, day_2_operations_evals_cron_failure_modes [INFERRED 0.80]
- **Canvas Artifact Lifecycle Endpoints** — api_canvas_endpoints_patch_draft, api_canvas_endpoints_post_versions, api_canvas_endpoints_post_restore, api_canvas_endpoints_view, api_canvas_endpoints_guest_token [EXTRACTED 0.90]

## Communities (1087 total, 39 thin omitted)

### Community 0 - "Project Root"
Cohesion: 0.02
Nodes (97): dependencies, ai, @ai-sdk/anthropic, @ai-sdk/gateway, @ai-sdk/google, @ai-sdk/openai, @ai-sdk/react, @arizeai/openinference-core (+89 more)

### Community 1 - "Utilities"
Cohesion: 0.05
Nodes (60): ACTIVE_PILL_CLASSES, ModeSelector(), {
  getCookieMock,
  mapSearchModeCookieValueMock,
  readSearchModeCookieMock,
  syncSearchModeMock
}, getModelsConfig(), loadModelsConfig(), loadModelsConfigSync(), ModelsConfig, VALID_MODEL_TYPES (+52 more)

### Community 2 - "Components"
Cohesion: 0.04
Nodes (56): ActivitySearchItem(), ActivitySearchItemProps, CREATED_BY_ICON, CREATED_BY_LABELS, CollapsibleMessage(), CollapsibleMessageProps, DragOverlay(), ProcessHeader() (+48 more)

### Community 3 - "Database & Schema"
Cohesion: 0.05
Nodes (69): MessageFeedbackUpdateResult, appendArtifactRevision(), AppendArtifactRevisionInput, ArtifactStatus, claimArtifactForRebuild(), clearAllChats(), CreateArtifactInput, createArtifactRecord() (+61 more)

### Community 4 - "Components"
Cohesion: 0.05
Nodes (52): AnswerSection(), AnswerSectionProps, Attachment, AttachmentPreview(), AttachmentPreviewProps, ChatMessages(), ChatMessagesProps, toolTypes (+44 more)

### Community 5 - "Rate Limiting"
Cohesion: 0.07
Nodes (38): feedbackInputSchema, submitFeedback(), mockCreateClient, mockGetUser, POST(), Feedback, checkAndEnforceOverallChatLimit(), checkOverallChatLimit() (+30 more)

### Community 6 - "Server Actions"
Cohesion: 0.07
Nodes (55): clearChats(), createChat(), createChatAndSaveMessage(), createChatWithFirstMessage(), deleteChat(), deleteMessagesAfter(), deleteMessagesFromIndex(), getChats() (+47 more)

### Community 7 - "Charts"
Cohesion: 0.06
Nodes (51): Area(), AreaProps, AreaChart(), AreaChartProps, ChartInnerProps, DEFAULT_MARGIN, CurveFactory, ChartContext (+43 more)

### Community 8 - "React Hooks"
Cohesion: 0.06
Nodes (47): ChatStatus, { mockUseVoiceInput, mockUseVoicePlayer, isQuotaExhausted }, useVoiceConversation(), UseVoiceConversationOptions, UseVoiceConversationReturn, getSpeechRecognition(), getSpeechSupportSnapshot(), SpeechRecognitionInstance (+39 more)

### Community 9 - "Components"
Cohesion: 0.06
Nodes (37): DEFAULT_COLORS, ChatError(), ChatErrorProps, ForgotPasswordForm(), mockResetPasswordForEmail, LoginForm(), mockPush, mockRefresh (+29 more)

### Community 10 - "Streaming"
Cohesion: 0.05
Nodes (66): createRelatedQuestionsStream(), ChatAgent, createChatValidationContract(), AuthenticatedRouteContext, BaseValidatedRouteContext, ChatRouteTrigger, createRouteAgentFactory(), GuestRouteContext (+58 more)

### Community 11 - "Tools"
Cohesion: 0.09
Nodes (36): DisplayOptionListInput, DisplayOptionListOutput, inputSchema, optionSchema, outputSchema, toolName, serverTool, DisplayQuestionWizardInput (+28 more)

### Community 12 - "UI Primitives"
Cohesion: 0.06
Nodes (43): ErrorModal(), ErrorModalProps, FeedbackModalProps, Sentiment, NewUserDemoPopup(), NewUserDemoPopupProps, FilteredImagesState, FilterStatus (+35 more)

### Community 13 - "Canvas Engine"
Cohesion: 0.15
Nodes (15): buildTailwindCss(), extractCandidates(), buildCompileSteps(), COMPILE_STEP_DEFINITIONS, compileCanvasArtifactCore(), CompileCanvasArtifactInput, CompileCanvasArtifactResult, createVirtualPlugin() (+7 more)

### Community 14 - "Evals Service"
Cohesion: 0.09
Nodes (28): CitationAccuracySchema, createCitationAccuracyExperimentEvaluator(), createFaithfulnessExperimentEvaluator(), FAITHFULNESS_PROMPT_TEMPLATE, FaithfulnessRecord, mockEvaluate, createRelevanceExperimentEvaluator(), RELEVANCE_PROMPT_TEMPLATE (+20 more)

### Community 15 - "Canvas UI"
Cohesion: 0.03
Nodes (67): ActivityAction, ActivityContext, ActivityContextValue, ActivityItem, ActivityItemData, ActivityProvider(), ActivityState, initialState (+59 more)

### Community 16 - "Tools"
Cohesion: 0.12
Nodes (25): BaseSearchProvider, SearchProvider, SearchTelemetryHook, BraveImageResult, BraveSearchProvider, BraveVideoResult, BraveWebResult, createHttpSearchError() (+17 more)

### Community 17 - "Agents"
Cohesion: 0.10
Nodes (40): BUILD_AGENT_ACTIVE_TOOLS, createBuildAgent(), createBuildAgentDefinition(), isEvalMode(), ChatAgentDefinition, ChatAgentId, CreateChatAgentArgs, createConfiguredChatAgent() (+32 more)

### Community 18 - "Evals Library"
Cohesion: 0.09
Nodes (28): ComparisonTable(), ScoreValue(), clampScore(), getScoreStatus(), getScoreStatusLabel(), ScoreBar(), ScoreBarStatus, STATUS_FILL_CLASS (+20 more)

### Community 19 - ""
Cohesion: 0.15
Nodes (17): OpenRouter Provider, Architecture Decisions (ADRs), Arize Phoenix Observability Decision, Scheduled Jobs (Vercel + Railway cron), Search & Extraction Provider Strategy, Supabase Auth/Backend Decision, polymorph Service, polymorph-stack Compose Environment (+9 more)

### Community 20 - "Evals UI"
Cohesion: 0.09
Nodes (32): ScoreFeature(), SNAP, pct(), CollapsibleComparison(), SNAP, getLastSyncText(), Header(), BADGE_TINT (+24 more)

### Community 21 - "Components"
Cohesion: 0.12
Nodes (22): ActivityFetchItem(), ActivityFetchItemProps, CitationLinkProps, CurrentUserAvatar(), SearchResults(), SearchResultsProps, VideoCarouselDialogProps, VideoResultGrid() (+14 more)

### Community 22 - "Server Actions"
Cohesion: 0.07
Nodes (21): applyOrdering(), assertWriteAccess(), canReadRow(), clone(), columnProperty(), DeleteBuilder, dialect, Gate (+13 more)

### Community 23 - "UI Primitives"
Cohesion: 0.08
Nodes (34): CanvasEditor(), CanvasFile, ConflictState, cssExtensions, getExtensionsForFile(), isReadOnly(), jsonExtensions, tsxExtensions (+26 more)

### Community 24 - "Evals Library"
Cohesion: 0.08
Nodes (34): EvalsDashboardV2(), EMPTY, mockSearchParamGet, POPULATED, POPULATED_CAPABILITY, evalCaseResults, evalSummaries, EvalsPage() (+26 more)

### Community 25 - ""
Cohesion: 0.05
Nodes (47): Architecture RLS Policy Chain, Implementation details, Policy details, RLS Policy Chain, Architecture Streaming Overview, Key implementation details, Streaming Architecture, Two stream paths (+39 more)

### Community 26 - "Tool UI"
Cohesion: 0.08
Nodes (34): ResearchStatusLine(), ResearchStatusLineProps, DisplayToolPart, renderLoadingToolPlaceholder(), renderToolPart(), RenderToolPartArgs, ToolPartState, DisplayToolPart (+26 more)

### Community 27 - "Canvas UI"
Cohesion: 0.05
Nodes (40): created_at, current_revision_id, current_runtime_session_id, framework, title, user_id, visibility, default (+32 more)

### Community 28 - "App Routes"
Cohesion: 0.03
Nodes (72): injectViewportFitStyles(), Opaque-origin sandbox isolation for canvas HTML, getPublicArtifactOwnerUserId(), serveCanvasHtml(), slugify(), exportCanvasArtifactHtml(), canvasSourceSchema, draftRequestSchema (+64 more)

### Community 29 - "React Hooks"
Cohesion: 0.08
Nodes (23): PolymorphWordmark(), PolySuffixFluid(), SUFFIX_MAX_LEN, SUFFIX_WORDS, close, connect, createMediaElementSource, createMediaStreamDestination (+15 more)

### Community 30 - "Evals Service"
Cohesion: 0.13
Nodes (31): getCorpusVersion(), createNoToolPlaceholdersExperimentEvaluator(), PLACEHOLDER_PATTERNS, runRegressionSuite(), buildDatasetExamples(), buildEvalSummaryMetadata(), buildExperimentEvaluators(), buildExperimentTask() (+23 more)

### Community 31 - "Canvas Engine"
Cohesion: 0.06
Nodes (55): GET(), loadPublicChatArtifactState(), mockGetChat, mockGetCurrentUserId, mockLoadCanvasArtifactByChatId, mockLoadCanvasArtifactState, mockVerifyGuestCanvasToken, Draft revision optimistic concurrency (+47 more)

### Community 32 - "Tool UI"
Cohesion: 0.08
Nodes (29): GeoMap, GeoMapEngine, GeoMapCenterViewportSchema, GeoMapClientProps, GeoMapClustering, GeoMapClusteringSchema, GeoMapFitTarget, GeoMapFitTargetSchema (+21 more)

### Community 33 - "Canvas Engine"
Cohesion: 0.38
Nodes (6): fixHallucinatedImports(), countSupportedAppDeclarations(), fixMissingDefaultExport(), SUPPORTED_APP_DECLARATIONS, runPreProcessors(), CanvasSourceFiles

### Community 34 - "Tool UI"
Cohesion: 0.09
Nodes (34): inferTodoProgress(), ResearchPlan(), ResearchPlanProps, toDomIdSegment(), mapTodoWriteToPlan(), STATUS_MAP, TodoWriteOutput, toPlanTodo() (+26 more)

### Community 35 - "Tool UI"
Cohesion: 0.05
Nodes (39): expires_at, last_heartbeat_at, preview_url, provider, sandbox_id, started_at, status, name (+31 more)

### Community 36 - "Tool UI"
Cohesion: 0.12
Nodes (25): AgentArtifact(), artifactIcons, findNextNonBlankLineIndex(), getDownloadFilename(), isMarkdownTableSeparator(), parseCsvTable(), ParsedTable, parseMarkdownTable() (+17 more)

### Community 37 - "Components"
Cohesion: 0.06
Nodes (46): ActivityItemRenderer(), CanvasCompileProgress(), CanvasCompileProgressProps, getElapsedTime(), CanvasDiagnosticsPanel(), DiagnosticItem(), DiagnosticSection(), hasDiagnostics() (+38 more)

### Community 38 - "Evals Library"
Cohesion: 0.15
Nodes (18): DASHBOARD_SUITE_BY_PERSISTED, getAlertedSuite(), getDefaultSuite(), getFirstAvailableSuite(), persistedSuiteToDashboardSuite(), BASE_SNAPSHOT, EMPTY, DashboardWithViews() (+10 more)

### Community 39 - "Evals Service"
Cohesion: 0.14
Nodes (28): asRecord(), ChatSampleRow, citationsFromMessage(), citationsFromSearchOutput(), conversationMessageFromRow(), dedupeCitations(), dedupeSearchResults(), dedupeStrings() (+20 more)

### Community 40 - "Charts"
Cohesion: 0.11
Nodes (20): ChartStatFlow(), ChartStatFlowFormat, ChartStatFlowProps, defaultChartStatFlowFormat, DEFAULT_ACTIVE_GRADIENT, Gauge(), GaugeInnerProps, GaugeProps (+12 more)

### Community 41 - "Charts"
Cohesion: 0.14
Nodes (22): getStrokeWidth(), RadarArea(), RadarAreaProps, RadarAxis(), RadarAxisProps, RadarChart(), RadarChartInnerProps, RadarChartProps (+14 more)

### Community 42 - "Evals Service"
Cohesion: 0.20
Nodes (11): runCapabilitySuite(), config, client, closeDb(), EvalSummaryPersistError, main(), validateJudgeCredentials(), buildPersistCause() (+3 more)

### Community 43 - "Tool UI"
Cohesion: 0.10
Nodes (24): DataTableAccordionCard(), DataTableAccordionCardProps, DataTableBaseProps, DataTableBody(), DataTableCell(), DataTableCellProps, DataTableComponent, DataTableContext (+16 more)

### Community 44 - "Agents"
Cohesion: 0.11
Nodes (25): mockGenerateTrendingSuggestions, ActionButtonsProps, ActionCategory, CATEGORY_SEED_OFFSETS, dayOfEpoch(), DEFAULT_SUGGESTIONS, seededRandom(), selectDailySuggestionsFromPool() (+17 more)

### Community 45 - "Evals UI"
Cohesion: 0.08
Nodes (33): ExpandedRow(), deltaPts(), AutoBadge(), COLOR, Delta(), Direction, ICON, buildDiagnosticOverview() (+25 more)

### Community 46 - ""
Cohesion: 0.05
Nodes (41): Display Tools (Server), Generative UI Display Tools, Schema example (displayTable), Tool definitions, AnswerSection, Buffer-and-flush explained, Display tool rendering, Display tool text suppression (+33 more)

### Community 47 - "Tools"
Cohesion: 0.11
Nodes (27): AdvancedSearchInput, advancedSearchSchema, advancedSearchXNGSearch(), calculateRelevanceScore(), crawlPage(), extractPublicationDate(), fetchHtml(), fetchHtmlWithTimeout() (+19 more)

### Community 48 - "Components"
Cohesion: 0.08
Nodes (34): chat, research, quality, speed, models, byMode, relatedQuestions, trendingSuggestions (+26 more)

### Community 49 - "Evals Service"
Cohesion: 0.10
Nodes (22): buildGoldenSearchResults(), ExpectedEvaluatorResult, GoldenExample, GoldenExampleInput, GoldenExpected, EvalContextLike, fixtureSample, mockCloseDb (+14 more)

### Community 50 - "Tool UI"
Cohesion: 0.09
Nodes (27): tryRenderResult(), tryRenderResult(), tryRenderResult(), tryRenderResult(), tryRenderResult(), tryRenderResult(), tryRenderResult(), tryRenderResult() (+19 more)

### Community 51 - "Components"
Cohesion: 0.09
Nodes (12): buildChatRequestBody(), getLatestGuestCanvasToken(), mockCanvasContext, mockChatMessages, mockChatPanel, mockNewUserDemoPopup, mockSidebar, mockToast (+4 more)

### Community 52 - "Components"
Cohesion: 0.10
Nodes (12): metadata, RootLayout(), viewport, ThemeProvider(), MockVisualViewport, VisualViewportHeight(), SoftKeyboardChangeDetail, useSoftKeyboardOpen() (+4 more)

### Community 53 - "Evals Service"
Cohesion: 0.13
Nodes (24): asEvalRunResult(), buildEvalCaseResultRows(), clampPassRateBps(), classifyFailureMode(), collectAppModelIds(), computeEvaluatorAverages(), EvalCaseResultInsert, EvalFailureMode (+16 more)

### Community 54 - "Tools"
Cohesion: 0.25
Nodes (11): CanvasServiceResult, ResultComponent, CreateCanvasArtifactInput, CreateCanvasArtifactOutput, inputSchema, outputSchema, toolName, createCanvasArtifactTool() (+3 more)

### Community 55 - "Agents"
Cohesion: 0.10
Nodes (24): canvasArtifactDataSchema, canvasArtifactEventDataSchema, canvasArtifactStatusDataSchema, canvasDiagnosticsDataSchema, chatMessageDataSchemas, chatMessageMetadataSchema, relatedQuestionsDataSchema, validateChatUIMessages() (+16 more)

### Community 56 - "Tool UI"
Cohesion: 0.18
Nodes (21): Citation(), CitationProps, extractDomain(), formatDate(), CitationList(), CitationListProps, OverflowIndicator(), OverflowIndicatorProps (+13 more)

### Community 57 - "Tools"
Cohesion: 0.06
Nodes (42): ActionButtons(), ActionButtonsLayout, actionCategories, ActiveView, promptSamples, EMPTY_MESSAGES, ChatPanel(), FileUploadButton() (+34 more)

### Community 58 - "Tool UI"
Cohesion: 0.08
Nodes (34): chat, research, quality, speed, models, byMode, relatedQuestions, trendingSuggestions (+26 more)

### Community 59 - "Tool UI"
Cohesion: 0.08
Nodes (24): Geo-map react-leaflet adapter contract, collectFitPoints(), dedupeClusterFeatures(), DEFAULT_CENTER, EMPTY_POLYGONS, EMPTY_ROUTES, GeoMapBbox, GeoMapClusterFeature (+16 more)

### Community 60 - "Activity UI"
Cohesion: 0.06
Nodes (34): name, notNull, primaryKey, type, chat_id, current_revision_id, current_runtime_session_id, framework (+26 more)

### Community 61 - "Agents"
Cohesion: 0.14
Nodes (25): ChatSpecialist, chatSpecialistFixtures, competitorResearchSpecialist, competitorResearchSpecialistFixture, defineChatSpecialist(), buildCompetitorResearchOutput(), buildEvidenceStatement(), collectFinalToolResult() (+17 more)

### Community 62 - "lib"
Cohesion: 0.15
Nodes (13): FirecrawlClient, FirecrawlImageResult, FirecrawlImageSearchOptions, FirecrawlImageSearchResponse, FirecrawlImageSearchResponseData, FirecrawlNewsResult, FirecrawlSearchOptions, FirecrawlSearchResponse (+5 more)

### Community 63 - "Types"
Cohesion: 0.17
Nodes (11): DataPart, dataPartSchema, DBMessage, DynamicToolInput, DynamicToolOutput, DynamicToolType, MCPGitHubInput, Metadata (+3 more)

### Community 64 - "Evals UI"
Cohesion: 0.06
Nodes (34): name, notNull, primaryKey, type, chat_id, current_revision_id, current_runtime_session_id, framework (+26 more)

### Community 65 - "Tools"
Cohesion: 0.14
Nodes (18): DisplayGeoMapInput, DisplayGeoMapOutput, GeoMapClusteringSchema, GeoMapMarkerIconSchema, GeoMapMarkerSchema, GeoMapPolygonSchema, GeoMapRoutePointSchema, GeoMapRouteSchema (+10 more)

### Community 66 - "Evals Service"
Cohesion: 0.08
Nodes (23): dependencies, ai, @arizeai/phoenix-client, @arizeai/phoenix-evals, drizzle-orm, @openrouter/ai-sdk-provider, @paralleldrive/cuid2, postgres (+15 more)

### Community 67 - "Project Root"
Cohesion: 0.08
Nodes (25): devDependencies, eslint, eslint-config-next, eslint-plugin-simple-import-sort, husky, lint-staged, @opentelemetry/context-async-hooks, @opentelemetry/sdk-trace-base (+17 more)

### Community 68 - "Tools"
Cohesion: 0.19
Nodes (11): BaseCommunitySource, forbiddenImportSegments, getToolUiCommunitySourceById(), isPublicPackageImport(), ToolUiCommunitySource, ToolUiCommunitySourceId, ToolUiCommunitySourceType, ToolUiLocalCommunitySource (+3 more)

### Community 69 - "UI Primitives"
Cohesion: 0.05
Nodes (55): AdminShellTrigger(), mockUseSidebar, ADMIN_NAV_ITEMS, AdminNavItem, AdminSidebar(), mockUsePathname, AdminLayout(), getCurrentUser() (+47 more)

### Community 70 - "UI Primitives"
Cohesion: 0.06
Nodes (34): name, notNull, primaryKey, type, chat_id, current_revision_id, current_runtime_session_id, framework (+26 more)

### Community 71 - "Tool UI"
Cohesion: 0.14
Nodes (24): ArrayValue(), ArrayValueProps, BadgeValue(), BadgeValueProps, BooleanValue(), BooleanValueProps, CurrencyValue(), CurrencyValueProps (+16 more)

### Community 72 - "Tools"
Cohesion: 0.38
Nodes (8): ResultComponent, tryRenderResult(), DisplayCalloutInput, DisplayCalloutOutput, inputSchema, outputSchema, toolName, serverTool

### Community 73 - "Canvas Engine"
Cohesion: 0.13
Nodes (16): CANVAS_ALLOWED_FILES, CANVAS_DATA_PART_NAMES, CANVAS_REQUIRED_FILES, CANVAS_STATUSES, CANVAS_TOOL_NAMES, CANVAS_VERSION_CREATED_BY, CanvasExternalDependency, classifyImport() (+8 more)

### Community 74 - "Tool UI"
Cohesion: 0.07
Nodes (40): DynamicToolDisplay(), DynamicToolDisplayProps, DynamicToolPart, extractToolUIFromText(), CompetitorResearchResult(), CompetitorResearchResultProps, competitorResearchResultSchema, getMatrixColumns() (+32 more)

### Community 75 - "Tools"
Cohesion: 0.38
Nodes (9): ResultComponent, inputSchema, outputSchema, toolName, UpdateCanvasArtifactInput, UpdateCanvasArtifactOutput, UpdateCanvasArtifactSchema, serverTool (+1 more)

### Community 76 - "UI Primitives"
Cohesion: 0.06
Nodes (34): name, notNull, primaryKey, type, chat_id, current_revision_id, current_runtime_session_id, framework (+26 more)

### Community 77 - "Components"
Cohesion: 0.06
Nodes (34): name, notNull, primaryKey, type, chat_id, current_revision_id, current_runtime_session_id, framework (+26 more)

### Community 78 - "Tools"
Cohesion: 0.15
Nodes (16): SearchProviderType, getSearchSchemaForModel(), inputSchema, outputSchema, PartialInquiry, SearchInput, SearchOutput, searchSchema (+8 more)

### Community 79 - "Scripts"
Cohesion: 0.14
Nodes (18): assertLocalDatabaseUrl(), buildSeedEvalCaseResultRows(), buildSeedEvalSummaryRows(), buildSeedJudgeMetadata(), bunImportMeta, FAILURE_MODE_BY_EVALUATOR, getSeedEvaluatorForFailure(), getSeedResetPattern() (+10 more)

### Community 80 - "Evals Service"
Cohesion: 0.17
Nodes (19): createConfig(), CreateConfigOptions, EvalsConfig, parseRunMode(), required(), requiredEvalRunnerSettings(), requiredSmokeSettings(), validFloat() (+11 more)

### Community 81 - "UI Primitives"
Cohesion: 0.11
Nodes (13): AnimationState, AudioAnalyserOptions, Bar, BarVisualizer, BarVisualizerComponent, BarVisualizerProps, clamp(), generateThinkingSequenceBar() (+5 more)

### Community 82 - "Tool UI"
Cohesion: 0.15
Nodes (20): SerializableCalloutSchema, ChartPropsSchema, SerializableCitationSchema, AlignEnum, CellValueSchema, formatSchema, inferRowIdKey(), isUniqueRowIdCandidate() (+12 more)

### Community 83 - "Tools"
Cohesion: 0.16
Nodes (16): FetchInput, FetchOutput, fetchSchema, inputSchema, outputSchema, PartialInquiry, toolName, extractProviderMessage() (+8 more)

### Community 84 - "Evals Service"
Cohesion: 0.06
Nodes (34): name, notNull, primaryKey, type, chat_id, current_revision_id, current_runtime_session_id, framework (+26 more)

### Community 85 - "Tool UI"
Cohesion: 0.08
Nodes (37): OptionListOptionSchema, OptionListPropsSchema, OptionListSchemaInvariantInput, OptionListSelectionSchema, selectionToIds(), SerializableOptionListSchema, validateOptionListInvariants(), ActionButtons() (+29 more)

### Community 86 - "Project Root"
Cohesion: 0.10
Nodes (20): compilerOptions, allowJs, esModuleInterop, incremental, isolatedModules, jsx, lib, module (+12 more)

### Community 87 - "Agents"
Cohesion: 0.15
Nodes (17): POST(), buildContext(), getTrendingContext(), getTrendingContextFromBrave(), getTrendingContextFromExa(), getTrendingContextFromTavily(), PROVIDERS, sleep() (+9 more)

### Community 88 - "Agents"
Cohesion: 0.25
Nodes (15): One-artifact-per-chat canvas invariant, getCanvasArtifactsPrompt(), getChatModePrompt(), getImageGenerationPrompt(), getResearchModePrompt(), RESEARCH_MODE_PROMPT, REQUIRED_TOOLS, getContentTypesGuidance() (+7 more)

### Community 89 - "Project Root"
Cohesion: 0.10
Nodes (19): aliases, components, hooks, lib, ui, utils, iconLibrary, registries (+11 more)

### Community 90 - "Tool UI"
Cohesion: 0.17
Nodes (18): LinkPreview(), LinkPreviewProps, SerializableLinkPreview, SerializableLinkPreviewSchemaContract, AspectRatio, AspectRatioSchema, getFitClass(), getRatioClass() (+10 more)

### Community 91 - "Tool UI"
Cohesion: 0.16
Nodes (19): ElapsedTimeBadge(), formatElapsedTime(), formatElapsedTimeDateTime(), getReceiptState(), ProgressTrackerBaseProps, ProgressTrackerComponent, ProgressTrackerReceipt(), ProgressTrackerRoot() (+11 more)

### Community 92 - "Tool UI"
Cohesion: 0.11
Nodes (24): Chart, ChartClientProps, ChartDataPoint, ChartProps, ChartSeries, ChartSeriesSchema, SerializableChart, SerializableChartSchemaContract (+16 more)

### Community 93 - "Evals Service"
Cohesion: 0.16
Nodes (12): createToolSelectionExperimentEvaluator(), Label, parseLabel(), baseInput, makeEvaluator(), mockLm(), ToolSelectionInput, ToolSelectionOutput (+4 more)

### Community 94 - ""
Cohesion: 0.18
Nodes (10): Admin Access, Admin surface, ADMIN_USER_ID, Environment Operations Variables, Evals cron (Railway `polymorph-evals`), Production HTTPS Trace Enforcement, Phoenix Tracing Configuration, Tracing (Arize Phoenix) (+2 more)

### Community 95 - "Project Root"
Cohesion: 0.11
Nodes (18): scripts, build, build:canvas-vendor, chat, dev, format, format:check, lint (+10 more)

### Community 96 - "Configuration"
Cohesion: 0.33
Nodes (8): baseEnvSchema, Env, envSchema, featureWarnings, getEnv(), isProductionTarget(), validateEnv(), register()

### Community 97 - "Tools"
Cohesion: 0.38
Nodes (8): ResultComponent, citationSchema, DisplayCitationsInput, DisplayCitationsOutput, inputSchema, outputSchema, toolName, serverTool

### Community 98 - "lib"
Cohesion: 0.10
Nodes (29): GenerateImage(), ResultComponent, tryRenderResult(), GenerateImageError, generateImageErrorSchema, GenerateImageInput, GenerateImageOutput, generateImageOutputSchema (+21 more)

### Community 99 - "lib"
Cohesion: 0.13
Nodes (18): CAPABILITY_CASES, getAllCases(), getCasesForEvaluation(), getCasesForSuite(), getSmoketestCases(), REGRESSION_CASES, SMOKE_CASES, authenticateSmokeUser() (+10 more)

### Community 100 - "Canvas Engine"
Cohesion: 0.06
Nodes (34): name, notNull, primaryKey, type, chat_id, current_revision_id, current_runtime_session_id, framework (+26 more)

### Community 101 - "Tool UI"
Cohesion: 0.11
Nodes (25): Callout, variantConfig, CalloutProps, CalloutVariant, CalloutVariantSchema, SerializableCallout, SerializableCalloutSchemaContract, StaggerList() (+17 more)

### Community 102 - "Tools"
Cohesion: 0.14
Nodes (17): DataPart, groupConsecutiveParts(), isDataPart(), isReasoningPart(), isTextPart(), isToolPart(), MessagePart, Props (+9 more)

### Community 103 - "Tool UI"
Cohesion: 0.15
Nodes (13): CaseRunResults, EvalRunnerClientConfig, EvalRunnerHttpError, EvalRunnerRequest, runEvalCase(), baseCaseSpec, mockFetch, successBody (+5 more)

### Community 104 - ""
Cohesion: 0.29
Nodes (8): BraveSearchProvider (default), createSearchProvider Factory, ExaSearchProvider (fallback), FirecrawlSearchProvider (opt-in), SearchProvider Interface / BaseSearchProvider, TavilySearchProvider (fallback), Provider Selection Logic (optimized/general), SearchResults Interface

### Community 105 - "Evals Service"
Cohesion: 0.12
Nodes (15): mockBuildDatasetExamples, mockBuildEvalSummaryMetadata, mockBuildExperimentEvaluators, mockBuildExperimentTask, mockBuildSuiteRunResult, mockBuildTimestampedDatasetName, mockCheckExperimentThresholds, mockConfig (+7 more)

### Community 106 - "Tools"
Cohesion: 0.11
Nodes (19): execute(), importFresh(), mockFetch, originalEnv, buildCoordsPath(), DirectionsInput, fetchRoute(), formatDistanceLabel() (+11 more)

### Community 107 - "lib"
Cohesion: 0.06
Nodes (34): name, notNull, primaryKey, type, chat_id, current_revision_id, current_runtime_session_id, framework (+26 more)

### Community 108 - "Tool UI"
Cohesion: 0.06
Nodes (34): name, notNull, primaryKey, type, chat_id, current_revision_id, current_runtime_session_id, framework (+26 more)

### Community 109 - ""
Cohesion: 0.09
Nodes (22): Event Format, How the Client Consumes the Stream, Key Files, Portable Tool Boundary, Response Headers, SSE Protocol, Streaming SSE Protocol, Authenticated Streams (`create-chat-stream-response.ts`) (+14 more)

### Community 110 - "Tools"
Cohesion: 0.06
Nodes (33): appendKey(), buildMapTilerUrl(), buildPublicMapTilerUrl(), fetchMapTilerJson(), getPublicApiKey(), getServerOrPublicApiKey(), MapTilerApiError, MapTilerConfigError (+25 more)

### Community 111 - "Evals UI"
Cohesion: 0.11
Nodes (17): ActivityList(), buildRows(), Row, DASHBOARD_SUITE_TO_PERSISTED, DashboardSuiteId, getSuiteAction(), getSuiteDefinition(), getSuiteDisplay() (+9 more)

### Community 112 - "UI Primitives"
Cohesion: 0.06
Nodes (34): name, notNull, primaryKey, type, chat_id, current_revision_id, current_runtime_session_id, framework (+26 more)

### Community 113 - ""
Cohesion: 0.12
Nodes (14): Cloud Mode (`POLYMORPH_CLOUD_DEPLOYMENT`), Custom Supabase Port Range (4432x), Environment Local Setup, Guest Chat (`ENABLE_GUEST_CHAT`), Implementation Details, Local setup workflow, Local Supabase Stack, Authentication posture in Docker (+6 more)

### Community 114 - ""
Cohesion: 0.13
Nodes (15): Key Files, Research Agent Key Files, End-to-End Pipeline, Overview, Pipeline Steps, Research Agent Pipeline, Display Tools (manifest-managed), Tool UI Manifest Runtime (+7 more)

### Community 115 - "Auth"
Cohesion: 0.06
Nodes (34): name, notNull, primaryKey, type, chat_id, current_revision_id, current_runtime_session_id, framework (+26 more)

### Community 116 - "App Routes"
Cohesion: 0.13
Nodes (16): generateTrendingSuggestions(), mockGenerateTrendingSuggestions, assertPrivilegedRole(), getPrivilegedDb(), trendingSuggestionsCache, GET(), Privileged RLS-bypass DB write pattern, evalRequestSchema (+8 more)

### Community 117 - "UI Primitives"
Cohesion: 0.06
Nodes (34): name, notNull, primaryKey, type, chat_id, current_revision_id, current_runtime_session_id, framework (+26 more)

### Community 118 - "Tool UI"
Cohesion: 0.06
Nodes (34): name, notNull, primaryKey, type, chat_id, current_revision_id, current_runtime_session_id, framework (+26 more)

### Community 119 - "Tools"
Cohesion: 0.38
Nodes (8): ResultComponent, DisplayPlanInput, DisplayPlanOutput, inputSchema, outputSchema, PlanTodoSchema, toolName, serverTool

### Community 120 - "Tools"
Cohesion: 0.33
Nodes (9): ResultComponent, ColumnSchema, DisplayTableInput, DisplayTableOutput, FormatSchema, inputSchema, outputSchema, toolName (+1 more)

### Community 121 - "Tools"
Cohesion: 0.38
Nodes (8): ResultComponent, DisplayTimelineInput, DisplayTimelineOutput, inputSchema, outputSchema, TimelineEventSchema, toolName, serverTool

### Community 122 - "tests"
Cohesion: 0.23
Nodes (10): eslintConfig, escapeRegExp(), eslintConfigSource, FlatConfigBlock, getBareDisableDirectiveOffenders(), getGlobalRuleDowngradeOffenders(), getRuleLevel(), hasConfigDowngradeJustification() (+2 more)

### Community 123 - "lib"
Cohesion: 0.21
Nodes (9): PillPresence(), cardEntrance, identityCard, identityPill, identityStaggerParent, pillPresence, ResolvedVariants, staggerParent (+1 more)

### Community 124 - "Scripts"
Cohesion: 0.24
Nodes (6): ChatApiConfig, ChatApiTester, ChatPayload, main(), parseArgs(), UIMessage

### Community 125 - "Tools"
Cohesion: 0.15
Nodes (12): execute(), importFresh(), mockFetch, originalEnv, GetIsochroneInputSchema, GetIsochroneResult, getIsochroneTool, LatitudeSchema (+4 more)

### Community 126 - "Tools"
Cohesion: 0.38
Nodes (8): ResultComponent, ChartSeriesSchema, DisplayChartInput, DisplayChartOutput, inputSchema, outputSchema, toolName, serverTool

### Community 127 - "Tools"
Cohesion: 0.44
Nodes (7): ResultComponent, DisplayLinkPreviewInput, DisplayLinkPreviewOutput, inputSchema, outputSchema, toolName, serverTool

### Community 128 - "Evals Service"
Cohesion: 0.17
Nodes (11): compilerOptions, esModuleInterop, module, moduleResolution, outDir, rootDir, skipLibCheck, strict (+3 more)

### Community 129 - "Utilities"
Cohesion: 0.06
Nodes (34): name, notNull, primaryKey, type, chat_id, current_revision_id, current_runtime_session_id, framework (+26 more)

### Community 130 - ""
Cohesion: 0.14
Nodes (13): Architecture Tool System, Tool Availability by Agent, Tool System, Architecture Navigation Hub, Display Tools, MapTiler, OpenRouteService, Spatial Helper Tools (+5 more)

### Community 131 - "Project Root"
Cohesion: 0.17
Nodes (11): engines, bun, license, lint-staged, *.{js,mjs,cjs,json,md,css,yaml,yml}, *.{ts,tsx}, name, overrides (+3 more)

### Community 132 - "Tools"
Cohesion: 0.14
Nodes (21): CanvasToolContext, CreateCanvasArtifactSchema, inputSchema, outputSchema, ReadCanvasArtifactInput, ReadCanvasArtifactOutput, ReadCanvasArtifactSchema, toolName (+13 more)

### Community 133 - "Evals Service"
Cohesion: 0.17
Nodes (11): mockConfig, mockCreateClient, mockCreateDataset, mockCreateOpenRouter, mockCreateOrGetDataset, mockGetCasesForEvaluation, mockGetDatasetExamples, mockPersistEvalSummary (+3 more)

### Community 134 - ""
Cohesion: 0.07
Nodes (28): 10. Tool Loop Execution, 11. Related Questions Generation, 12. Stream Finalization (`onFinish`), 13. Response Return, 7. Message Preparation, 8. Title Generation (Parallel, New Chats Only), 9. Chat Agent Streaming, Streaming Agent Finalization (+20 more)

### Community 135 - "Streaming"
Cohesion: 0.06
Nodes (34): name, notNull, primaryKey, type, chat_id, current_revision_id, current_runtime_session_id, framework (+26 more)

### Community 136 - "Types"
Cohesion: 0.06
Nodes (33): file_fields_required, reasoning_text_required, text_text_required, tool_fields_required, tool_state_valid, name, value, parts_message_id_messages_id_fk (+25 more)

### Community 137 - "Utilities"
Cohesion: 0.19
Nodes (10): DataTable, DataTableProvider(), FormatConfig, Column, createDataTableRowKeys(), getDataTableMobileDescriptionId(), getRowIdentifier(), parseNumericLike() (+2 more)

### Community 138 - "Auth"
Cohesion: 0.12
Nodes (19): updateMessageFeedback(), getSafeRedirectPath(), RLS-scoped message metadata read/write pattern, GET(), POST(), GET(), annotatePhoenixUserFeedback(), getPhoenixBaseUrl() (+11 more)

### Community 139 - ""
Cohesion: 0.09
Nodes (22): 1. Clone and Install, 2. Start Local Supabase, 3. Configure Environment, 4. Run Migrations, 5. Start the Dev Server, 6. Your First Search, Common Setup Issues, Next Steps (+14 more)

### Community 140 - "lib"
Cohesion: 0.06
Nodes (33): expires_at, last_heartbeat_at, preview_url, provider, sandbox_id, started_at, name, notNull (+25 more)

### Community 141 - "lib"
Cohesion: 0.06
Nodes (33): expires_at, last_heartbeat_at, preview_url, provider, sandbox_id, started_at, name, notNull (+25 more)

### Community 142 - "Tools"
Cohesion: 0.06
Nodes (33): expires_at, last_heartbeat_at, preview_url, provider, sandbox_id, started_at, name, notNull (+25 more)

### Community 143 - "Tool UI"
Cohesion: 0.06
Nodes (33): expires_at, last_heartbeat_at, preview_url, provider, sandbox_id, started_at, name, notNull (+25 more)

### Community 144 - "Components"
Cohesion: 0.06
Nodes (33): expires_at, last_heartbeat_at, preview_url, provider, sandbox_id, started_at, name, notNull (+25 more)

### Community 145 - "Tools"
Cohesion: 0.06
Nodes (33): expires_at, last_heartbeat_at, preview_url, provider, sandbox_id, started_at, name, notNull (+25 more)

### Community 146 - ""
Cohesion: 0.15
Nodes (13): Architecture Tech Stack, Tech Stack, Cloud Deployment Behavior, Default Model Assignments, Model Selection, Provider Registry, Drizzle ORM, Next.js 16 App Router (+5 more)

### Community 147 - "Tool UI"
Cohesion: 0.06
Nodes (33): expires_at, last_heartbeat_at, preview_url, provider, sandbox_id, started_at, name, notNull (+25 more)

### Community 148 - ""
Cohesion: 0.18
Nodes (10): 1. `correct_tool` (positive class, score 1.0), 2. `wrong_tool` (negative class, score 0.0), 3. `missing_tool` (negative class, score 0.0), 4. `not_required` (null, skipped), Chat agent tool roster (as of 2026-05-20), Failure taxonomy, Judging rubric (for the LLM judge prompt), Method (+2 more)

### Community 149 - "lib"
Cohesion: 0.06
Nodes (33): expires_at, last_heartbeat_at, preview_url, provider, sandbox_id, started_at, name, notNull (+25 more)

### Community 150 - "Tools"
Cohesion: 0.06
Nodes (33): expires_at, last_heartbeat_at, preview_url, provider, sandbox_id, started_at, name, notNull (+25 more)

### Community 151 - "App Routes"
Cohesion: 0.06
Nodes (33): expires_at, last_heartbeat_at, preview_url, provider, sandbox_id, started_at, name, notNull (+25 more)

### Community 152 - ""
Cohesion: 0.25
Nodes (9): Core Tools (search/fetch/todoWrite), Community Portability Proof (Workstream 5), competitorResearch Specialist, fetch Tool (content extraction), search Tool (multi-provider), SearchProviderError / Retry Semantics, search-config Utility, Search Provider Delegation (+1 more)

### Community 153 - ""
Cohesion: 0.08
Nodes (23): Analytics Tracking, Client Disconnection, Client-Side Error Handling, Database Persistence, Database Persistence Errors, Effect on Perceived Latency, Error Handling, Parallel Operations (+15 more)

### Community 154 - ""
Cohesion: 0.06
Nodes (33): expires_at, last_heartbeat_at, preview_url, provider, sandbox_id, started_at, name, notNull (+25 more)

### Community 155 - ""
Cohesion: 0.25
Nodes (8): Canvas Artifact Lifecycle Endpoints, Canvas CSP Sandbox Isolation, GET /api/canvas-artifacts/[artifactId]/export, GET /api/canvas-artifacts/[artifactId], POST /api/canvas-artifacts/[artifactId]/restore, POST /api/canvas-artifacts/[artifactId]/versions, POST /api/canvas-artifacts/[artifactId]/runtime-diagnostics, GET /api/canvas-artifacts/[artifactId]/view

### Community 156 - ""
Cohesion: 0.18
Nodes (11): API Reference Navigation Hub, HMAC-Signed Guest Canvas Token, PATCH /api/canvas-artifacts/[artifactId]/draft, POST /api/chat (SSE streaming chat), Chat Request Flow (middleware to handler), POST /api/evals/run (eval replay), Traffic-Monitor Sampler (services/evals/src/sampler.ts), Evals Cron Failure Modes (PHOENIX UNAVAILABLE / DB WRITE FAILED) (+3 more)

### Community 157 - ""
Cohesion: 0.12
Nodes (17): GET /api/health, Incident Triage Flow, Key Rotation Checklist, Phoenix API Key Rotation, HTTPS Enforcement in Production Tracing, instrumentation.ts (OTLP tracing exporter), OpenInference Input/Output Masking, Phoenix Persistence Verification (+9 more)

### Community 158 - "Project Root"
Cohesion: 0.27
Nodes (9): PHOENIX_HOST, npx, firecrawl, next-devtools, phoenix, railway, @arizeai/phoenix-mcp, firecrawl-mcp (+1 more)

### Community 159 - ""
Cohesion: 0.32
Nodes (8): Community-Portability Evidence, competitorResearch Tool, Build Agent (Build Mode), Chat Agent Factory, Chat Agent Registry / resolveChatAgentId, Research Agent (Research Mode), Search Agent (Chat Mode), Shared Chat Agent Toolset

### Community 160 - ""
Cohesion: 0.32
Nodes (8): Compose-First Geo Design, Geo & Spatial Toolchain, geocodeAddress Tool, getDirections Tool, getIsochrone Tool, getStaticMapImage Tool, MapTiler Provider, OpenRouteService Provider

### Community 161 - ""
Cohesion: 0.15
Nodes (13): Chat Mode, Mode Comparison, Research Agent Modes, Research Mode, Search Modes, General, Optimized (Default), Search Types: Optimized vs General (+5 more)

### Community 162 - ""
Cohesion: 0.13
Nodes (15): Abort Handling, Auxiliary Agents, Message Metadata, Related Questions Generator, Research Agent Streaming and Auxiliary Agents, Stream Creation, Streaming Integration, Title Generator (+7 more)

### Community 163 - "App Routes"
Cohesion: 0.06
Nodes (33): expires_at, last_heartbeat_at, preview_url, provider, sandbox_id, started_at, name, notNull (+25 more)

### Community 164 - "UI Primitives"
Cohesion: 0.06
Nodes (33): expires_at, last_heartbeat_at, preview_url, provider, sandbox_id, started_at, name, notNull (+25 more)

### Community 165 - "App Routes"
Cohesion: 0.06
Nodes (33): expires_at, last_heartbeat_at, preview_url, provider, sandbox_id, started_at, name, notNull (+25 more)

### Community 166 - "Components"
Cohesion: 0.06
Nodes (33): expires_at, last_heartbeat_at, preview_url, provider, sandbox_id, started_at, name, notNull (+25 more)

### Community 167 - ""
Cohesion: 0.29
Nodes (7): displayOptionList Tool, displayQuestionWizard Tool, submitInteractiveToolOutput Callback, addToolOutput (AI SDK), OptionList Client-Resolved Flow, ActionSchema, Action System

### Community 168 - ""
Cohesion: 0.15
Nodes (12): POST /api/advanced-search (SearXNG), GET /api/canvas-assets/image-proxy, Core API Endpoints Hub, GET /api/chats (chat listing), Database Issues, Migration Failures, No Search Results, Search Issues (+4 more)

### Community 169 - ""
Cohesion: 0.20
Nodes (10): Supabase Auth Modes (authenticated/guest/disabled), Supabase Middleware (lib/supabase/middleware.ts), Build Issues, ESLint Import Order Errors, Prettier Formatting Failures, TypeScript Strict Mode Errors, 401 Unauthorized on /api/chat, getUser Timeout (middleware 5s Promise.race) (+2 more)

### Community 170 - "Tool UI"
Cohesion: 0.06
Nodes (33): expires_at, last_heartbeat_at, preview_url, provider, sandbox_id, started_at, name, notNull (+25 more)

### Community 171 - ""
Cohesion: 0.25
Nodes (7): tests/eslint-config.test.ts Guard, eslint.config.mjs, Inline ESLint Disable Reason Trailer Rule, ESLint Rule Policy, build-canvas-vendor.ts, build-demo.sh, Scripts

### Community 172 - ""
Cohesion: 0.14
Nodes (13): Anonymous Mode (Local Development Only), Authentication Modes, Supabase Authentication (Recommended), Authentication (Supabase), POLYMORPH_CLOUD_DEPLOYMENT, Cloud deployment controls, Core behavior controls, DATABASE_RESTRICTED_URL (+5 more)

### Community 173 - ""
Cohesion: 0.33
Nodes (7): Admin Surface (/admin/evals), Chat API Endpoint (app/api/chat/route.ts), Admin Detection (lib/auth/is-admin.ts), App Router Route Groups, Polymorph System Overview, User-Mode Vocabulary (search/research/build), selectModel()

### Community 174 - ""
Cohesion: 0.25
Nodes (8): Conditional Tools, Dynamic Tool Parts, Message Persistence Contract, Research Agent Conditional Tools, Canvas Artifact Tools, generateImage Tool, Message Persistence Contract (ui_message), Interactive Tool UI Continuation Bridge

### Community 175 - "Evals Service"
Cohesion: 0.33
Nodes (5): mockConfig, mockRunCapabilitySuite, mockRunRegressionSuite, mockRunSmokeSuite, mockRunTrafficMonitorSuite

### Community 176 - "App Routes"
Cohesion: 0.40
Nodes (4): mockGetCurrentUser, mockIsAdminUserId, mockNotFound, mockRedirect

### Community 177 - ""
Cohesion: 0.17
Nodes (12): /admin/evals Dashboard, ADMIN_USER_ID Admin Access Gate, seed:evals (synthetic eval rows), Browser QA for Authenticated Admin Surfaces, Evidence To Report, Ground Rules, Local Admin QA, Preview QA (+4 more)

### Community 178 - "Canvas UI"
Cohesion: 0.06
Nodes (32): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, columns (+24 more)

### Community 179 - "Tool UI"
Cohesion: 0.40
Nodes (4): SerializableGenerateImageSchema, SerializableGeoMapSchema, validGeoMapPayload, Tool UI serializable schema contract (defineToolUiContract)

### Community 180 - "Evals Service"
Cohesion: 0.40
Nodes (4): mockCapability, mockRegression, mockSmoke, mockTrafficMonitor

### Community 181 - "Tools"
Cohesion: 0.06
Nodes (32): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, columns (+24 more)

### Community 182 - "Tools"
Cohesion: 0.06
Nodes (32): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, columns (+24 more)

### Community 183 - "Streaming"
Cohesion: 0.06
Nodes (32): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, columns (+24 more)

### Community 184 - "Tools"
Cohesion: 0.06
Nodes (32): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, columns (+24 more)

### Community 188 - ""
Cohesion: 0.67
Nodes (3): GET /api/suggestions, GET /api/suggestions/refresh (Vercel cron), trending_suggestions_cache Singleton

### Community 189 - ""
Cohesion: 0.67
Nodes (3): API Error Response Conventions, Rate Limiting (Upstash Redis, cloud-only), Rate Limit Exceeded (runtime)

### Community 191 - ""
Cohesion: 0.67
Nodes (3): onFinish callback, persistStreamResults, retryDatabaseOperation

### Community 196 - ""
Cohesion: 0.15
Nodes (12): Auto-collapse behavior, Behavior, Dynamic Tool Display, Generative UI Interactive and Dynamic Tools, Grouping logic, Inspector Panel, Interactive Tool: OptionList, Part rendering (+4 more)

### Community 222 - ""
Cohesion: 0.06
Nodes (32): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, columns (+24 more)

### Community 226 - ""
Cohesion: 0.06
Nodes (32): current_version_id, draft_compiled_html, draft_diagnostics, draft_revision, draft_source, last_compiled_at, name, notNull (+24 more)

### Community 238 - ""
Cohesion: 0.06
Nodes (32): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, columns (+24 more)

### Community 243 - "Community 243"
Cohesion: 0.06
Nodes (32): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, columns (+24 more)

### Community 244 - "Community 244"
Cohesion: 0.06
Nodes (32): current_version_id, draft_compiled_html, draft_diagnostics, draft_revision, draft_source, last_compiled_at, name, notNull (+24 more)

### Community 245 - "Community 245"
Cohesion: 0.06
Nodes (32): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, columns (+24 more)

### Community 246 - "Community 246"
Cohesion: 0.06
Nodes (32): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, columns (+24 more)

### Community 247 - "Community 247"
Cohesion: 0.06
Nodes (32): current_version_id, draft_compiled_html, draft_diagnostics, draft_revision, draft_source, last_compiled_at, name, notNull (+24 more)

### Community 248 - "Community 248"
Cohesion: 0.06
Nodes (32): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, columns (+24 more)

### Community 249 - "Community 249"
Cohesion: 0.06
Nodes (32): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, columns (+24 more)

### Community 250 - "Community 250"
Cohesion: 0.06
Nodes (32): current_version_id, draft_compiled_html, draft_diagnostics, draft_revision, draft_source, last_compiled_at, name, notNull (+24 more)

### Community 251 - "Community 251"
Cohesion: 0.06
Nodes (32): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, columns (+24 more)

### Community 252 - "Community 252"
Cohesion: 0.06
Nodes (32): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, columns (+24 more)

### Community 253 - "Community 253"
Cohesion: 0.06
Nodes (32): current_version_id, draft_compiled_html, draft_diagnostics, draft_revision, draft_source, last_compiled_at, name, notNull (+24 more)

### Community 254 - "Community 254"
Cohesion: 0.06
Nodes (32): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, columns (+24 more)

### Community 255 - "Community 255"
Cohesion: 0.06
Nodes (32): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, columns (+24 more)

### Community 256 - "Community 256"
Cohesion: 0.06
Nodes (32): current_version_id, draft_compiled_html, draft_diagnostics, draft_revision, draft_source, last_compiled_at, name, notNull (+24 more)

### Community 257 - "Community 257"
Cohesion: 0.06
Nodes (32): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, columns (+24 more)

### Community 258 - "Community 258"
Cohesion: 0.06
Nodes (32): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, columns (+24 more)

### Community 259 - "Community 259"
Cohesion: 0.06
Nodes (32): current_version_id, draft_compiled_html, draft_diagnostics, draft_revision, draft_source, last_compiled_at, name, notNull (+24 more)

### Community 260 - "Community 260"
Cohesion: 0.06
Nodes (32): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, columns (+24 more)

### Community 261 - "Community 261"
Cohesion: 0.06
Nodes (32): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, columns (+24 more)

### Community 262 - "Community 262"
Cohesion: 0.06
Nodes (32): current_version_id, draft_compiled_html, draft_diagnostics, draft_revision, draft_source, last_compiled_at, name, notNull (+24 more)

### Community 263 - "Community 263"
Cohesion: 0.06
Nodes (32): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, columns (+24 more)

### Community 264 - "Community 264"
Cohesion: 0.06
Nodes (32): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, columns (+24 more)

### Community 265 - "Community 265"
Cohesion: 0.06
Nodes (32): current_version_id, draft_compiled_html, draft_diagnostics, draft_revision, draft_source, last_compiled_at, name, notNull (+24 more)

### Community 266 - "Community 266"
Cohesion: 0.06
Nodes (32): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, columns (+24 more)

### Community 267 - "Community 267"
Cohesion: 0.06
Nodes (32): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, columns (+24 more)

### Community 268 - "Community 268"
Cohesion: 0.06
Nodes (32): current_version_id, draft_compiled_html, draft_diagnostics, draft_revision, draft_source, last_compiled_at, name, notNull (+24 more)

### Community 269 - "Community 269"
Cohesion: 0.06
Nodes (32): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, columns (+24 more)

### Community 270 - "Community 270"
Cohesion: 0.06
Nodes (32): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, columns (+24 more)

### Community 271 - "Community 271"
Cohesion: 0.06
Nodes (32): current_version_id, draft_compiled_html, draft_diagnostics, draft_revision, draft_source, last_compiled_at, name, notNull (+24 more)

### Community 272 - "Community 272"
Cohesion: 0.06
Nodes (32): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, columns (+24 more)

### Community 273 - "Community 273"
Cohesion: 0.06
Nodes (32): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, columns (+24 more)

### Community 274 - "Community 274"
Cohesion: 0.06
Nodes (32): current_version_id, draft_compiled_html, draft_diagnostics, draft_revision, draft_source, last_compiled_at, name, notNull (+24 more)

### Community 275 - "Community 275"
Cohesion: 0.06
Nodes (32): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, columns (+24 more)

### Community 276 - "Community 276"
Cohesion: 0.06
Nodes (32): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, columns (+24 more)

### Community 277 - "Community 277"
Cohesion: 0.06
Nodes (32): current_version_id, draft_compiled_html, draft_diagnostics, draft_revision, draft_source, last_compiled_at, name, notNull (+24 more)

### Community 278 - "Community 278"
Cohesion: 0.06
Nodes (32): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, columns (+24 more)

### Community 279 - "Community 279"
Cohesion: 0.06
Nodes (32): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, columns (+24 more)

### Community 280 - "Community 280"
Cohesion: 0.06
Nodes (32): current_version_id, draft_compiled_html, draft_diagnostics, draft_revision, draft_source, last_compiled_at, name, notNull (+24 more)

### Community 281 - "Community 281"
Cohesion: 0.06
Nodes (32): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, columns (+24 more)

### Community 282 - "Community 282"
Cohesion: 0.06
Nodes (32): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, columns (+24 more)

### Community 283 - "Community 283"
Cohesion: 0.06
Nodes (32): current_version_id, draft_compiled_html, draft_diagnostics, draft_revision, draft_source, last_compiled_at, name, notNull (+24 more)

### Community 284 - "Community 284"
Cohesion: 0.06
Nodes (31): message_id, provider_metadata, reasoning_text, source_url_source_id, tool_dynamic_input, tool_todoRead_output, name, notNull (+23 more)

### Community 285 - "Community 285"
Cohesion: 0.06
Nodes (31): data_content, source_document_media_type, source_url_title, source_url_url, tool_dynamic_output, tool_state, name, notNull (+23 more)

### Community 286 - "Community 286"
Cohesion: 0.06
Nodes (31): data_content, order, source_document_filename, source_url_source_id, tool_question_input, tool_search_output, name, notNull (+23 more)

### Community 287 - "Community 287"
Cohesion: 0.06
Nodes (31): messages_chat_id_chats_id_fk, messages_chat_id_created_at_idx, messages_chat_id_idx, columnsFrom, columnsTo, name, onDelete, onUpdate (+23 more)

### Community 288 - "Community 288"
Cohesion: 0.07
Nodes (29): columns, concurrently, isUnique, method, name, with, columns, concurrently (+21 more)

### Community 289 - "Community 289"
Cohesion: 0.07
Nodes (29): columns, concurrently, isUnique, method, name, with, columns, concurrently (+21 more)

### Community 290 - "Community 290"
Cohesion: 0.07
Nodes (29): columns, concurrently, isUnique, method, name, with, columns, concurrently (+21 more)

### Community 291 - "Community 291"
Cohesion: 0.07
Nodes (29): columns, concurrently, isUnique, method, name, with, columns, concurrently (+21 more)

### Community 292 - "Community 292"
Cohesion: 0.07
Nodes (29): columns, concurrently, isUnique, method, name, with, columns, concurrently (+21 more)

### Community 293 - "Community 293"
Cohesion: 0.07
Nodes (29): columns, concurrently, isUnique, method, name, with, columns, concurrently (+21 more)

### Community 294 - "Community 294"
Cohesion: 0.07
Nodes (29): columns, concurrently, isUnique, method, name, with, columns, concurrently (+21 more)

### Community 295 - "Community 295"
Cohesion: 0.07
Nodes (29): columns, concurrently, isUnique, method, name, with, columns, concurrently (+21 more)

### Community 296 - "Community 296"
Cohesion: 0.07
Nodes (29): columns, concurrently, isUnique, method, name, with, columns, concurrently (+21 more)

### Community 297 - "Community 297"
Cohesion: 0.07
Nodes (29): columns, concurrently, isUnique, method, name, with, columns, concurrently (+21 more)

### Community 298 - "Community 298"
Cohesion: 0.07
Nodes (29): columns, concurrently, isUnique, method, name, with, columns, concurrently (+21 more)

### Community 299 - "Community 299"
Cohesion: 0.07
Nodes (29): columns, concurrently, isUnique, method, name, with, columns, concurrently (+21 more)

### Community 300 - "Community 300"
Cohesion: 0.07
Nodes (29): columns, concurrently, isUnique, method, name, with, columns, concurrently (+21 more)

### Community 301 - "Community 301"
Cohesion: 0.07
Nodes (29): columns, concurrently, isUnique, method, name, with, columns, concurrently (+21 more)

### Community 302 - "Community 302"
Cohesion: 0.07
Nodes (29): columns, concurrently, isUnique, method, name, with, columns, concurrently (+21 more)

### Community 303 - "Community 303"
Cohesion: 0.07
Nodes (29): columns, concurrently, isUnique, method, name, with, columns, concurrently (+21 more)

### Community 304 - "Community 304"
Cohesion: 0.07
Nodes (29): columns, concurrently, isUnique, method, name, with, columns, concurrently (+21 more)

### Community 305 - "Community 305"
Cohesion: 0.07
Nodes (27): Canvas API Endpoints, Canvas Artifact Endpoints, Error Responses, Error Responses, Error Responses, Error Responses, Error Responses, Error Responses (+19 more)

### Community 306 - "Community 306"
Cohesion: 0.07
Nodes (27): title, user_id, visibility, checkConstraints, columns, compositePrimaryKeys, foreignKeys, indexes (+19 more)

### Community 307 - "Community 307"
Cohesion: 0.07
Nodes (27): name, notNull, primaryKey, type, chat_id, created_at, metadata, role (+19 more)

### Community 308 - "Community 308"
Cohesion: 0.07
Nodes (27): name, notNull, primaryKey, type, chat_id, created_at, metadata, role (+19 more)

### Community 309 - "Community 309"
Cohesion: 0.07
Nodes (27): name, notNull, primaryKey, type, chat_id, created_at, metadata, role (+19 more)

### Community 310 - "Community 310"
Cohesion: 0.07
Nodes (27): name, notNull, primaryKey, type, chat_id, created_at, metadata, role (+19 more)

### Community 311 - "Community 311"
Cohesion: 0.07
Nodes (27): created_at, prompt_summary, sandbox_snapshot_ref, source_files, triggering_message_id, default, name, notNull (+19 more)

### Community 312 - "Community 312"
Cohesion: 0.07
Nodes (27): created_at, prompt_summary, sandbox_snapshot_ref, source_files, triggering_message_id, default, name, notNull (+19 more)

### Community 313 - "Community 313"
Cohesion: 0.07
Nodes (27): created_at, prompt_summary, sandbox_snapshot_ref, source_files, triggering_message_id, default, name, notNull (+19 more)

### Community 314 - "Community 314"
Cohesion: 0.07
Nodes (27): created_at, prompt_summary, sandbox_snapshot_ref, source_files, triggering_message_id, default, name, notNull (+19 more)

### Community 315 - "Community 315"
Cohesion: 0.07
Nodes (27): created_at, prompt_summary, sandbox_snapshot_ref, source_files, triggering_message_id, default, name, notNull (+19 more)

### Community 316 - "Community 316"
Cohesion: 0.08
Nodes (26): name, notNull, primaryKey, type, artifact_id, prompt_summary, sandbox_snapshot_ref, source_files (+18 more)

### Community 317 - "Community 317"
Cohesion: 0.07
Nodes (27): created_at, prompt_summary, sandbox_snapshot_ref, source_files, triggering_message_id, default, name, notNull (+19 more)

### Community 318 - "Community 318"
Cohesion: 0.07
Nodes (27): created_at, prompt_summary, sandbox_snapshot_ref, source_files, triggering_message_id, default, name, notNull (+19 more)

### Community 319 - "Community 319"
Cohesion: 0.07
Nodes (27): created_at, prompt_summary, sandbox_snapshot_ref, source_files, triggering_message_id, default, name, notNull (+19 more)

### Community 320 - "Community 320"
Cohesion: 0.07
Nodes (27): created_at, prompt_summary, sandbox_snapshot_ref, source_files, triggering_message_id, default, name, notNull (+19 more)

### Community 321 - "Community 321"
Cohesion: 0.07
Nodes (27): created_at, prompt_summary, sandbox_snapshot_ref, source_files, triggering_message_id, default, name, notNull (+19 more)

### Community 322 - "Community 322"
Cohesion: 0.08
Nodes (26): name, notNull, primaryKey, type, artifact_id, prompt_summary, sandbox_snapshot_ref, source_files (+18 more)

### Community 323 - "Community 323"
Cohesion: 0.08
Nodes (26): data_content, data_id, reasoning_text, tool_dynamic_input, tool_dynamic_name, name, notNull, primaryKey (+18 more)

### Community 324 - "Community 324"
Cohesion: 0.08
Nodes (26): data_content, data_prefix, order, tool_dynamic_name, tool_dynamic_type, name, notNull, primaryKey (+18 more)

### Community 325 - "Community 325"
Cohesion: 0.08
Nodes (26): name, notNull, primaryKey, type, chat_id, id, metadata, role (+18 more)

### Community 326 - "Community 326"
Cohesion: 0.08
Nodes (26): title, user_id, visibility, checkConstraints, columns, compositePrimaryKeys, foreignKeys, isRLSEnabled (+18 more)

### Community 327 - "Community 327"
Cohesion: 0.08
Nodes (26): id, message, page_url, sentiment, user_agent, name, notNull, primaryKey (+18 more)

### Community 328 - "Community 328"
Cohesion: 0.08
Nodes (26): id, message, page_url, sentiment, user_agent, name, notNull, primaryKey (+18 more)

### Community 329 - "Community 329"
Cohesion: 0.08
Nodes (26): title, user_id, visibility, checkConstraints, columns, compositePrimaryKeys, foreignKeys, isRLSEnabled (+18 more)

### Community 330 - "Community 330"
Cohesion: 0.10
Nodes (21): name, notNull, primaryKey, type, chat_id, metadata, role, updated_at (+13 more)

### Community 331 - "Community 331"
Cohesion: 0.10
Nodes (21): message, page_url, sentiment, user_agent, name, notNull, primaryKey, type (+13 more)

### Community 332 - "Community 332"
Cohesion: 0.10
Nodes (21): message, page_url, sentiment, user_agent, name, notNull, primaryKey, type (+13 more)

### Community 333 - "Community 333"
Cohesion: 0.07
Nodes (27): name, notNull, primaryKey, type, chat_id, created_at, metadata, role (+19 more)

### Community 334 - "Community 334"
Cohesion: 0.08
Nodes (26): name, notNull, primaryKey, type, artifact_id, prompt_summary, sandbox_snapshot_ref, source_files (+18 more)

### Community 335 - "Community 335"
Cohesion: 0.08
Nodes (26): name, notNull, primaryKey, type, artifact_id, prompt_summary, sandbox_snapshot_ref, source_files (+18 more)

### Community 336 - "Community 336"
Cohesion: 0.08
Nodes (26): name, notNull, primaryKey, type, artifact_id, prompt_summary, sandbox_snapshot_ref, source_files (+18 more)

### Community 337 - "Community 337"
Cohesion: 0.07
Nodes (27): created_at, prompt_summary, sandbox_snapshot_ref, source_files, triggering_message_id, default, name, notNull (+19 more)

### Community 338 - "Community 338"
Cohesion: 0.08
Nodes (24): columns, concurrently, isUnique, method, name, with, columns, concurrently (+16 more)

### Community 339 - "Community 339"
Cohesion: 0.08
Nodes (24): columns, concurrently, isUnique, method, name, with, columns, concurrently (+16 more)

### Community 340 - "Community 340"
Cohesion: 0.08
Nodes (24): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, canvas_artifact_versions_artifact_id_canvas_artifacts_id_fk (+16 more)

### Community 341 - "Community 341"
Cohesion: 0.08
Nodes (24): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, canvas_artifacts_chat_id_chats_id_fk (+16 more)

### Community 342 - "Community 342"
Cohesion: 0.08
Nodes (24): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, canvas_artifact_versions_artifact_id_canvas_artifacts_id_fk (+16 more)

### Community 343 - "Community 343"
Cohesion: 0.08
Nodes (24): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, canvas_artifacts_chat_id_chats_id_fk (+16 more)

### Community 344 - "Community 344"
Cohesion: 0.08
Nodes (24): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, canvas_artifact_versions_artifact_id_canvas_artifacts_id_fk (+16 more)

### Community 345 - "Community 345"
Cohesion: 0.08
Nodes (24): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, canvas_artifacts_chat_id_chats_id_fk (+16 more)

### Community 346 - "Community 346"
Cohesion: 0.08
Nodes (24): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, canvas_artifact_versions_artifact_id_canvas_artifacts_id_fk (+16 more)

### Community 347 - "Community 347"
Cohesion: 0.08
Nodes (24): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, canvas_artifacts_chat_id_chats_id_fk (+16 more)

### Community 348 - "Community 348"
Cohesion: 0.08
Nodes (24): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, canvas_artifact_versions_artifact_id_canvas_artifacts_id_fk (+16 more)

### Community 349 - "Community 349"
Cohesion: 0.08
Nodes (24): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, canvas_artifacts_chat_id_chats_id_fk (+16 more)

### Community 350 - "Community 350"
Cohesion: 0.08
Nodes (24): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, canvas_artifact_versions_artifact_id_canvas_artifacts_id_fk (+16 more)

### Community 351 - "Community 351"
Cohesion: 0.08
Nodes (24): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, canvas_artifacts_chat_id_chats_id_fk (+16 more)

### Community 352 - "Community 352"
Cohesion: 0.08
Nodes (24): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, canvas_artifact_versions_artifact_id_canvas_artifacts_id_fk (+16 more)

### Community 353 - "Community 353"
Cohesion: 0.08
Nodes (24): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, canvas_artifacts_chat_id_chats_id_fk (+16 more)

### Community 354 - "Community 354"
Cohesion: 0.08
Nodes (24): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, canvas_artifact_versions_artifact_id_canvas_artifacts_id_fk (+16 more)

### Community 355 - "Community 355"
Cohesion: 0.08
Nodes (24): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, canvas_artifacts_chat_id_chats_id_fk (+16 more)

### Community 356 - "Community 356"
Cohesion: 0.08
Nodes (24): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, canvas_artifact_versions_artifact_id_canvas_artifacts_id_fk (+16 more)

### Community 357 - "Community 357"
Cohesion: 0.08
Nodes (24): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, canvas_artifacts_chat_id_chats_id_fk (+16 more)

### Community 358 - "Community 358"
Cohesion: 0.08
Nodes (24): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, canvas_artifact_versions_artifact_id_canvas_artifacts_id_fk (+16 more)

### Community 359 - "Community 359"
Cohesion: 0.08
Nodes (24): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, canvas_artifacts_chat_id_chats_id_fk (+16 more)

### Community 360 - "Community 360"
Cohesion: 0.08
Nodes (24): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, canvas_artifact_versions_artifact_id_canvas_artifacts_id_fk (+16 more)

### Community 361 - "Community 361"
Cohesion: 0.08
Nodes (24): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, canvas_artifacts_chat_id_chats_id_fk (+16 more)

### Community 362 - "Community 362"
Cohesion: 0.08
Nodes (24): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, canvas_artifact_versions_artifact_id_canvas_artifacts_id_fk (+16 more)

### Community 363 - "Community 363"
Cohesion: 0.08
Nodes (24): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, canvas_artifacts_chat_id_chats_id_fk (+16 more)

### Community 364 - "Community 364"
Cohesion: 0.08
Nodes (24): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, canvas_artifact_versions_artifact_id_canvas_artifacts_id_fk (+16 more)

### Community 365 - "Community 365"
Cohesion: 0.08
Nodes (24): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, canvas_artifacts_chat_id_chats_id_fk (+16 more)

### Community 366 - "Community 366"
Cohesion: 0.08
Nodes (24): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, canvas_artifact_versions_artifact_id_canvas_artifacts_id_fk (+16 more)

### Community 367 - "Community 367"
Cohesion: 0.08
Nodes (24): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, canvas_artifacts_chat_id_chats_id_fk (+16 more)

### Community 368 - "Community 368"
Cohesion: 0.08
Nodes (24): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, canvas_artifact_versions_artifact_id_canvas_artifacts_id_fk (+16 more)

### Community 369 - "Community 369"
Cohesion: 0.08
Nodes (24): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, canvas_artifacts_chat_id_chats_id_fk (+16 more)

### Community 370 - "Community 370"
Cohesion: 0.09
Nodes (23): created_at, title, user_id, visibility, default, name, notNull, primaryKey (+15 more)

### Community 371 - "Community 371"
Cohesion: 0.09
Nodes (22): id, title, user_id, visibility, name, notNull, primaryKey, type (+14 more)

### Community 372 - "Community 372"
Cohesion: 0.09
Nodes (22): id, title, user_id, visibility, name, notNull, primaryKey, type (+14 more)

### Community 373 - "Community 373"
Cohesion: 0.09
Nodes (23): created_at, title, user_id, visibility, default, name, notNull, primaryKey (+15 more)

### Community 374 - "Community 374"
Cohesion: 0.09
Nodes (22): id, title, user_id, visibility, name, notNull, primaryKey, type (+14 more)

### Community 375 - "Community 375"
Cohesion: 0.09
Nodes (23): columns, concurrently, isUnique, method, name, with, artifact_revisions_artifact_id_created_at_idx, users_manage_own_artifact_revisions (+15 more)

### Community 376 - "Community 376"
Cohesion: 0.09
Nodes (23): created_at, title, user_id, visibility, default, name, notNull, primaryKey (+15 more)

### Community 377 - "Community 377"
Cohesion: 0.09
Nodes (23): current_revision_id, current_runtime_session_id, framework, status, name, notNull, primaryKey, type (+15 more)

### Community 378 - "Community 378"
Cohesion: 0.09
Nodes (23): columns, concurrently, isUnique, method, name, with, artifact_revisions_artifact_id_created_at_idx, users_manage_own_artifact_revisions (+15 more)

### Community 379 - "Community 379"
Cohesion: 0.09
Nodes (23): columns, concurrently, isUnique, method, name, with, artifact_revisions_artifact_id_created_at_idx, users_manage_own_artifact_revisions (+15 more)

### Community 380 - "Community 380"
Cohesion: 0.09
Nodes (23): columns, concurrently, isUnique, method, name, with, artifact_revisions_artifact_id_created_at_idx, users_manage_own_artifact_revisions (+15 more)

### Community 381 - "Community 381"
Cohesion: 0.09
Nodes (23): columns, concurrently, isUnique, method, name, with, artifact_revisions_artifact_id_created_at_idx, users_manage_own_artifact_revisions (+15 more)

### Community 382 - "Community 382"
Cohesion: 0.09
Nodes (23): columns, concurrently, isUnique, method, name, with, artifact_revisions_artifact_id_created_at_idx, users_manage_own_artifact_revisions (+15 more)

### Community 383 - "Community 383"
Cohesion: 0.09
Nodes (23): columns, concurrently, isUnique, method, name, with, artifact_revisions_artifact_id_created_at_idx, users_manage_own_artifact_revisions (+15 more)

### Community 384 - "Community 384"
Cohesion: 0.09
Nodes (23): columns, concurrently, isUnique, method, name, with, artifact_revisions_artifact_id_created_at_idx, users_manage_own_artifact_revisions (+15 more)

### Community 385 - "Community 385"
Cohesion: 0.09
Nodes (23): columns, concurrently, isUnique, method, name, with, artifact_revisions_artifact_id_created_at_idx, users_manage_own_artifact_revisions (+15 more)

### Community 386 - "Community 386"
Cohesion: 0.09
Nodes (23): columns, concurrently, isUnique, method, name, with, artifact_revisions_artifact_id_created_at_idx, users_manage_own_artifact_revisions (+15 more)

### Community 387 - "Community 387"
Cohesion: 0.09
Nodes (23): columns, concurrently, isUnique, method, name, with, artifact_revisions_artifact_id_created_at_idx, users_manage_own_artifact_revisions (+15 more)

### Community 388 - "Community 388"
Cohesion: 0.09
Nodes (23): columns, concurrently, isUnique, method, name, with, artifact_revisions_artifact_id_created_at_idx, users_manage_own_artifact_revisions (+15 more)

### Community 389 - "Community 389"
Cohesion: 0.09
Nodes (23): columns, concurrently, isUnique, method, name, with, artifact_revisions_artifact_id_created_at_idx, users_manage_own_artifact_revisions (+15 more)

### Community 390 - "Community 390"
Cohesion: 0.09
Nodes (23): columns, concurrently, isUnique, method, name, with, artifact_revisions_artifact_id_created_at_idx, users_manage_own_artifact_revisions (+15 more)

### Community 391 - "Community 391"
Cohesion: 0.09
Nodes (23): columns, concurrently, isUnique, method, name, with, artifact_revisions_artifact_id_created_at_idx, users_manage_own_artifact_revisions (+15 more)

### Community 392 - "Community 392"
Cohesion: 0.09
Nodes (23): columns, concurrently, isUnique, method, name, with, artifact_revisions_artifact_id_created_at_idx, users_manage_own_artifact_revisions (+15 more)

### Community 393 - "Community 393"
Cohesion: 0.09
Nodes (23): columns, concurrently, isUnique, method, name, with, artifact_revisions_artifact_id_created_at_idx, users_manage_own_artifact_revisions (+15 more)

### Community 394 - "Community 394"
Cohesion: 0.09
Nodes (22): name, notNull, primaryKey, type, chat_id, created_at, id, role (+14 more)

### Community 395 - "Community 395"
Cohesion: 0.09
Nodes (22): id, title, user_id, visibility, name, notNull, primaryKey, type (+14 more)

### Community 396 - "Community 396"
Cohesion: 0.09
Nodes (22): id, title, user_id, visibility, name, notNull, primaryKey, type (+14 more)

### Community 397 - "Community 397"
Cohesion: 0.09
Nodes (22): columns, concurrently, isUnique, method, name, with, columns, concurrently (+14 more)

### Community 398 - "Community 398"
Cohesion: 0.09
Nodes (22): columns, concurrently, isUnique, method, name, with, columns, concurrently (+14 more)

### Community 399 - "Community 399"
Cohesion: 0.09
Nodes (23): created_at, title, user_id, visibility, default, name, notNull, primaryKey (+15 more)

### Community 400 - "Community 400"
Cohesion: 0.09
Nodes (22): name, notNull, primaryKey, type, chat_id, metadata, role, updated_at (+14 more)

### Community 401 - "Community 401"
Cohesion: 0.09
Nodes (22): name, notNull, primaryKey, type, chat_id, metadata, role, updated_at (+14 more)

### Community 402 - "Community 402"
Cohesion: 0.09
Nodes (22): id, title, user_id, visibility, name, notNull, primaryKey, type (+14 more)

### Community 403 - "Community 403"
Cohesion: 0.09
Nodes (22): id, title, user_id, visibility, name, notNull, primaryKey, type (+14 more)

### Community 404 - "Community 404"
Cohesion: 0.09
Nodes (22): id, title, user_id, visibility, name, notNull, primaryKey, type (+14 more)

### Community 405 - "Community 405"
Cohesion: 0.09
Nodes (22): id, title, user_id, visibility, name, notNull, primaryKey, type (+14 more)

### Community 406 - "Community 406"
Cohesion: 0.09
Nodes (22): id, title, user_id, visibility, name, notNull, primaryKey, type (+14 more)

### Community 407 - "Community 407"
Cohesion: 0.09
Nodes (22): id, title, user_id, visibility, name, notNull, primaryKey, type (+14 more)

### Community 408 - "Community 408"
Cohesion: 0.09
Nodes (22): created_at, created_by, source_snapshot, version_number, default, name, notNull, primaryKey (+14 more)

### Community 409 - "Community 409"
Cohesion: 0.09
Nodes (22): id, title, user_id, visibility, name, notNull, primaryKey, type (+14 more)

### Community 410 - "Community 410"
Cohesion: 0.09
Nodes (22): id, title, user_id, visibility, name, notNull, primaryKey, type (+14 more)

### Community 411 - "Community 411"
Cohesion: 0.09
Nodes (22): id, title, user_id, visibility, name, notNull, primaryKey, type (+14 more)

### Community 412 - "Community 412"
Cohesion: 0.09
Nodes (22): created_at, created_by, source_snapshot, version_number, default, name, notNull, primaryKey (+14 more)

### Community 413 - "Community 413"
Cohesion: 0.09
Nodes (22): id, title, user_id, visibility, name, notNull, primaryKey, type (+14 more)

### Community 414 - "Community 414"
Cohesion: 0.09
Nodes (22): id, title, user_id, visibility, name, notNull, primaryKey, type (+14 more)

### Community 415 - "Community 415"
Cohesion: 0.09
Nodes (22): id, title, user_id, visibility, name, notNull, primaryKey, type (+14 more)

### Community 416 - "Community 416"
Cohesion: 0.09
Nodes (22): id, title, user_id, visibility, name, notNull, primaryKey, type (+14 more)

### Community 417 - "Community 417"
Cohesion: 0.09
Nodes (22): id, title, user_id, visibility, name, notNull, primaryKey, type (+14 more)

### Community 418 - "Community 418"
Cohesion: 0.10
Nodes (21): name, notNull, primaryKey, type, artifact_id, created_by, source_snapshot, version_number (+13 more)

### Community 419 - "Community 419"
Cohesion: 0.09
Nodes (22): id, title, user_id, visibility, name, notNull, primaryKey, type (+14 more)

### Community 420 - "Community 420"
Cohesion: 0.10
Nodes (21): data_prefix, reasoning_text, source_document_title, tool_dynamic_name, name, notNull, primaryKey, type (+13 more)

### Community 421 - "Community 421"
Cohesion: 0.10
Nodes (21): message, page_url, sentiment, user_agent, name, notNull, primaryKey, type (+13 more)

### Community 422 - "Community 422"
Cohesion: 0.10
Nodes (21): message, page_url, sentiment, user_agent, name, notNull, primaryKey, type (+13 more)

### Community 423 - "Community 423"
Cohesion: 0.07
Nodes (27): name, notNull, primaryKey, type, chat_id, created_at, metadata, role (+19 more)

### Community 424 - "Community 424"
Cohesion: 0.40
Nodes (5): data_content, name, notNull, primaryKey, type

### Community 425 - "Community 425"
Cohesion: 0.08
Nodes (26): name, notNull, primaryKey, type, chat_id, id, metadata, role (+18 more)

### Community 426 - "Community 426"
Cohesion: 0.40
Nodes (5): data_content, name, notNull, primaryKey, type

### Community 427 - "Community 427"
Cohesion: 0.10
Nodes (21): message, page_url, sentiment, user_agent, name, notNull, primaryKey, type (+13 more)

### Community 428 - "Community 428"
Cohesion: 0.08
Nodes (26): name, notNull, primaryKey, type, chat_id, id, metadata, role (+18 more)

### Community 429 - "Community 429"
Cohesion: 0.10
Nodes (21): message, page_url, sentiment, user_agent, name, notNull, primaryKey, type (+13 more)

### Community 430 - "Community 430"
Cohesion: 0.10
Nodes (21): as, for, name, to, withCheck, as, for, name (+13 more)

### Community 431 - "Community 431"
Cohesion: 0.10
Nodes (21): name, notNull, primaryKey, type, artifact_id, prompt_summary, sandbox_snapshot_ref, triggering_message_id (+13 more)

### Community 432 - "Community 432"
Cohesion: 0.10
Nodes (21): message, page_url, sentiment, user_agent, name, notNull, primaryKey, type (+13 more)

### Community 433 - "Community 433"
Cohesion: 0.10
Nodes (21): as, for, name, to, withCheck, as, for, name (+13 more)

### Community 434 - "Community 434"
Cohesion: 0.10
Nodes (21): message, page_url, sentiment, user_agent, name, notNull, primaryKey, type (+13 more)

### Community 435 - "Community 435"
Cohesion: 0.10
Nodes (21): name, notNull, primaryKey, type, artifact_id, created_by, source_snapshot, version_number (+13 more)

### Community 436 - "Community 436"
Cohesion: 0.10
Nodes (21): name, notNull, primaryKey, type, artifact_id, created_by, source_snapshot, version_number (+13 more)

### Community 437 - "Community 437"
Cohesion: 0.10
Nodes (21): name, notNull, primaryKey, type, artifact_id, created_by, source_snapshot, version_number (+13 more)

### Community 438 - "Community 438"
Cohesion: 0.10
Nodes (21): name, notNull, primaryKey, type, artifact_id, created_by, source_snapshot, version_number (+13 more)

### Community 439 - "Community 439"
Cohesion: 0.10
Nodes (21): name, notNull, primaryKey, type, artifact_id, created_by, source_snapshot, version_number (+13 more)

### Community 440 - "Community 440"
Cohesion: 0.09
Nodes (22): created_at, created_by, source_snapshot, version_number, default, name, notNull, primaryKey (+14 more)

### Community 441 - "Community 441"
Cohesion: 0.10
Nodes (21): name, notNull, primaryKey, type, artifact_id, created_by, source_snapshot, version_number (+13 more)

### Community 442 - "Community 442"
Cohesion: 0.10
Nodes (21): name, notNull, primaryKey, type, artifact_id, created_by, source_snapshot, version_number (+13 more)

### Community 443 - "Community 443"
Cohesion: 0.10
Nodes (21): name, notNull, primaryKey, type, artifact_id, created_by, source_snapshot, version_number (+13 more)

### Community 444 - "Community 444"
Cohesion: 0.10
Nodes (21): name, notNull, primaryKey, type, artifact_id, created_by, source_snapshot, version_number (+13 more)

### Community 445 - "Community 445"
Cohesion: 0.10
Nodes (21): name, notNull, primaryKey, type, artifact_id, created_by, source_snapshot, version_number (+13 more)

### Community 446 - "Community 446"
Cohesion: 0.09
Nodes (22): created_at, created_by, source_snapshot, version_number, default, name, notNull, primaryKey (+14 more)

### Community 447 - "Community 447"
Cohesion: 0.10
Nodes (19): Admin surface, AI provider options (Direct), Authentication (Supabase), Canvas artifacts, Cloud deployment controls, Core behavior controls, Evals cron (Railway `polymorph-evals`), Implementation Details (+11 more)

### Community 448 - "Community 448"
Cohesion: 0.11
Nodes (18): Adapter Pattern, Adding a New Display Tool, Community-portability evidence, Display Tool Components, Display Tools (Server), Dynamic Tool Display, End-to-End Rendering Pipeline, Generative UI (+10 more)

### Community 449 - "Community 449"
Cohesion: 0.11
Nodes (18): Auxiliary Agents, Context Window Management, Dynamic Tool Parts, End-to-End Pipeline, Extending the Agent, Key Files, Message Persistence Contract, Model Selection (+10 more)

### Community 450 - "Community 450"
Cohesion: 0.11
Nodes (18): API Reference, Authentication, Canvas Artifact Endpoints, Endpoints, Error Response Conventions, GET `/api/canvas-assets/image-proxy`, GET `/api/chats`, GET `/api/health` (+10 more)

### Community 451 - "Community 451"
Cohesion: 0.11
Nodes (18): id, message_id, order, type, name, notNull, primaryKey, type (+10 more)

### Community 452 - "Community 452"
Cohesion: 0.12
Nodes (15): Error Handling and Retries, Provider Comparison, Provider Selection Logic, Search Provider Selection, 1. Create the provider class, 2. Register in the factory, 3. Add environment variables, 4. Test with the agent (+7 more)

### Community 453 - "Community 453"
Cohesion: 0.12
Nodes (17): messages_chat_id_chats_id_fk, columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo (+9 more)

### Community 454 - "Community 454"
Cohesion: 0.12
Nodes (17): parts_message_id_messages_id_fk, columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo (+9 more)

### Community 455 - "Community 455"
Cohesion: 0.12
Nodes (17): messages_chat_id_chats_id_fk, columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo (+9 more)

### Community 456 - "Community 456"
Cohesion: 0.12
Nodes (17): parts_message_id_messages_id_fk, columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo (+9 more)

### Community 457 - "Community 457"
Cohesion: 0.12
Nodes (17): messages_chat_id_chats_id_fk, columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo (+9 more)

### Community 458 - "Community 458"
Cohesion: 0.12
Nodes (17): parts_message_id_messages_id_fk, columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo (+9 more)

### Community 459 - "Community 459"
Cohesion: 0.12
Nodes (17): messages_chat_id_chats_id_fk, columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo (+9 more)

### Community 460 - "Community 460"
Cohesion: 0.12
Nodes (17): messages_chat_id_chats_id_fk, columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo (+9 more)

### Community 461 - "Community 461"
Cohesion: 0.12
Nodes (17): parts_message_id_messages_id_fk, columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo (+9 more)

### Community 462 - "Community 462"
Cohesion: 0.12
Nodes (17): messages_chat_id_chats_id_fk, columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo (+9 more)

### Community 463 - "Community 463"
Cohesion: 0.12
Nodes (17): parts_message_id_messages_id_fk, columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo (+9 more)

### Community 464 - "Community 464"
Cohesion: 0.12
Nodes (16): dialect, id, prevId, checkConstraints, compositePrimaryKeys, foreignKeys, isRLSEnabled, name (+8 more)

### Community 465 - "Community 465"
Cohesion: 0.12
Nodes (17): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, columnsFrom (+9 more)

### Community 466 - "Community 466"
Cohesion: 0.12
Nodes (16): dialect, id, prevId, checkConstraints, compositePrimaryKeys, foreignKeys, isRLSEnabled, name (+8 more)

### Community 467 - "Community 467"
Cohesion: 0.12
Nodes (17): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, columnsFrom (+9 more)

### Community 468 - "Community 468"
Cohesion: 0.12
Nodes (17): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, columnsFrom (+9 more)

### Community 469 - "Community 469"
Cohesion: 0.12
Nodes (17): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, columnsFrom (+9 more)

### Community 470 - "Community 470"
Cohesion: 0.12
Nodes (17): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, columnsFrom (+9 more)

### Community 471 - "Community 471"
Cohesion: 0.12
Nodes (17): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, columnsFrom (+9 more)

### Community 472 - "Community 472"
Cohesion: 0.12
Nodes (17): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, columnsFrom (+9 more)

### Community 473 - "Community 473"
Cohesion: 0.12
Nodes (17): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, columnsFrom (+9 more)

### Community 474 - "Community 474"
Cohesion: 0.12
Nodes (17): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, columnsFrom (+9 more)

### Community 475 - "Community 475"
Cohesion: 0.12
Nodes (17): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, columnsFrom (+9 more)

### Community 476 - "Community 476"
Cohesion: 0.12
Nodes (17): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, columnsFrom (+9 more)

### Community 477 - "Community 477"
Cohesion: 0.12
Nodes (17): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, columnsFrom (+9 more)

### Community 478 - "Community 478"
Cohesion: 0.12
Nodes (17): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, columnsFrom (+9 more)

### Community 479 - "Community 479"
Cohesion: 0.12
Nodes (17): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, columnsFrom (+9 more)

### Community 480 - "Community 480"
Cohesion: 0.12
Nodes (17): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, columnsFrom (+9 more)

### Community 481 - "Community 481"
Cohesion: 0.12
Nodes (17): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, columnsFrom (+9 more)

### Community 482 - "Community 482"
Cohesion: 0.12
Nodes (17): columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo, columnsFrom (+9 more)

### Community 483 - "Community 483"
Cohesion: 0.22
Nodes (8): checkAndEnforceVoiceLimit(), POST(), {
  getCurrentUserId,
  jsonError,
  isVoiceEnabled,
  resolveProvider,
  synthesizeElevenLabs,
  synthesizeOpenAI,
  checkAndEnforceVoiceLimit
}, withProviderTimeout(), resolveProvider(), synthesizeElevenLabs(), synthesizeOpenAI(), validateAudioContentType()

### Community 484 - "Community 484"
Cohesion: 0.12
Nodes (16): Auxiliary API Endpoints, Environment Variables, Error Responses, Error Responses, GET `/api/health`, GET `/api/suggestions`, GET `/api/suggestions/refresh`, POST `/api/advanced-search` (+8 more)

### Community 485 - "Community 485"
Cohesion: 0.12
Nodes (16): Constraints, Error Responses, Error Responses, Example, Example, Example, GET `/api/chats`, POST `/api/feedback` (+8 more)

### Community 486 - "Community 486"
Cohesion: 0.24
Nodes (11): AccordionContent, AccordionItem, AccordionTrigger, Table, TableBody, TableCaption, TableCell, TableFooter (+3 more)

### Community 487 - "Community 487"
Cohesion: 0.12
Nodes (16): file_fields_required, reasoning_text_required, text_text_required, tool_fields_required, tool_state_valid, name, value, checkConstraints (+8 more)

### Community 488 - "Community 488"
Cohesion: 0.12
Nodes (16): file_fields_required, reasoning_text_required, text_text_required, tool_fields_required, tool_state_valid, name, value, checkConstraints (+8 more)

### Community 489 - "Community 489"
Cohesion: 0.12
Nodes (16): file_fields_required, reasoning_text_required, text_text_required, tool_fields_required, tool_state_valid, name, value, checkConstraints (+8 more)

### Community 490 - "Community 490"
Cohesion: 0.12
Nodes (16): file_fields_required, reasoning_text_required, text_text_required, tool_fields_required, tool_state_valid, name, value, checkConstraints (+8 more)

### Community 491 - "Community 491"
Cohesion: 0.12
Nodes (16): file_fields_required, reasoning_text_required, text_text_required, tool_fields_required, tool_state_valid, name, value, checkConstraints (+8 more)

### Community 492 - "Community 492"
Cohesion: 0.12
Nodes (16): file_fields_required, reasoning_text_required, text_text_required, tool_fields_required, tool_state_valid, name, value, checkConstraints (+8 more)

### Community 493 - "Community 493"
Cohesion: 0.12
Nodes (16): file_media_type, reasoning_text, source_document_source_id, name, notNull, primaryKey, type, columns (+8 more)

### Community 494 - "Community 494"
Cohesion: 0.12
Nodes (16): users_manage_own_chats, checkConstraints, compositePrimaryKeys, foreignKeys, isRLSEnabled, name, policies, schema (+8 more)

### Community 495 - "Community 495"
Cohesion: 0.12
Nodes (16): file_fields_required, reasoning_text_required, text_text_required, tool_fields_required, tool_state_valid, name, value, checkConstraints (+8 more)

### Community 496 - "Community 496"
Cohesion: 0.12
Nodes (16): messages_chat_id_chats_id_fk, columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo (+8 more)

### Community 497 - "Community 497"
Cohesion: 0.12
Nodes (16): file_fields_required, reasoning_text_required, text_text_required, tool_fields_required, tool_state_valid, name, value, checkConstraints (+8 more)

### Community 498 - "Community 498"
Cohesion: 0.12
Nodes (16): messages_chat_id_chats_id_fk, columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo (+8 more)

### Community 499 - "Community 499"
Cohesion: 0.12
Nodes (16): file_fields_required, reasoning_text_required, text_text_required, tool_fields_required, tool_state_valid, name, value, checkConstraints (+8 more)

### Community 500 - "Community 500"
Cohesion: 0.12
Nodes (16): file_media_type, message_id, reasoning_text, name, notNull, primaryKey, type, name (+8 more)

### Community 501 - "Community 501"
Cohesion: 0.12
Nodes (16): messages_chat_id_chats_id_fk, columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo (+8 more)

### Community 502 - "Community 502"
Cohesion: 0.12
Nodes (16): file_fields_required, reasoning_text_required, text_text_required, tool_fields_required, tool_state_valid, name, value, checkConstraints (+8 more)

### Community 503 - "Community 503"
Cohesion: 0.12
Nodes (16): file_media_type, message_id, reasoning_text, name, notNull, primaryKey, type, name (+8 more)

### Community 504 - "Community 504"
Cohesion: 0.12
Nodes (16): messages_chat_id_chats_id_fk, columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo (+8 more)

### Community 505 - "Community 505"
Cohesion: 0.12
Nodes (16): messages_chat_id_chats_id_fk, columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo (+8 more)

### Community 506 - "Community 506"
Cohesion: 0.15
Nodes (13): Generative UI Display Contracts, Geo-map rendering contract, Mode-specific tool availability, Related: side-effect tools, Passive vs Interactive Display Tool Distinction, Geo-map Rendering Contract, Mode-specific Tool Availability (Chat/Research), Side-effect Tools (generateImage, canvas) (+5 more)

### Community 507 - "Community 507"
Cohesion: 0.13
Nodes (14): dialect, enums, id, _meta, columns, schemas, tables, policies (+6 more)

### Community 508 - "Community 508"
Cohesion: 0.13
Nodes (15): messages_chat_id_created_at_idx, messages_chat_id_idx, columns, concurrently, isUnique, method, name, with (+7 more)

### Community 509 - "Community 509"
Cohesion: 0.13
Nodes (15): parts_message_id_idx, parts_message_id_order_idx, columns, concurrently, isUnique, method, name, with (+7 more)

### Community 510 - "Community 510"
Cohesion: 0.13
Nodes (14): dialect, enums, id, _meta, columns, schemas, tables, policies (+6 more)

### Community 511 - "Community 511"
Cohesion: 0.13
Nodes (15): messages_chat_id_created_at_idx, messages_chat_id_idx, columns, concurrently, isUnique, method, name, with (+7 more)

### Community 512 - "Community 512"
Cohesion: 0.13
Nodes (15): parts_message_id_idx, parts_message_id_order_idx, columns, concurrently, isUnique, method, name, with (+7 more)

### Community 513 - "Community 513"
Cohesion: 0.13
Nodes (14): dialect, enums, id, _meta, columns, schemas, tables, policies (+6 more)

### Community 514 - "Community 514"
Cohesion: 0.13
Nodes (15): messages_chat_id_created_at_idx, messages_chat_id_idx, columns, concurrently, isUnique, method, name, with (+7 more)

### Community 515 - "Community 515"
Cohesion: 0.13
Nodes (15): parts_message_id_idx, parts_message_id_order_idx, columns, concurrently, isUnique, method, name, with (+7 more)

### Community 516 - "Community 516"
Cohesion: 0.13
Nodes (14): dialect, enums, id, _meta, columns, schemas, tables, policies (+6 more)

### Community 517 - "Community 517"
Cohesion: 0.13
Nodes (15): messages_chat_id_created_at_idx, messages_chat_id_idx, columns, concurrently, isUnique, method, name, with (+7 more)

### Community 518 - "Community 518"
Cohesion: 0.13
Nodes (15): parts_message_id_idx, parts_message_id_order_idx, columns, concurrently, isUnique, method, name, with (+7 more)

### Community 519 - "Community 519"
Cohesion: 0.13
Nodes (14): dialect, enums, id, _meta, columns, schemas, tables, policies (+6 more)

### Community 520 - "Community 520"
Cohesion: 0.13
Nodes (15): messages_chat_id_created_at_idx, messages_chat_id_idx, columns, concurrently, isUnique, method, name, with (+7 more)

### Community 521 - "Community 521"
Cohesion: 0.13
Nodes (15): parts_message_id_idx, parts_message_id_order_idx, columns, concurrently, isUnique, method, name, with (+7 more)

### Community 522 - "Community 522"
Cohesion: 0.13
Nodes (14): dialect, enums, id, _meta, columns, schemas, tables, policies (+6 more)

### Community 523 - "Community 523"
Cohesion: 0.13
Nodes (15): as, for, name, to, withCheck, anyone_can_insert_feedback, checkConstraints, compositePrimaryKeys (+7 more)

### Community 524 - "Community 524"
Cohesion: 0.13
Nodes (15): columns, concurrently, isUnique, method, name, with, columns, concurrently (+7 more)

### Community 525 - "Community 525"
Cohesion: 0.13
Nodes (15): messages_chat_id_created_at_idx, messages_chat_id_idx, columns, concurrently, isUnique, method, name, with (+7 more)

### Community 526 - "Community 526"
Cohesion: 0.13
Nodes (15): parts_message_id_idx, parts_message_id_order_idx, columns, concurrently, isUnique, method, name, with (+7 more)

### Community 527 - "Community 527"
Cohesion: 0.13
Nodes (14): dialect, enums, id, _meta, columns, schemas, tables, policies (+6 more)

### Community 528 - "Community 528"
Cohesion: 0.13
Nodes (15): as, for, name, to, withCheck, anyone_can_insert_feedback, checkConstraints, compositePrimaryKeys (+7 more)

### Community 529 - "Community 529"
Cohesion: 0.13
Nodes (15): columns, concurrently, isUnique, method, name, with, columns, concurrently (+7 more)

### Community 530 - "Community 530"
Cohesion: 0.13
Nodes (15): columns, concurrently, isUnique, method, name, with, columns, concurrently (+7 more)

### Community 531 - "Community 531"
Cohesion: 0.13
Nodes (15): messages_chat_id_created_at_idx, messages_chat_id_idx, columns, concurrently, isUnique, method, name, with (+7 more)

### Community 532 - "Community 532"
Cohesion: 0.13
Nodes (15): parts_message_id_idx, parts_message_id_order_idx, columns, concurrently, isUnique, method, name, with (+7 more)

### Community 533 - "Community 533"
Cohesion: 0.13
Nodes (15): users_manage_chat_messages, checkConstraints, compositePrimaryKeys, isRLSEnabled, name, policies, schema, uniqueConstraints (+7 more)

### Community 534 - "Community 534"
Cohesion: 0.13
Nodes (15): users_manage_message_parts, compositePrimaryKeys, isRLSEnabled, name, policies, schema, uniqueConstraints, tables (+7 more)

### Community 535 - "Community 535"
Cohesion: 0.13
Nodes (14): dialect, enums, id, _meta, columns, schemas, tables, policies (+6 more)

### Community 536 - "Community 536"
Cohesion: 0.13
Nodes (15): as, for, name, to, withCheck, anyone_can_insert_feedback, checkConstraints, compositePrimaryKeys (+7 more)

### Community 537 - "Community 537"
Cohesion: 0.13
Nodes (15): columns, concurrently, isUnique, method, name, with, columns, concurrently (+7 more)

### Community 538 - "Community 538"
Cohesion: 0.13
Nodes (15): columns, concurrently, isUnique, method, name, with, columns, concurrently (+7 more)

### Community 539 - "Community 539"
Cohesion: 0.13
Nodes (15): messages_chat_id_created_at_idx, messages_chat_id_idx, columns, concurrently, isUnique, method, name, with (+7 more)

### Community 540 - "Community 540"
Cohesion: 0.13
Nodes (15): parts_message_id_idx, parts_message_id_order_idx, columns, concurrently, isUnique, method, name, with (+7 more)

### Community 541 - "Community 541"
Cohesion: 0.13
Nodes (14): dialect, enums, id, _meta, columns, schemas, tables, policies (+6 more)

### Community 542 - "Community 542"
Cohesion: 0.13
Nodes (15): as, for, name, to, withCheck, anyone_can_insert_feedback, checkConstraints, compositePrimaryKeys (+7 more)

### Community 543 - "Community 543"
Cohesion: 0.13
Nodes (15): columns, concurrently, isUnique, method, name, with, columns, concurrently (+7 more)

### Community 544 - "Community 544"
Cohesion: 0.13
Nodes (15): columns, concurrently, isUnique, method, name, with, columns, concurrently (+7 more)

### Community 545 - "Community 545"
Cohesion: 0.13
Nodes (15): messages_chat_id_created_at_idx, messages_chat_id_idx, columns, concurrently, isUnique, method, name, with (+7 more)

### Community 546 - "Community 546"
Cohesion: 0.13
Nodes (15): parts_message_id_idx, parts_message_id_order_idx, columns, concurrently, isUnique, method, name, with (+7 more)

### Community 547 - "Community 547"
Cohesion: 0.13
Nodes (14): dialect, enums, id, _meta, columns, schemas, tables, policies (+6 more)

### Community 548 - "Community 548"
Cohesion: 0.13
Nodes (15): as, for, name, to, withCheck, anyone_can_insert_feedback, checkConstraints, compositePrimaryKeys (+7 more)

### Community 549 - "Community 549"
Cohesion: 0.13
Nodes (15): columns, concurrently, isUnique, method, name, with, columns, concurrently (+7 more)

### Community 550 - "Community 550"
Cohesion: 0.13
Nodes (15): columns, concurrently, isUnique, method, name, with, columns, concurrently (+7 more)

### Community 551 - "Community 551"
Cohesion: 0.13
Nodes (15): messages_chat_id_created_at_idx, messages_chat_id_idx, columns, concurrently, isUnique, method, name, with (+7 more)

### Community 552 - "Community 552"
Cohesion: 0.13
Nodes (15): parts_message_id_idx, parts_message_id_order_idx, columns, concurrently, isUnique, method, name, with (+7 more)

### Community 553 - "Community 553"
Cohesion: 0.13
Nodes (15): checkConstraints, compositePrimaryKeys, foreignKeys, isRLSEnabled, name, schema, uniqueConstraints, compositePrimaryKeys (+7 more)

### Community 554 - "Community 554"
Cohesion: 0.13
Nodes (15): columns, concurrently, isUnique, method, name, with, columns, concurrently (+7 more)

### Community 555 - "Community 555"
Cohesion: 0.13
Nodes (15): columns, concurrently, isUnique, method, name, with, columns, concurrently (+7 more)

### Community 556 - "Community 556"
Cohesion: 0.13
Nodes (15): messages_chat_id_created_at_idx, messages_chat_id_idx, columns, concurrently, isUnique, method, name, with (+7 more)

### Community 557 - "Community 557"
Cohesion: 0.13
Nodes (15): parts_message_id_idx, parts_message_id_order_idx, columns, concurrently, isUnique, method, name, with (+7 more)

### Community 558 - "Community 558"
Cohesion: 0.13
Nodes (15): checkConstraints, compositePrimaryKeys, foreignKeys, isRLSEnabled, name, schema, uniqueConstraints, compositePrimaryKeys (+7 more)

### Community 559 - "Community 559"
Cohesion: 0.13
Nodes (15): columns, concurrently, isUnique, method, name, with, columns, concurrently (+7 more)

### Community 560 - "Community 560"
Cohesion: 0.13
Nodes (15): messages_chat_id_created_at_idx, messages_chat_id_idx, columns, concurrently, isUnique, method, name, with (+7 more)

### Community 561 - "Community 561"
Cohesion: 0.13
Nodes (15): columns, concurrently, isUnique, method, name, with, columns, concurrently (+7 more)

### Community 562 - "Community 562"
Cohesion: 0.13
Nodes (15): columns, concurrently, isUnique, method, name, with, columns, concurrently (+7 more)

### Community 563 - "Community 563"
Cohesion: 0.13
Nodes (15): columns, concurrently, isUnique, method, name, with, columns, concurrently (+7 more)

### Community 564 - "Community 564"
Cohesion: 0.13
Nodes (15): columns, concurrently, isUnique, method, name, with, columns, concurrently (+7 more)

### Community 565 - "Community 565"
Cohesion: 0.13
Nodes (15): columns, concurrently, isUnique, method, name, with, columns, concurrently (+7 more)

### Community 566 - "Community 566"
Cohesion: 0.13
Nodes (15): columns, concurrently, isUnique, method, name, with, columns, concurrently (+7 more)

### Community 567 - "Community 567"
Cohesion: 0.13
Nodes (15): columns, concurrently, isUnique, method, name, with, columns, concurrently (+7 more)

### Community 568 - "Community 568"
Cohesion: 0.13
Nodes (15): columns, concurrently, isUnique, method, name, with, columns, concurrently (+7 more)

### Community 569 - "Community 569"
Cohesion: 0.13
Nodes (15): columns, concurrently, isUnique, method, name, with, columns, concurrently (+7 more)

### Community 570 - "Community 570"
Cohesion: 0.13
Nodes (15): columns, concurrently, isUnique, method, name, with, columns, concurrently (+7 more)

### Community 571 - "Community 571"
Cohesion: 0.13
Nodes (15): columns, concurrently, isUnique, method, name, with, columns, concurrently (+7 more)

### Community 572 - "Community 572"
Cohesion: 0.13
Nodes (15): columns, concurrently, isUnique, method, name, with, columns, concurrently (+7 more)

### Community 573 - "Community 573"
Cohesion: 0.13
Nodes (15): columns, concurrently, isUnique, method, name, with, columns, concurrently (+7 more)

### Community 574 - "Community 574"
Cohesion: 0.13
Nodes (15): columns, concurrently, isUnique, method, name, with, columns, concurrently (+7 more)

### Community 575 - "Community 575"
Cohesion: 0.13
Nodes (15): columns, concurrently, isUnique, method, name, with, columns, concurrently (+7 more)

### Community 576 - "Community 576"
Cohesion: 0.13
Nodes (15): columns, concurrently, isUnique, method, name, with, columns, concurrently (+7 more)

### Community 577 - "Community 577"
Cohesion: 0.13
Nodes (15): columns, concurrently, isUnique, method, name, with, columns, concurrently (+7 more)

### Community 578 - "Community 578"
Cohesion: 0.13
Nodes (15): columns, concurrently, isUnique, method, name, with, columns, concurrently (+7 more)

### Community 579 - "Community 579"
Cohesion: 0.13
Nodes (15): columns, concurrently, isUnique, method, name, with, columns, concurrently (+7 more)

### Community 580 - "Community 580"
Cohesion: 0.13
Nodes (15): columns, concurrently, isUnique, method, name, with, columns, concurrently (+7 more)

### Community 581 - "Community 581"
Cohesion: 0.13
Nodes (15): columns, concurrently, isUnique, method, name, with, columns, concurrently (+7 more)

### Community 582 - "Community 582"
Cohesion: 0.13
Nodes (15): columns, concurrently, isUnique, method, name, with, columns, concurrently (+7 more)

### Community 583 - "Community 583"
Cohesion: 0.13
Nodes (15): columns, concurrently, isUnique, method, name, with, columns, concurrently (+7 more)

### Community 584 - "Community 584"
Cohesion: 0.13
Nodes (15): columns, concurrently, isUnique, method, name, with, columns, concurrently (+7 more)

### Community 585 - "Community 585"
Cohesion: 0.13
Nodes (15): columns, concurrently, isUnique, method, name, with, columns, concurrently (+7 more)

### Community 586 - "Community 586"
Cohesion: 0.13
Nodes (15): columns, concurrently, isUnique, method, name, with, columns, concurrently (+7 more)

### Community 587 - "Community 587"
Cohesion: 0.13
Nodes (15): columns, concurrently, isUnique, method, name, with, columns, concurrently (+7 more)

### Community 588 - "Community 588"
Cohesion: 0.13
Nodes (15): columns, concurrently, isUnique, method, name, with, columns, concurrently (+7 more)

### Community 589 - "Community 589"
Cohesion: 0.13
Nodes (15): columns, concurrently, isUnique, method, name, with, columns, concurrently (+7 more)

### Community 590 - "Community 590"
Cohesion: 0.13
Nodes (15): columns, concurrently, isUnique, method, name, with, columns, concurrently (+7 more)

### Community 591 - "Community 591"
Cohesion: 0.13
Nodes (15): columns, concurrently, isUnique, method, name, with, columns, concurrently (+7 more)

### Community 592 - "Community 592"
Cohesion: 0.14
Nodes (13): 1. Contract definition, 2. Serializable vs. runtime types, 3. Shared base schemas, Adapter behavior, Adapter contents by component, Adapter Pattern, Generative UI Registry and Adapters, `isRegisteredToolUI(toolName)` (+5 more)

### Community 593 - "Community 593"
Cohesion: 0.14
Nodes (13): Architecture, Authentication Flow, Database Schema, Generative UI Component Tree, Key File Reference, Model Selection, RLS Policy Chain, Streaming Architecture (+5 more)

### Community 594 - "Community 594"
Cohesion: 0.23
Nodes (11): Vendor global registry shim (__CANVAS_VENDOR__), buildChunk(), buildVendorEntry(), createShimPlugin(), EXPORT_NAMES, VENDOR_DIR, assertChunkMapComplete(), getAllBundleSpecifiers() (+3 more)

### Community 595 - "Community 595"
Cohesion: 0.20
Nodes (13): ColumnKey, DataTableClientProps, DataTableContextValue, DataTableProps, DataTableRowData, DataTableSerializableProps, FormatFor, JsonPrimitive (+5 more)

### Community 596 - "Community 596"
Cohesion: 0.14
Nodes (14): public_chat_messages_readable, users_manage_chat_messages, as, for, name, to, using, policies (+6 more)

### Community 597 - "Community 597"
Cohesion: 0.14
Nodes (14): public_chat_parts_readable, users_manage_message_parts, as, for, name, to, using, policies (+6 more)

### Community 598 - "Community 598"
Cohesion: 0.14
Nodes (14): public_chats_readable, users_manage_own_chats, policies, as, for, name, to, using (+6 more)

### Community 599 - "Community 599"
Cohesion: 0.14
Nodes (14): public_chat_messages_readable, users_manage_chat_messages, as, for, name, to, using, policies (+6 more)

### Community 600 - "Community 600"
Cohesion: 0.14
Nodes (14): public_chat_parts_readable, users_manage_message_parts, as, for, name, to, using, policies (+6 more)

### Community 601 - "Community 601"
Cohesion: 0.14
Nodes (14): public_chats_readable, users_manage_own_chats, policies, as, for, name, to, using (+6 more)

### Community 602 - "Community 602"
Cohesion: 0.14
Nodes (14): public_chat_messages_readable, users_manage_chat_messages, as, for, name, to, using, policies (+6 more)

### Community 603 - "Community 603"
Cohesion: 0.14
Nodes (14): public_chat_parts_readable, users_manage_message_parts, as, for, name, to, using, policies (+6 more)

### Community 604 - "Community 604"
Cohesion: 0.14
Nodes (14): public_chats_readable, users_manage_own_chats, policies, as, for, name, to, using (+6 more)

### Community 605 - "Community 605"
Cohesion: 0.14
Nodes (14): public_chat_messages_readable, users_manage_chat_messages, as, for, name, to, using, policies (+6 more)

### Community 606 - "Community 606"
Cohesion: 0.14
Nodes (14): public_chat_parts_readable, users_manage_message_parts, as, for, name, to, using, policies (+6 more)

### Community 607 - "Community 607"
Cohesion: 0.14
Nodes (14): public_chats_readable, users_manage_own_chats, policies, as, for, name, to, using (+6 more)

### Community 608 - "Community 608"
Cohesion: 0.14
Nodes (14): public_chat_messages_readable, users_manage_chat_messages, as, for, name, to, using, policies (+6 more)

### Community 609 - "Community 609"
Cohesion: 0.14
Nodes (14): public_chats_readable, users_manage_own_chats, policies, as, for, name, to, using (+6 more)

### Community 610 - "Community 610"
Cohesion: 0.14
Nodes (14): public_chat_messages_readable, users_manage_chat_messages, as, for, name, to, using, policies (+6 more)

### Community 611 - "Community 611"
Cohesion: 0.14
Nodes (14): public_chats_readable, users_manage_own_chats, policies, as, for, name, to, using (+6 more)

### Community 612 - "Community 612"
Cohesion: 0.14
Nodes (13): dialect, id, users_manage_own_chats, prevId, compositePrimaryKeys, foreignKeys, name, policies (+5 more)

### Community 613 - "Community 613"
Cohesion: 0.14
Nodes (13): dialect, id, users_manage_own_chats, prevId, compositePrimaryKeys, foreignKeys, name, policies (+5 more)

### Community 614 - "Community 614"
Cohesion: 0.14
Nodes (13): dialect, id, users_manage_own_chats, prevId, compositePrimaryKeys, foreignKeys, name, policies (+5 more)

### Community 615 - "Community 615"
Cohesion: 0.14
Nodes (13): dialect, id, users_manage_own_chats, prevId, compositePrimaryKeys, foreignKeys, name, policies (+5 more)

### Community 616 - "Community 616"
Cohesion: 0.14
Nodes (13): dialect, id, users_manage_own_chats, prevId, compositePrimaryKeys, foreignKeys, name, policies (+5 more)

### Community 617 - "Community 617"
Cohesion: 0.14
Nodes (13): dialect, id, users_manage_own_chats, prevId, compositePrimaryKeys, foreignKeys, name, policies (+5 more)

### Community 618 - "Community 618"
Cohesion: 0.14
Nodes (13): dialect, id, users_manage_own_chats, prevId, compositePrimaryKeys, foreignKeys, name, policies (+5 more)

### Community 619 - "Community 619"
Cohesion: 0.14
Nodes (13): dialect, id, users_manage_own_chats, prevId, compositePrimaryKeys, foreignKeys, name, policies (+5 more)

### Community 620 - "Community 620"
Cohesion: 0.14
Nodes (13): dialect, id, users_manage_own_chats, prevId, compositePrimaryKeys, foreignKeys, name, policies (+5 more)

### Community 621 - "Community 621"
Cohesion: 0.14
Nodes (13): dialect, id, users_manage_own_chats, prevId, compositePrimaryKeys, foreignKeys, name, policies (+5 more)

### Community 622 - "Community 622"
Cohesion: 0.14
Nodes (13): dialect, id, users_manage_own_chats, prevId, compositePrimaryKeys, foreignKeys, name, policies (+5 more)

### Community 623 - "Community 623"
Cohesion: 0.14
Nodes (13): dialect, id, users_manage_own_chats, prevId, compositePrimaryKeys, foreignKeys, name, policies (+5 more)

### Community 624 - "Community 624"
Cohesion: 0.14
Nodes (13): dialect, id, users_manage_own_chats, prevId, compositePrimaryKeys, foreignKeys, name, policies (+5 more)

### Community 625 - "Community 625"
Cohesion: 0.14
Nodes (13): dialect, id, users_manage_own_chats, prevId, compositePrimaryKeys, foreignKeys, name, policies (+5 more)

### Community 626 - "Community 626"
Cohesion: 0.14
Nodes (13): dialect, id, users_manage_own_chats, prevId, compositePrimaryKeys, foreignKeys, name, policies (+5 more)

### Community 627 - "Community 627"
Cohesion: 0.15
Nodes (10): Callout, Files, Quick check, Citation, Files, Quick check, Generative UI Components Catalog, Files (+2 more)

### Community 628 - "Community 628"
Cohesion: 0.20
Nodes (4): SPECIFIER_TO_CHUNK, VENDOR_CHUNKS, VENDOR_JS_MAP, VendorChunk

### Community 629 - "Community 629"
Cohesion: 0.14
Nodes (14): eval_summaries Table, PHOENIX UNAVAILABLE vs DB WRITE FAILED Labels, Judge Model (gemini-flash-lite), Railway Redeploy Manual-Run Caveat, polymorph-evals Pipeline, Chat Agent Tool Roster, correct_tool Label, Custom > Generic Rubric Principle (+6 more)

### Community 630 - "Community 630"
Cohesion: 0.15
Nodes (12): CRON_SECRET, Healthcheck expectations, Production Deployment, Production minimum requirements, Recommended targets, Rollback strategy, Staging checklist, Trending Suggestions Refresh Cron (+4 more)

### Community 631 - "Community 631"
Cohesion: 0.15
Nodes (13): as, for, name, to, withCheck, as, for, name (+5 more)

### Community 632 - "Community 632"
Cohesion: 0.15
Nodes (13): id, message_id, order, name, notNull, primaryKey, type, name (+5 more)

### Community 633 - "Community 633"
Cohesion: 0.11
Nodes (16): Deployment Guide, Enabling tracing on Vercel, Evals cron service, Healthcheck expectations, Observability (Phoenix on Railway), Persistence verification (run after every Phoenix deploy), Production minimum requirements, Recommended targets (+8 more)

### Community 634 - "Community 634"
Cohesion: 0.17
Nodes (10): createProviderRegistry (lib/utils/registry.ts), Vercel AI Gateway Provider, getModel(modelString), Adding Provider Options, AI Provider Registry, How to Change Models, Provider Registry, Testing (+2 more)

### Community 635 - "Community 635"
Cohesion: 0.17
Nodes (11): AgentArtifact (`components/tool-ui/agent-artifact/`), Callout (`components/tool-ui/callout/`), Chart (`components/tool-ui/chart/`), CitationList (`components/tool-ui/citation/`), DataTable (`components/tool-ui/data-table/`), Display Tool Components, Generative UI Components, LinkPreview (`components/tool-ui/link-preview/`) (+3 more)

### Community 636 - "Community 636"
Cohesion: 0.17
Nodes (11): Error Handling, Key Files, Mermaid Diagram, Overview, Parallel Operations, Smooth Streaming, Stream Lifecycle, Streaming Architecture (+3 more)

### Community 637 - "Community 637"
Cohesion: 0.20
Nodes (9): AI Provider Options, Baseline Bootstrap, Cloud Deployment Controls, Configuration Guide, Geo and Map Tools, Optional Features, Optional secondary/fallback providers, Primary search provider (recommended) (+1 more)

### Community 638 - "Community 638"
Cohesion: 0.17
Nodes (11): AI provider options (Direct), Canvas artifacts, Environment Provider Variables, GUEST_CANVAS_SECRET, Map tiles (geo-map Tool UI), MapTiler Keys (Client + Server), Optional platform features, Search provider options (+3 more)

### Community 639 - "Community 639"
Cohesion: 0.17
Nodes (12): Advanced Usage, chat-cli.ts, Features, Message Regeneration, Model Types, Options, Output Format, Requirements (+4 more)

### Community 640 - "Community 640"
Cohesion: 0.18
Nodes (12): AllowedPackage, CANVAS_ALLOWED_PACKAGE_IMPORTS_LIST, CANVAS_ALLOWED_PACKAGES, exactSet, isAllowedCanvasImport(), prefixEntries, makeArtifactRow(), VENDOR_CHUNK_DEFS as single source of truth for allowed packages (+4 more)

### Community 641 - "Community 641"
Cohesion: 0.18
Nodes (10): Authenticated Rate Limits, Error Response Conventions, Error Responses, Evals API and Rate Limits, Guest Rate Limits, Non-Cloud Deployments, POST `/api/evals/run`, Rate Limiting (+2 more)

### Community 642 - "Community 642"
Cohesion: 0.20
Nodes (9): Config File Structure, Configuration Profiles, Default Models, How to Add a New Provider, How to Change Models, Model Configuration, Model Selection Algorithm, Overview (+1 more)

### Community 643 - "Community 643"
Cohesion: 0.20
Nodes (9): Brave, Exa, Firecrawl, Provider Details, Provider Selection Logic, Research Agent Search Providers, Search Providers, SearXNG (+1 more)

### Community 644 - "Community 644"
Cohesion: 0.20
Nodes (9): Brave (Default), Configuring Providers, Exa, Search Provider Setup, SearXNG, SearXNG Advanced Search Mode, Tavily, SearXNGSearchProvider (opt-in) (+1 more)

### Community 645 - "Community 645"
Cohesion: 0.20
Nodes (9): Adding a New Provider, Configuring Providers, Error Handling and Retries, Provider Comparison, Provider Selection Logic, Search in Chat vs Research Modes, Search Providers, Search Tool Schema (+1 more)

### Community 646 - "Community 646"
Cohesion: 0.20
Nodes (10): checkConstraints, compositePrimaryKeys, foreignKeys, indexes, isRLSEnabled, name, policies, schema (+2 more)

### Community 647 - "Community 647"
Cohesion: 0.20
Nodes (10): checkConstraints, compositePrimaryKeys, foreignKeys, indexes, isRLSEnabled, name, policies, schema (+2 more)

### Community 648 - "Community 648"
Cohesion: 0.20
Nodes (10): checkConstraints, compositePrimaryKeys, foreignKeys, indexes, isRLSEnabled, name, policies, schema (+2 more)

### Community 649 - "Community 649"
Cohesion: 0.20
Nodes (9): Authentication, Chat API, Cookies Read, Error Responses, Example, POST `/api/chat`, Request Body, Request Flow (+1 more)

### Community 650 - "Community 650"
Cohesion: 0.20
Nodes (9): Core API Endpoints, GET `/api/chats`, GET `/api/health`, GET `/api/suggestions`, GET `/api/suggestions/refresh`, POST `/api/advanced-search`, POST `/api/feedback`, POST `/api/upload` (+1 more)

### Community 651 - "Community 651"
Cohesion: 0.22
Nodes (8): 1) Authentication and Backend, 2) Search and Content Extraction, 3) AI Model Orchestration, 4) Storage Strategy, 5) Deployment Target, 6) Observability, 7) Scheduled Jobs, Architecture Decisions

### Community 652 - "Community 652"
Cohesion: 0.22
Nodes (8): Canvas / Inspector, Display tools (server), Generative UI Key Files, Key File Reference, Rendering pipeline, Tool UI components (client), Tool UI manifest runtime, Types

### Community 653 - "Community 653"
Cohesion: 0.22
Nodes (8): `competitorResearch` — Live competitor specialist, Core Tools, `fetch` — Web content extraction, Research Agent Core Tools, `search` — Multi-provider web search, Spatial tools — geocoding, routing, isochrones, and static maps, `todoWrite` — Research task tracking, Tool System

### Community 654 - "Community 654"
Cohesion: 0.22
Nodes (8): Context Window Management, Model Context Windows, Processing Stages, Research Agent Models and Context, Token Counting, Truncation Strategy, Token Counting (js-tiktoken cl100k_base), Message Truncation Strategy

### Community 655 - "Community 655"
Cohesion: 0.46
Nodes (4): trackChatEvent(), AnalyticsProvider, ChatEventData, calculateConversationTurn()

### Community 656 - "Community 656"
Cohesion: 0.22
Nodes (9): messages_chat_id_chats_id_fk, columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo (+1 more)

### Community 657 - "Community 657"
Cohesion: 0.22
Nodes (9): parts_message_id_messages_id_fk, columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo (+1 more)

### Community 658 - "Community 658"
Cohesion: 0.12
Nodes (16): parts_message_id_messages_id_fk, columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo (+8 more)

### Community 659 - "Community 659"
Cohesion: 0.12
Nodes (16): parts_message_id_messages_id_fk, columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo (+8 more)

### Community 660 - "Community 660"
Cohesion: 0.22
Nodes (9): parts_message_id_messages_id_fk, columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo (+1 more)

### Community 661 - "Community 661"
Cohesion: 0.22
Nodes (8): dialect, enums, id, prevId, roles, schemas, sequences, version

### Community 662 - "Community 662"
Cohesion: 0.22
Nodes (9): parts_message_id_messages_id_fk, columnsFrom, columnsTo, name, onDelete, onUpdate, tableFrom, tableTo (+1 more)

### Community 663 - "Community 663"
Cohesion: 0.21
Nodes (6): createAdvancedSearchResult(), advancedSearchMocks, advancedSearchMocks, collectAdvancedSearchChunks(), collectSearchChunks(), providerSearchMocks

### Community 664 - "Community 664"
Cohesion: 0.29
Nodes (7): Agent Artifact _adapter.tsx, Agent Artifact Community Port, Agent Artifact Non-Commercial Upstream License, Npm-First Source Boundary, Adding Generative UI Tools Checklist, community-sources.ts Porting Record, AI SDK v6 + Tool UI Manifest Contract

### Community 665 - "Community 665"
Cohesion: 0.25
Nodes (8): 1. Install the SDK, 2. Register the Provider, 3. Add Enablement Check, 4. Configure a Model, 5. Set Environment Variable, 6. Update Environment Documentation, 7. Test, How to Add a New Provider

### Community 666 - "Community 666"
Cohesion: 0.25
Nodes (7): Configuration, `displayGeoMap` Contract, Geo & Spatial Tools, Key Files, Overview, Static vs Interactive Output, Tool Inventory

### Community 667 - "Community 667"
Cohesion: 0.25
Nodes (7): Default Models, Example scenarios, Full resolution order, Inputs, Model Selection, Model Selection Algorithm, Step-by-step process

### Community 668 - "Community 668"
Cohesion: 0.25
Nodes (7): Adding a New AI Provider, Adding a New Core Tool, Adding a New Display Tool, Adding a New Search Provider, Extending the Agent, Modifying System Prompts, Research Agent Extension

### Community 669 - "Community 669"
Cohesion: 0.25
Nodes (7): 1. Client Sends Message, 2. API Route Receives Request, 3. Model Selection, 4. Stream Path Dispatch, 5. Authorization and Chat Loading (Authenticated Only), 6. Stream Creation with `createUIMessageStream`, Streaming Request Dispatch

### Community 670 - "Community 670"
Cohesion: 0.39
Nodes (7): assembleCanvasHtml(), AssembleCanvasHtmlOptions, buildAssetScript(), buildBootstrapScript(), escapeHtml(), escapeStyleContent(), CanvasMetaJson

### Community 672 - "Community 672"
Cohesion: 0.46
Nodes (7): createEmojiIcon(), createImageIcon(), escapeHtml(), isSafeHttpUrl(), LeafletIconRuntime, resolveMarkerIcon(), GeoMapMarker

### Community 673 - "Community 673"
Cohesion: 0.25
Nodes (8): checkConstraints, compositePrimaryKeys, foreignKeys, isRLSEnabled, name, schema, uniqueConstraints, public.chats

### Community 674 - "Community 674"
Cohesion: 0.25
Nodes (8): checkConstraints, compositePrimaryKeys, foreignKeys, isRLSEnabled, name, schema, uniqueConstraints, public.chats

### Community 675 - "Community 675"
Cohesion: 0.25
Nodes (8): checkConstraints, compositePrimaryKeys, foreignKeys, isRLSEnabled, name, schema, uniqueConstraints, public.feedback

### Community 676 - "Community 676"
Cohesion: 0.25
Nodes (7): Architecture, Enabling tracing on Vercel, Observability (Phoenix on Railway), Persistence verification (run after every Phoenix deploy), Phoenix Operations, Phoenix service, Rotating Phoenix API keys

### Community 677 - "Community 677"
Cohesion: 0.25
Nodes (7): Build Issues, Database Issues, Getting More Help, Runtime Issues, Search Issues, Setup Issues, Troubleshooting

### Community 678 - "Community 678"
Cohesion: 0.25
Nodes (6): Files, Option List, Quick check, Files, Progress Tracker, Quick check

### Community 679 - "Community 679"
Cohesion: 0.29
Nodes (7): columns, concurrently, isUnique, method, name, with, chats_user_id_idx

### Community 680 - "Community 680"
Cohesion: 0.29
Nodes (6): Adding a New Display Tool, Adding Generative UI Tools, How to Add a New Generative UI Tool, License-Aware Community Porting Record, Npm Upgrade Workflow, Repeatability Acceptance Criteria

### Community 681 - "Community 681"
Cohesion: 0.29
Nodes (6): Config File Structure, Configuration Profiles, Dimensions, Model Configuration Structure, Model Object, Overview

### Community 682 - "Community 682"
Cohesion: 0.29
Nodes (6): Architecture, Getting Started, Operations, Polymorph Documentation, Reference, Where to Start

### Community 683 - "Community 683"
Cohesion: 0.29
Nodes (7): columns, concurrently, isUnique, method, name, with, chats_user_id_idx

### Community 685 - "Community 685"
Cohesion: 0.33
Nodes (6): created_at, default, name, notNull, primaryKey, type

### Community 686 - "Community 686"
Cohesion: 0.29
Nodes (7): columns, concurrently, isUnique, method, name, with, chats_user_id_idx

### Community 687 - "Community 687"
Cohesion: 0.40
Nodes (4): mockAssembleCanvasHtml, mockBuildTailwindCss, mockEsbuildBuild, mockValidateCanvasSource

### Community 688 - "Community 688"
Cohesion: 0.29
Nodes (7): columns, concurrently, isUnique, method, name, with, chats_user_id_created_at_idx

### Community 689 - "Community 689"
Cohesion: 0.29
Nodes (7): columns, concurrently, isUnique, method, name, with, chats_user_id_idx

### Community 690 - "Community 690"
Cohesion: 0.29
Nodes (7): columns, concurrently, isUnique, method, name, with, chats_user_id_created_at_idx

### Community 691 - "Community 691"
Cohesion: 0.29
Nodes (7): columns, concurrently, isUnique, method, name, with, chats_user_id_idx

### Community 692 - "Community 692"
Cohesion: 0.29
Nodes (6): 401 Unauthorized on `/api/chat`, 403 Forbidden on Share Page, `getUser` Timeout, Rate Limit Exceeded, Runtime Issues, Troubleshooting Runtime Issues

### Community 693 - "Community 693"
Cohesion: 0.29
Nodes (6): `DATABASE_SSL_DISABLED` Not Set, Missing `.env.local`, Port Conflicts, "Provider not enabled" Error, Setup Issues, Troubleshooting Setup Issues

### Community 694 - "Community 694"
Cohesion: 0.33
Nodes (5): Architecture Models and Tool State, Default model configuration, Model Selection, Tool State Lifecycle, UI rendering per state

### Community 695 - "Community 695"
Cohesion: 0.33
Nodes (5): Configuration, How It Works, Invocation, Research Agent Tool Loop, The ToolLoopAgent Pattern

### Community 696 - "Community 696"
Cohesion: 0.40
Nodes (5): tool_error_text, name, notNull, primaryKey, type

### Community 697 - "Community 697"
Cohesion: 0.33
Nodes (5): Architecture, Attribution, Documentation, Evaluation, Features

### Community 698 - "Community 698"
Cohesion: 0.33
Nodes (5): Canvas Asset API Endpoints, Error Responses, GET `/api/canvas-assets/image-proxy`, Query Parameters, Response

### Community 699 - "Community 699"
Cohesion: 0.33
Nodes (5): cSpell.words, editor.codeActionsOnSave, source.organizeImports, editor.defaultFormatter, editor.formatOnSave

### Community 700 - "Community 700"
Cohesion: 0.40
Nodes (4): End-to-End Rendering Pipeline, Generative UI Overview, Overview, State transitions during rendering

### Community 701 - "Community 701"
Cohesion: 0.40
Nodes (4): Action system, Community-portability evidence, Generative UI Portability and Actions, Shared base fields

### Community 702 - "Community 702"
Cohesion: 0.40
Nodes (4): Architecture Database Schema, Database Schema, Indexes, Schema details

### Community 703 - "Community 703"
Cohesion: 0.40
Nodes (5): data_content, name, notNull, primaryKey, type

### Community 704 - "Community 704"
Cohesion: 0.40
Nodes (5): data_id, name, notNull, primaryKey, type

### Community 705 - "Community 705"
Cohesion: 0.40
Nodes (5): file_filename, name, notNull, primaryKey, type

### Community 706 - "Community 706"
Cohesion: 0.40
Nodes (5): file_media_type, name, notNull, primaryKey, type

### Community 707 - "Community 707"
Cohesion: 0.40
Nodes (5): file_url, name, notNull, primaryKey, type

### Community 708 - "Community 708"
Cohesion: 0.40
Nodes (5): message_id, name, notNull, primaryKey, type

### Community 709 - "Community 709"
Cohesion: 0.40
Nodes (5): order, name, notNull, primaryKey, type

### Community 710 - "Community 710"
Cohesion: 0.40
Nodes (5): provider_metadata, name, notNull, primaryKey, type

### Community 711 - "Community 711"
Cohesion: 0.40
Nodes (5): source_document_filename, name, notNull, primaryKey, type

### Community 712 - "Community 712"
Cohesion: 0.40
Nodes (5): source_document_media_type, name, notNull, primaryKey, type

### Community 713 - "Community 713"
Cohesion: 0.40
Nodes (5): source_document_snippet, name, notNull, primaryKey, type

### Community 714 - "Community 714"
Cohesion: 0.40
Nodes (5): source_document_source_id, name, notNull, primaryKey, type

### Community 715 - "Community 715"
Cohesion: 0.40
Nodes (5): source_document_url, name, notNull, primaryKey, type

### Community 716 - "Community 716"
Cohesion: 0.40
Nodes (5): source_url_source_id, name, notNull, primaryKey, type

### Community 717 - "Community 717"
Cohesion: 0.40
Nodes (5): source_url_title, name, notNull, primaryKey, type

### Community 718 - "Community 718"
Cohesion: 0.40
Nodes (5): source_url_url, name, notNull, primaryKey, type

### Community 719 - "Community 719"
Cohesion: 0.40
Nodes (5): text_text, name, notNull, primaryKey, type

### Community 720 - "Community 720"
Cohesion: 0.40
Nodes (5): tool_dynamic_input, name, notNull, primaryKey, type

### Community 721 - "Community 721"
Cohesion: 0.40
Nodes (5): data_prefix, name, notNull, primaryKey, type

### Community 722 - "Community 722"
Cohesion: 0.40
Nodes (5): tool_dynamic_output, name, notNull, primaryKey, type

### Community 723 - "Community 723"
Cohesion: 0.40
Nodes (5): tool_dynamic_type, name, notNull, primaryKey, type

### Community 724 - "Community 724"
Cohesion: 0.40
Nodes (5): tool_fetch_input, name, notNull, primaryKey, type

### Community 725 - "Community 725"
Cohesion: 0.40
Nodes (5): tool_fetch_output, name, notNull, primaryKey, type

### Community 726 - "Community 726"
Cohesion: 0.40
Nodes (5): tool_question_input, name, notNull, primaryKey, type

### Community 727 - "Community 727"
Cohesion: 0.40
Nodes (5): tool_question_output, name, notNull, primaryKey, type

### Community 728 - "Community 728"
Cohesion: 0.40
Nodes (5): tool_search_input, name, notNull, primaryKey, type

### Community 729 - "Community 729"
Cohesion: 0.40
Nodes (5): tool_search_output, name, notNull, primaryKey, type

### Community 730 - "Community 730"
Cohesion: 0.40
Nodes (5): tool_state, name, notNull, primaryKey, type

### Community 731 - "Community 731"
Cohesion: 0.40
Nodes (5): tool_todoRead_input, name, notNull, primaryKey, type

### Community 732 - "Community 732"
Cohesion: 0.40
Nodes (5): tool_todoRead_output, name, notNull, primaryKey, type

### Community 733 - "Community 733"
Cohesion: 0.40
Nodes (5): tool_todoWrite_input, name, notNull, primaryKey, type

### Community 734 - "Community 734"
Cohesion: 0.40
Nodes (5): tool_todoWrite_output, name, notNull, primaryKey, type

### Community 735 - "Community 735"
Cohesion: 0.40
Nodes (5): tool_tool_call_id, name, notNull, primaryKey, type

### Community 736 - "Community 736"
Cohesion: 0.40
Nodes (5): type, name, notNull, primaryKey, type

### Community 737 - "Community 737"
Cohesion: 0.40
Nodes (5): order, name, notNull, primaryKey, type

### Community 738 - "Community 738"
Cohesion: 0.40
Nodes (5): file_filename, name, notNull, primaryKey, type

### Community 739 - "Community 739"
Cohesion: 0.40
Nodes (5): file_media_type, name, notNull, primaryKey, type

### Community 740 - "Community 740"
Cohesion: 0.40
Nodes (5): file_url, name, notNull, primaryKey, type

### Community 741 - "Community 741"
Cohesion: 0.40
Nodes (5): message_id, name, notNull, primaryKey, type

### Community 742 - "Community 742"
Cohesion: 0.40
Nodes (5): provider_metadata, name, notNull, primaryKey, type

### Community 743 - "Community 743"
Cohesion: 0.40
Nodes (5): tool_dynamic_type, name, notNull, primaryKey, type

### Community 744 - "Community 744"
Cohesion: 0.40
Nodes (5): source_document_filename, name, notNull, primaryKey, type

### Community 745 - "Community 745"
Cohesion: 0.40
Nodes (5): source_document_media_type, name, notNull, primaryKey, type

### Community 746 - "Community 746"
Cohesion: 0.40
Nodes (5): source_document_snippet, name, notNull, primaryKey, type

### Community 747 - "Community 747"
Cohesion: 0.40
Nodes (5): source_document_source_id, name, notNull, primaryKey, type

### Community 748 - "Community 748"
Cohesion: 0.40
Nodes (5): source_document_title, name, notNull, primaryKey, type

### Community 749 - "Community 749"
Cohesion: 0.40
Nodes (5): source_document_url, name, notNull, primaryKey, type

### Community 750 - "Community 750"
Cohesion: 0.40
Nodes (5): source_url_source_id, name, notNull, primaryKey, type

### Community 751 - "Community 751"
Cohesion: 0.40
Nodes (5): source_url_title, name, notNull, primaryKey, type

### Community 752 - "Community 752"
Cohesion: 0.40
Nodes (5): source_url_url, name, notNull, primaryKey, type

### Community 753 - "Community 753"
Cohesion: 0.40
Nodes (5): text_text, name, notNull, primaryKey, type

### Community 754 - "Community 754"
Cohesion: 0.40
Nodes (5): text_text, name, notNull, primaryKey, type

### Community 755 - "Community 755"
Cohesion: 0.40
Nodes (5): tool_dynamic_output, name, notNull, primaryKey, type

### Community 756 - "Community 756"
Cohesion: 0.40
Nodes (5): tool_error_text, name, notNull, primaryKey, type

### Community 757 - "Community 757"
Cohesion: 0.40
Nodes (5): tool_fetch_input, name, notNull, primaryKey, type

### Community 758 - "Community 758"
Cohesion: 0.40
Nodes (5): tool_fetch_output, name, notNull, primaryKey, type

### Community 759 - "Community 759"
Cohesion: 0.40
Nodes (5): tool_question_input, name, notNull, primaryKey, type

### Community 760 - "Community 760"
Cohesion: 0.40
Nodes (5): tool_question_output, name, notNull, primaryKey, type

### Community 761 - "Community 761"
Cohesion: 0.40
Nodes (5): tool_search_input, name, notNull, primaryKey, type

### Community 762 - "Community 762"
Cohesion: 0.40
Nodes (5): tool_search_output, name, notNull, primaryKey, type

### Community 763 - "Community 763"
Cohesion: 0.40
Nodes (5): tool_state, name, notNull, primaryKey, type

### Community 764 - "Community 764"
Cohesion: 0.40
Nodes (5): tool_todoRead_input, name, notNull, primaryKey, type

### Community 765 - "Community 765"
Cohesion: 0.40
Nodes (5): tool_todoRead_output, name, notNull, primaryKey, type

### Community 766 - "Community 766"
Cohesion: 0.40
Nodes (5): tool_todoWrite_input, name, notNull, primaryKey, type

### Community 767 - "Community 767"
Cohesion: 0.40
Nodes (5): tool_todoWrite_output, name, notNull, primaryKey, type

### Community 768 - "Community 768"
Cohesion: 0.40
Nodes (5): tool_tool_call_id, name, notNull, primaryKey, type

### Community 769 - "Community 769"
Cohesion: 0.40
Nodes (5): type, name, notNull, primaryKey, type

### Community 770 - "Community 770"
Cohesion: 0.40
Nodes (5): data_id, name, notNull, primaryKey, type

### Community 771 - "Community 771"
Cohesion: 0.40
Nodes (5): file_filename, name, notNull, primaryKey, type

### Community 772 - "Community 772"
Cohesion: 0.40
Nodes (5): file_media_type, name, notNull, primaryKey, type

### Community 773 - "Community 773"
Cohesion: 0.40
Nodes (5): file_url, name, notNull, primaryKey, type

### Community 774 - "Community 774"
Cohesion: 0.40
Nodes (5): message_id, name, notNull, primaryKey, type

### Community 775 - "Community 775"
Cohesion: 0.40
Nodes (5): provider_metadata, name, notNull, primaryKey, type

### Community 776 - "Community 776"
Cohesion: 0.40
Nodes (5): reasoning_text, name, notNull, primaryKey, type

### Community 777 - "Community 777"
Cohesion: 0.40
Nodes (5): source_document_filename, name, notNull, primaryKey, type

### Community 778 - "Community 778"
Cohesion: 0.40
Nodes (5): source_document_media_type, name, notNull, primaryKey, type

### Community 779 - "Community 779"
Cohesion: 0.40
Nodes (5): source_document_snippet, name, notNull, primaryKey, type

### Community 780 - "Community 780"
Cohesion: 0.40
Nodes (5): source_document_source_id, name, notNull, primaryKey, type

### Community 781 - "Community 781"
Cohesion: 0.40
Nodes (5): source_document_title, name, notNull, primaryKey, type

### Community 782 - "Community 782"
Cohesion: 0.40
Nodes (5): source_document_url, name, notNull, primaryKey, type

### Community 783 - "Community 783"
Cohesion: 0.40
Nodes (5): source_url_source_id, name, notNull, primaryKey, type

### Community 784 - "Community 784"
Cohesion: 0.40
Nodes (5): source_url_title, name, notNull, primaryKey, type

### Community 785 - "Community 785"
Cohesion: 0.40
Nodes (5): source_url_url, name, notNull, primaryKey, type

### Community 786 - "Community 786"
Cohesion: 0.40
Nodes (5): text_text, name, notNull, primaryKey, type

### Community 787 - "Community 787"
Cohesion: 0.40
Nodes (5): tool_dynamic_input, name, notNull, primaryKey, type

### Community 788 - "Community 788"
Cohesion: 0.40
Nodes (5): tool_dynamic_output, name, notNull, primaryKey, type

### Community 789 - "Community 789"
Cohesion: 0.40
Nodes (5): tool_error_text, name, notNull, primaryKey, type

### Community 790 - "Community 790"
Cohesion: 0.40
Nodes (5): tool_fetch_input, name, notNull, primaryKey, type

### Community 791 - "Community 791"
Cohesion: 0.40
Nodes (5): tool_fetch_output, name, notNull, primaryKey, type

### Community 792 - "Community 792"
Cohesion: 0.40
Nodes (5): tool_question_input, name, notNull, primaryKey, type

### Community 793 - "Community 793"
Cohesion: 0.40
Nodes (5): tool_question_output, name, notNull, primaryKey, type

### Community 794 - "Community 794"
Cohesion: 0.40
Nodes (5): tool_search_input, name, notNull, primaryKey, type

### Community 795 - "Community 795"
Cohesion: 0.40
Nodes (5): tool_search_output, name, notNull, primaryKey, type

### Community 796 - "Community 796"
Cohesion: 0.40
Nodes (5): tool_state, name, notNull, primaryKey, type

### Community 797 - "Community 797"
Cohesion: 0.40
Nodes (5): tool_todoRead_input, name, notNull, primaryKey, type

### Community 798 - "Community 798"
Cohesion: 0.40
Nodes (5): tool_todoRead_output, name, notNull, primaryKey, type

### Community 799 - "Community 799"
Cohesion: 0.40
Nodes (5): tool_todoWrite_input, name, notNull, primaryKey, type

### Community 800 - "Community 800"
Cohesion: 0.40
Nodes (5): tool_todoWrite_output, name, notNull, primaryKey, type

### Community 801 - "Community 801"
Cohesion: 0.40
Nodes (5): tool_tool_call_id, name, notNull, primaryKey, type

### Community 802 - "Community 802"
Cohesion: 0.40
Nodes (5): type, name, notNull, primaryKey, type

### Community 803 - "Community 803"
Cohesion: 0.40
Nodes (5): data_content, name, notNull, primaryKey, type

### Community 804 - "Community 804"
Cohesion: 0.40
Nodes (5): data_id, name, notNull, primaryKey, type

### Community 805 - "Community 805"
Cohesion: 0.40
Nodes (5): data_prefix, name, notNull, primaryKey, type

### Community 806 - "Community 806"
Cohesion: 0.40
Nodes (5): file_filename, name, notNull, primaryKey, type

### Community 807 - "Community 807"
Cohesion: 0.40
Nodes (5): file_media_type, name, notNull, primaryKey, type

### Community 808 - "Community 808"
Cohesion: 0.40
Nodes (5): file_url, name, notNull, primaryKey, type

### Community 809 - "Community 809"
Cohesion: 0.40
Nodes (5): order, name, notNull, primaryKey, type

### Community 810 - "Community 810"
Cohesion: 0.40
Nodes (5): source_document_filename, name, notNull, primaryKey, type

### Community 811 - "Community 811"
Cohesion: 0.40
Nodes (5): source_document_media_type, name, notNull, primaryKey, type

### Community 812 - "Community 812"
Cohesion: 0.40
Nodes (5): source_document_snippet, name, notNull, primaryKey, type

### Community 813 - "Community 813"
Cohesion: 0.40
Nodes (5): source_document_source_id, name, notNull, primaryKey, type

### Community 814 - "Community 814"
Cohesion: 0.40
Nodes (5): source_document_title, name, notNull, primaryKey, type

### Community 815 - "Community 815"
Cohesion: 0.40
Nodes (5): source_document_url, name, notNull, primaryKey, type

### Community 816 - "Community 816"
Cohesion: 0.40
Nodes (5): source_url_title, name, notNull, primaryKey, type

### Community 817 - "Community 817"
Cohesion: 0.40
Nodes (5): source_url_url, name, notNull, primaryKey, type

### Community 818 - "Community 818"
Cohesion: 0.40
Nodes (5): text_text, name, notNull, primaryKey, type

### Community 819 - "Community 819"
Cohesion: 0.40
Nodes (5): tool_dynamic_name, name, notNull, primaryKey, type

### Community 820 - "Community 820"
Cohesion: 0.40
Nodes (5): tool_dynamic_output, name, notNull, primaryKey, type

### Community 821 - "Community 821"
Cohesion: 0.40
Nodes (5): tool_dynamic_type, name, notNull, primaryKey, type

### Community 822 - "Community 822"
Cohesion: 0.40
Nodes (5): tool_error_text, name, notNull, primaryKey, type

### Community 823 - "Community 823"
Cohesion: 0.40
Nodes (5): tool_fetch_input, name, notNull, primaryKey, type

### Community 824 - "Community 824"
Cohesion: 0.40
Nodes (5): tool_fetch_output, name, notNull, primaryKey, type

### Community 825 - "Community 825"
Cohesion: 0.40
Nodes (5): tool_question_input, name, notNull, primaryKey, type

### Community 826 - "Community 826"
Cohesion: 0.40
Nodes (5): tool_question_output, name, notNull, primaryKey, type

### Community 827 - "Community 827"
Cohesion: 0.40
Nodes (5): tool_search_input, name, notNull, primaryKey, type

### Community 828 - "Community 828"
Cohesion: 0.40
Nodes (5): tool_search_output, name, notNull, primaryKey, type

### Community 829 - "Community 829"
Cohesion: 0.40
Nodes (5): tool_state, name, notNull, primaryKey, type

### Community 830 - "Community 830"
Cohesion: 0.40
Nodes (5): tool_todoRead_input, name, notNull, primaryKey, type

### Community 831 - "Community 831"
Cohesion: 0.40
Nodes (5): tool_todoWrite_input, name, notNull, primaryKey, type

### Community 832 - "Community 832"
Cohesion: 0.40
Nodes (5): tool_todoWrite_output, name, notNull, primaryKey, type

### Community 833 - "Community 833"
Cohesion: 0.40
Nodes (5): tool_tool_call_id, name, notNull, primaryKey, type

### Community 834 - "Community 834"
Cohesion: 0.40
Nodes (5): type, name, notNull, primaryKey, type

### Community 835 - "Community 835"
Cohesion: 0.40
Nodes (5): data_id, name, notNull, primaryKey, type

### Community 836 - "Community 836"
Cohesion: 0.40
Nodes (5): data_prefix, name, notNull, primaryKey, type

### Community 837 - "Community 837"
Cohesion: 0.40
Nodes (5): file_filename, name, notNull, primaryKey, type

### Community 838 - "Community 838"
Cohesion: 0.40
Nodes (5): file_media_type, name, notNull, primaryKey, type

### Community 839 - "Community 839"
Cohesion: 0.40
Nodes (5): file_url, name, notNull, primaryKey, type

### Community 840 - "Community 840"
Cohesion: 0.40
Nodes (5): message_id, name, notNull, primaryKey, type

### Community 841 - "Community 841"
Cohesion: 0.40
Nodes (5): order, name, notNull, primaryKey, type

### Community 842 - "Community 842"
Cohesion: 0.40
Nodes (5): provider_metadata, name, notNull, primaryKey, type

### Community 843 - "Community 843"
Cohesion: 0.40
Nodes (5): reasoning_text, name, notNull, primaryKey, type

### Community 844 - "Community 844"
Cohesion: 0.40
Nodes (5): source_document_filename, name, notNull, primaryKey, type

### Community 845 - "Community 845"
Cohesion: 0.40
Nodes (5): source_document_snippet, name, notNull, primaryKey, type

### Community 846 - "Community 846"
Cohesion: 0.40
Nodes (5): source_document_source_id, name, notNull, primaryKey, type

### Community 847 - "Community 847"
Cohesion: 0.40
Nodes (5): source_document_title, name, notNull, primaryKey, type

### Community 848 - "Community 848"
Cohesion: 0.40
Nodes (5): source_document_url, name, notNull, primaryKey, type

### Community 849 - "Community 849"
Cohesion: 0.40
Nodes (5): source_url_source_id, name, notNull, primaryKey, type

### Community 850 - "Community 850"
Cohesion: 0.40
Nodes (5): tool_dynamic_input, name, notNull, primaryKey, type

### Community 851 - "Community 851"
Cohesion: 0.40
Nodes (5): tool_dynamic_name, name, notNull, primaryKey, type

### Community 852 - "Community 852"
Cohesion: 0.40
Nodes (5): message_id, name, notNull, primaryKey, type

### Community 853 - "Community 853"
Cohesion: 0.40
Nodes (5): tool_dynamic_type, name, notNull, primaryKey, type

### Community 854 - "Community 854"
Cohesion: 0.40
Nodes (5): tool_error_text, name, notNull, primaryKey, type

### Community 855 - "Community 855"
Cohesion: 0.40
Nodes (5): tool_fetch_input, name, notNull, primaryKey, type

### Community 856 - "Community 856"
Cohesion: 0.40
Nodes (5): tool_fetch_output, name, notNull, primaryKey, type

### Community 857 - "Community 857"
Cohesion: 0.40
Nodes (5): tool_question_input, name, notNull, primaryKey, type

### Community 858 - "Community 858"
Cohesion: 0.40
Nodes (5): tool_question_output, name, notNull, primaryKey, type

### Community 859 - "Community 859"
Cohesion: 0.40
Nodes (5): tool_search_input, name, notNull, primaryKey, type

### Community 860 - "Community 860"
Cohesion: 0.40
Nodes (5): tool_search_output, name, notNull, primaryKey, type

### Community 861 - "Community 861"
Cohesion: 0.40
Nodes (5): tool_todoRead_input, name, notNull, primaryKey, type

### Community 862 - "Community 862"
Cohesion: 0.40
Nodes (5): tool_todoRead_output, name, notNull, primaryKey, type

### Community 863 - "Community 863"
Cohesion: 0.40
Nodes (5): tool_todoWrite_input, name, notNull, primaryKey, type

### Community 864 - "Community 864"
Cohesion: 0.40
Nodes (5): tool_todoWrite_output, name, notNull, primaryKey, type

### Community 865 - "Community 865"
Cohesion: 0.40
Nodes (5): tool_tool_call_id, name, notNull, primaryKey, type

### Community 866 - "Community 866"
Cohesion: 0.40
Nodes (5): type, name, notNull, primaryKey, type

### Community 867 - "Community 867"
Cohesion: 0.40
Nodes (5): data_id, name, notNull, primaryKey, type

### Community 868 - "Community 868"
Cohesion: 0.40
Nodes (5): data_prefix, name, notNull, primaryKey, type

### Community 869 - "Community 869"
Cohesion: 0.40
Nodes (5): file_filename, name, notNull, primaryKey, type

### Community 870 - "Community 870"
Cohesion: 0.40
Nodes (5): file_media_type, name, notNull, primaryKey, type

### Community 871 - "Community 871"
Cohesion: 0.40
Nodes (5): file_url, name, notNull, primaryKey, type

### Community 872 - "Community 872"
Cohesion: 0.40
Nodes (5): message_id, name, notNull, primaryKey, type

### Community 873 - "Community 873"
Cohesion: 0.40
Nodes (5): provider_metadata, name, notNull, primaryKey, type

### Community 874 - "Community 874"
Cohesion: 0.40
Nodes (5): reasoning_text, name, notNull, primaryKey, type

### Community 875 - "Community 875"
Cohesion: 0.40
Nodes (5): source_document_media_type, name, notNull, primaryKey, type

### Community 876 - "Community 876"
Cohesion: 0.40
Nodes (5): source_document_snippet, name, notNull, primaryKey, type

### Community 877 - "Community 877"
Cohesion: 0.40
Nodes (5): source_document_source_id, name, notNull, primaryKey, type

### Community 878 - "Community 878"
Cohesion: 0.40
Nodes (5): source_document_title, name, notNull, primaryKey, type

### Community 879 - "Community 879"
Cohesion: 0.40
Nodes (5): source_document_url, name, notNull, primaryKey, type

### Community 880 - "Community 880"
Cohesion: 0.40
Nodes (5): source_url_title, name, notNull, primaryKey, type

### Community 881 - "Community 881"
Cohesion: 0.40
Nodes (5): source_url_url, name, notNull, primaryKey, type

### Community 882 - "Community 882"
Cohesion: 0.40
Nodes (5): text_text, name, notNull, primaryKey, type

### Community 883 - "Community 883"
Cohesion: 0.40
Nodes (5): tool_dynamic_input, name, notNull, primaryKey, type

### Community 884 - "Community 884"
Cohesion: 0.40
Nodes (5): tool_dynamic_name, name, notNull, primaryKey, type

### Community 885 - "Community 885"
Cohesion: 0.40
Nodes (5): tool_dynamic_output, name, notNull, primaryKey, type

### Community 886 - "Community 886"
Cohesion: 0.40
Nodes (5): tool_dynamic_type, name, notNull, primaryKey, type

### Community 887 - "Community 887"
Cohesion: 0.40
Nodes (5): tool_error_text, name, notNull, primaryKey, type

### Community 888 - "Community 888"
Cohesion: 0.40
Nodes (5): tool_fetch_input, name, notNull, primaryKey, type

### Community 889 - "Community 889"
Cohesion: 0.40
Nodes (5): tool_fetch_output, name, notNull, primaryKey, type

### Community 890 - "Community 890"
Cohesion: 0.40
Nodes (5): tool_question_output, name, notNull, primaryKey, type

### Community 891 - "Community 891"
Cohesion: 0.40
Nodes (5): tool_search_input, name, notNull, primaryKey, type

### Community 892 - "Community 892"
Cohesion: 0.40
Nodes (5): tool_state, name, notNull, primaryKey, type

### Community 893 - "Community 893"
Cohesion: 0.40
Nodes (5): tool_todoRead_input, name, notNull, primaryKey, type

### Community 894 - "Community 894"
Cohesion: 0.40
Nodes (5): tool_todoRead_output, name, notNull, primaryKey, type

### Community 895 - "Community 895"
Cohesion: 0.40
Nodes (5): tool_todoWrite_input, name, notNull, primaryKey, type

### Community 896 - "Community 896"
Cohesion: 0.40
Nodes (5): tool_todoWrite_output, name, notNull, primaryKey, type

### Community 897 - "Community 897"
Cohesion: 0.40
Nodes (5): tool_tool_call_id, name, notNull, primaryKey, type

### Community 898 - "Community 898"
Cohesion: 0.40
Nodes (5): type, name, notNull, primaryKey, type

### Community 899 - "Community 899"
Cohesion: 0.40
Nodes (5): data_content, name, notNull, primaryKey, type

### Community 900 - "Community 900"
Cohesion: 0.40
Nodes (5): data_id, name, notNull, primaryKey, type

### Community 901 - "Community 901"
Cohesion: 0.40
Nodes (5): data_prefix, name, notNull, primaryKey, type

### Community 902 - "Community 902"
Cohesion: 0.40
Nodes (5): file_filename, name, notNull, primaryKey, type

### Community 903 - "Community 903"
Cohesion: 0.40
Nodes (5): file_url, name, notNull, primaryKey, type

### Community 904 - "Community 904"
Cohesion: 0.40
Nodes (5): order, name, notNull, primaryKey, type

### Community 905 - "Community 905"
Cohesion: 0.40
Nodes (5): provider_metadata, name, notNull, primaryKey, type

### Community 906 - "Community 906"
Cohesion: 0.40
Nodes (5): source_document_filename, name, notNull, primaryKey, type

### Community 907 - "Community 907"
Cohesion: 0.40
Nodes (5): source_document_media_type, name, notNull, primaryKey, type

### Community 908 - "Community 908"
Cohesion: 0.40
Nodes (5): source_document_snippet, name, notNull, primaryKey, type

### Community 909 - "Community 909"
Cohesion: 0.40
Nodes (5): tool_fetch_input, name, notNull, primaryKey, type

### Community 910 - "Community 910"
Cohesion: 0.40
Nodes (5): source_document_title, name, notNull, primaryKey, type

### Community 911 - "Community 911"
Cohesion: 0.40
Nodes (5): source_document_url, name, notNull, primaryKey, type

### Community 912 - "Community 912"
Cohesion: 0.40
Nodes (5): source_url_source_id, name, notNull, primaryKey, type

### Community 913 - "Community 913"
Cohesion: 0.40
Nodes (5): source_url_title, name, notNull, primaryKey, type

### Community 914 - "Community 914"
Cohesion: 0.40
Nodes (5): source_url_url, name, notNull, primaryKey, type

### Community 915 - "Community 915"
Cohesion: 0.40
Nodes (5): text_text, name, notNull, primaryKey, type

### Community 916 - "Community 916"
Cohesion: 0.40
Nodes (5): tool_dynamic_input, name, notNull, primaryKey, type

### Community 917 - "Community 917"
Cohesion: 0.40
Nodes (5): tool_dynamic_name, name, notNull, primaryKey, type

### Community 918 - "Community 918"
Cohesion: 0.40
Nodes (5): tool_dynamic_output, name, notNull, primaryKey, type

### Community 919 - "Community 919"
Cohesion: 0.40
Nodes (5): tool_dynamic_type, name, notNull, primaryKey, type

### Community 920 - "Community 920"
Cohesion: 0.40
Nodes (5): tool_error_text, name, notNull, primaryKey, type

### Community 921 - "Community 921"
Cohesion: 0.40
Nodes (5): tool_fetch_input, name, notNull, primaryKey, type

### Community 922 - "Community 922"
Cohesion: 0.40
Nodes (5): tool_fetch_output, name, notNull, primaryKey, type

### Community 923 - "Community 923"
Cohesion: 0.40
Nodes (5): tool_question_input, name, notNull, primaryKey, type

### Community 924 - "Community 924"
Cohesion: 0.40
Nodes (5): tool_question_output, name, notNull, primaryKey, type

### Community 925 - "Community 925"
Cohesion: 0.40
Nodes (5): tool_search_input, name, notNull, primaryKey, type

### Community 926 - "Community 926"
Cohesion: 0.40
Nodes (5): tool_search_output, name, notNull, primaryKey, type

### Community 927 - "Community 927"
Cohesion: 0.40
Nodes (5): tool_state, name, notNull, primaryKey, type

### Community 928 - "Community 928"
Cohesion: 0.40
Nodes (5): tool_todoRead_input, name, notNull, primaryKey, type

### Community 929 - "Community 929"
Cohesion: 0.40
Nodes (5): tool_todoRead_output, name, notNull, primaryKey, type

### Community 930 - "Community 930"
Cohesion: 0.40
Nodes (5): tool_todoWrite_input, name, notNull, primaryKey, type

### Community 931 - "Community 931"
Cohesion: 0.40
Nodes (5): tool_todoWrite_output, name, notNull, primaryKey, type

### Community 932 - "Community 932"
Cohesion: 0.40
Nodes (5): tool_tool_call_id, name, notNull, primaryKey, type

### Community 933 - "Community 933"
Cohesion: 0.40
Nodes (5): type, name, notNull, primaryKey, type

### Community 934 - "Community 934"
Cohesion: 0.40
Nodes (5): data_id, name, notNull, primaryKey, type

### Community 935 - "Community 935"
Cohesion: 0.40
Nodes (5): data_prefix, name, notNull, primaryKey, type

### Community 936 - "Community 936"
Cohesion: 0.40
Nodes (5): file_filename, name, notNull, primaryKey, type

### Community 937 - "Community 937"
Cohesion: 0.12
Nodes (16): file_media_type, reasoning_text, source_document_url, name, notNull, primaryKey, type, columns (+8 more)

### Community 938 - "Community 938"
Cohesion: 0.40
Nodes (5): file_url, name, notNull, primaryKey, type

### Community 939 - "Community 939"
Cohesion: 0.40
Nodes (5): message_id, name, notNull, primaryKey, type

### Community 940 - "Community 940"
Cohesion: 0.40
Nodes (5): order, name, notNull, primaryKey, type

### Community 941 - "Community 941"
Cohesion: 0.40
Nodes (5): provider_metadata, name, notNull, primaryKey, type

### Community 942 - "Community 942"
Cohesion: 0.40
Nodes (5): tool_fetch_output, name, notNull, primaryKey, type

### Community 943 - "Community 943"
Cohesion: 0.40
Nodes (5): source_document_filename, name, notNull, primaryKey, type

### Community 944 - "Community 944"
Cohesion: 0.40
Nodes (5): source_document_media_type, name, notNull, primaryKey, type

### Community 945 - "Community 945"
Cohesion: 0.40
Nodes (5): source_document_snippet, name, notNull, primaryKey, type

### Community 946 - "Community 946"
Cohesion: 0.40
Nodes (5): source_document_source_id, name, notNull, primaryKey, type

### Community 947 - "Community 947"
Cohesion: 0.40
Nodes (5): source_document_title, name, notNull, primaryKey, type

### Community 948 - "Community 948"
Cohesion: 0.40
Nodes (5): tool_todoRead_output, name, notNull, primaryKey, type

### Community 949 - "Community 949"
Cohesion: 0.40
Nodes (5): source_url_source_id, name, notNull, primaryKey, type

### Community 950 - "Community 950"
Cohesion: 0.40
Nodes (5): source_url_title, name, notNull, primaryKey, type

### Community 951 - "Community 951"
Cohesion: 0.40
Nodes (5): source_url_url, name, notNull, primaryKey, type

### Community 952 - "Community 952"
Cohesion: 0.40
Nodes (5): text_text, name, notNull, primaryKey, type

### Community 953 - "Community 953"
Cohesion: 0.40
Nodes (5): tool_dynamic_input, name, notNull, primaryKey, type

### Community 954 - "Community 954"
Cohesion: 0.40
Nodes (5): tool_dynamic_name, name, notNull, primaryKey, type

### Community 955 - "Community 955"
Cohesion: 0.40
Nodes (5): tool_dynamic_output, name, notNull, primaryKey, type

### Community 956 - "Community 956"
Cohesion: 0.40
Nodes (5): tool_dynamic_type, name, notNull, primaryKey, type

### Community 957 - "Community 957"
Cohesion: 0.40
Nodes (5): tool_error_text, name, notNull, primaryKey, type

### Community 958 - "Community 958"
Cohesion: 0.40
Nodes (5): tool_question_input, name, notNull, primaryKey, type

### Community 959 - "Community 959"
Cohesion: 0.40
Nodes (5): tool_question_output, name, notNull, primaryKey, type

### Community 960 - "Community 960"
Cohesion: 0.40
Nodes (5): tool_search_input, name, notNull, primaryKey, type

### Community 961 - "Community 961"
Cohesion: 0.40
Nodes (5): tool_search_output, name, notNull, primaryKey, type

### Community 962 - "Community 962"
Cohesion: 0.40
Nodes (5): tool_state, name, notNull, primaryKey, type

### Community 963 - "Community 963"
Cohesion: 0.40
Nodes (5): tool_todoRead_input, name, notNull, primaryKey, type

### Community 964 - "Community 964"
Cohesion: 0.40
Nodes (5): tool_todoWrite_input, name, notNull, primaryKey, type

### Community 965 - "Community 965"
Cohesion: 0.40
Nodes (5): tool_todoWrite_output, name, notNull, primaryKey, type

### Community 966 - "Community 966"
Cohesion: 0.40
Nodes (5): tool_tool_call_id, name, notNull, primaryKey, type

### Community 967 - "Community 967"
Cohesion: 0.40
Nodes (5): type, name, notNull, primaryKey, type

### Community 968 - "Community 968"
Cohesion: 0.40
Nodes (5): data_id, name, notNull, primaryKey, type

### Community 969 - "Community 969"
Cohesion: 0.40
Nodes (5): data_prefix, name, notNull, primaryKey, type

### Community 970 - "Community 970"
Cohesion: 0.40
Nodes (5): file_filename, name, notNull, primaryKey, type

### Community 971 - "Community 971"
Cohesion: 0.12
Nodes (16): file_media_type, message_id, reasoning_text, name, notNull, primaryKey, type, name (+8 more)

### Community 972 - "Community 972"
Cohesion: 0.40
Nodes (5): file_url, name, notNull, primaryKey, type

### Community 973 - "Community 973"
Cohesion: 0.40
Nodes (5): tool_fetch_input, name, notNull, primaryKey, type

### Community 974 - "Community 974"
Cohesion: 0.40
Nodes (5): order, name, notNull, primaryKey, type

### Community 975 - "Community 975"
Cohesion: 0.40
Nodes (5): provider_metadata, name, notNull, primaryKey, type

### Community 976 - "Community 976"
Cohesion: 0.40
Nodes (5): tool_fetch_output, name, notNull, primaryKey, type

### Community 977 - "Community 977"
Cohesion: 0.40
Nodes (5): source_document_filename, name, notNull, primaryKey, type

### Community 978 - "Community 978"
Cohesion: 0.40
Nodes (5): source_document_media_type, name, notNull, primaryKey, type

### Community 979 - "Community 979"
Cohesion: 0.40
Nodes (5): source_document_snippet, name, notNull, primaryKey, type

### Community 980 - "Community 980"
Cohesion: 0.40
Nodes (5): source_document_source_id, name, notNull, primaryKey, type

### Community 981 - "Community 981"
Cohesion: 0.40
Nodes (5): source_document_title, name, notNull, primaryKey, type

### Community 982 - "Community 982"
Cohesion: 0.40
Nodes (5): source_document_url, name, notNull, primaryKey, type

### Community 983 - "Community 983"
Cohesion: 0.40
Nodes (5): source_url_source_id, name, notNull, primaryKey, type

### Community 984 - "Community 984"
Cohesion: 0.40
Nodes (5): source_url_title, name, notNull, primaryKey, type

### Community 985 - "Community 985"
Cohesion: 0.40
Nodes (5): source_url_url, name, notNull, primaryKey, type

### Community 986 - "Community 986"
Cohesion: 0.40
Nodes (5): text_text, name, notNull, primaryKey, type

### Community 987 - "Community 987"
Cohesion: 0.40
Nodes (5): tool_dynamic_input, name, notNull, primaryKey, type

### Community 988 - "Community 988"
Cohesion: 0.40
Nodes (5): tool_dynamic_name, name, notNull, primaryKey, type

### Community 989 - "Community 989"
Cohesion: 0.40
Nodes (5): tool_dynamic_output, name, notNull, primaryKey, type

### Community 990 - "Community 990"
Cohesion: 0.40
Nodes (5): tool_dynamic_type, name, notNull, primaryKey, type

### Community 991 - "Community 991"
Cohesion: 0.40
Nodes (5): tool_error_text, name, notNull, primaryKey, type

### Community 992 - "Community 992"
Cohesion: 0.40
Nodes (5): tool_question_input, name, notNull, primaryKey, type

### Community 993 - "Community 993"
Cohesion: 0.40
Nodes (5): tool_question_output, name, notNull, primaryKey, type

### Community 994 - "Community 994"
Cohesion: 0.40
Nodes (5): tool_search_input, name, notNull, primaryKey, type

### Community 995 - "Community 995"
Cohesion: 0.40
Nodes (5): tool_search_output, name, notNull, primaryKey, type

### Community 996 - "Community 996"
Cohesion: 0.40
Nodes (5): tool_state, name, notNull, primaryKey, type

### Community 997 - "Community 997"
Cohesion: 0.40
Nodes (5): tool_todoRead_input, name, notNull, primaryKey, type

### Community 998 - "Community 998"
Cohesion: 0.40
Nodes (5): tool_todoWrite_input, name, notNull, primaryKey, type

### Community 999 - "Community 999"
Cohesion: 0.40
Nodes (5): tool_todoWrite_output, name, notNull, primaryKey, type

### Community 1000 - "Community 1000"
Cohesion: 0.40
Nodes (5): tool_tool_call_id, name, notNull, primaryKey, type

### Community 1001 - "Community 1001"
Cohesion: 0.40
Nodes (5): type, name, notNull, primaryKey, type

### Community 1002 - "Community 1002"
Cohesion: 0.40
Nodes (5): data_content, name, notNull, primaryKey, type

### Community 1003 - "Community 1003"
Cohesion: 0.40
Nodes (5): data_id, name, notNull, primaryKey, type

### Community 1004 - "Community 1004"
Cohesion: 0.40
Nodes (5): data_prefix, name, notNull, primaryKey, type

### Community 1005 - "Community 1005"
Cohesion: 0.40
Nodes (5): file_filename, name, notNull, primaryKey, type

### Community 1006 - "Community 1006"
Cohesion: 0.40
Nodes (5): file_url, name, notNull, primaryKey, type

### Community 1007 - "Community 1007"
Cohesion: 0.40
Nodes (5): order, name, notNull, primaryKey, type

### Community 1008 - "Community 1008"
Cohesion: 0.40
Nodes (5): provider_metadata, name, notNull, primaryKey, type

### Community 1009 - "Community 1009"
Cohesion: 0.40
Nodes (5): tool_todoRead_output, name, notNull, primaryKey, type

### Community 1010 - "Community 1010"
Cohesion: 0.40
Nodes (5): source_document_filename, name, notNull, primaryKey, type

### Community 1011 - "Community 1011"
Cohesion: 0.40
Nodes (5): source_document_media_type, name, notNull, primaryKey, type

### Community 1012 - "Community 1012"
Cohesion: 0.40
Nodes (5): source_document_snippet, name, notNull, primaryKey, type

### Community 1013 - "Community 1013"
Cohesion: 0.40
Nodes (5): source_document_title, name, notNull, primaryKey, type

### Community 1014 - "Community 1014"
Cohesion: 0.40
Nodes (5): source_document_url, name, notNull, primaryKey, type

### Community 1015 - "Community 1015"
Cohesion: 0.40
Nodes (5): source_url_source_id, name, notNull, primaryKey, type

### Community 1016 - "Community 1016"
Cohesion: 0.40
Nodes (5): source_url_title, name, notNull, primaryKey, type

### Community 1017 - "Community 1017"
Cohesion: 0.40
Nodes (5): source_url_url, name, notNull, primaryKey, type

### Community 1018 - "Community 1018"
Cohesion: 0.40
Nodes (5): text_text, name, notNull, primaryKey, type

### Community 1019 - "Community 1019"
Cohesion: 0.40
Nodes (5): tool_dynamic_input, name, notNull, primaryKey, type

### Community 1020 - "Community 1020"
Cohesion: 0.40
Nodes (5): tool_dynamic_name, name, notNull, primaryKey, type

### Community 1021 - "Community 1021"
Cohesion: 0.40
Nodes (5): tool_dynamic_output, name, notNull, primaryKey, type

### Community 1022 - "Community 1022"
Cohesion: 0.40
Nodes (5): tool_dynamic_type, name, notNull, primaryKey, type

### Community 1023 - "Community 1023"
Cohesion: 0.40
Nodes (5): tool_error_text, name, notNull, primaryKey, type

### Community 1024 - "Community 1024"
Cohesion: 0.40
Nodes (5): tool_fetch_input, name, notNull, primaryKey, type

### Community 1025 - "Community 1025"
Cohesion: 0.40
Nodes (5): tool_fetch_output, name, notNull, primaryKey, type

### Community 1026 - "Community 1026"
Cohesion: 0.40
Nodes (5): tool_question_input, name, notNull, primaryKey, type

### Community 1027 - "Community 1027"
Cohesion: 0.40
Nodes (5): tool_question_output, name, notNull, primaryKey, type

### Community 1028 - "Community 1028"
Cohesion: 0.40
Nodes (5): tool_search_input, name, notNull, primaryKey, type

### Community 1029 - "Community 1029"
Cohesion: 0.40
Nodes (5): tool_search_output, name, notNull, primaryKey, type

### Community 1030 - "Community 1030"
Cohesion: 0.40
Nodes (5): tool_state, name, notNull, primaryKey, type

### Community 1031 - "Community 1031"
Cohesion: 0.40
Nodes (5): tool_todoRead_input, name, notNull, primaryKey, type

### Community 1032 - "Community 1032"
Cohesion: 0.40
Nodes (5): tool_todoRead_output, name, notNull, primaryKey, type

### Community 1033 - "Community 1033"
Cohesion: 0.40
Nodes (5): tool_todoWrite_input, name, notNull, primaryKey, type

### Community 1034 - "Community 1034"
Cohesion: 0.40
Nodes (5): tool_todoWrite_output, name, notNull, primaryKey, type

### Community 1035 - "Community 1035"
Cohesion: 0.40
Nodes (5): tool_tool_call_id, name, notNull, primaryKey, type

### Community 1036 - "Community 1036"
Cohesion: 0.40
Nodes (5): type, name, notNull, primaryKey, type

### Community 1037 - "Community 1037"
Cohesion: 0.40
Nodes (5): data_content, name, notNull, primaryKey, type

### Community 1038 - "Community 1038"
Cohesion: 0.40
Nodes (5): data_id, name, notNull, primaryKey, type

### Community 1039 - "Community 1039"
Cohesion: 0.40
Nodes (5): data_prefix, name, notNull, primaryKey, type

### Community 1040 - "Community 1040"
Cohesion: 0.40
Nodes (5): file_filename, name, notNull, primaryKey, type

### Community 1041 - "Community 1041"
Cohesion: 0.40
Nodes (5): file_url, name, notNull, primaryKey, type

### Community 1042 - "Community 1042"
Cohesion: 0.40
Nodes (5): source_document_source_id, name, notNull, primaryKey, type

### Community 1043 - "Community 1043"
Cohesion: 0.40
Nodes (5): order, name, notNull, primaryKey, type

### Community 1044 - "Community 1044"
Cohesion: 0.40
Nodes (5): provider_metadata, name, notNull, primaryKey, type

### Community 1045 - "Community 1045"
Cohesion: 0.40
Nodes (5): source_document_filename, name, notNull, primaryKey, type

### Community 1046 - "Community 1046"
Cohesion: 0.40
Nodes (5): source_document_media_type, name, notNull, primaryKey, type

### Community 1047 - "Community 1047"
Cohesion: 0.40
Nodes (5): source_document_snippet, name, notNull, primaryKey, type

### Community 1048 - "Community 1048"
Cohesion: 0.40
Nodes (5): source_document_title, name, notNull, primaryKey, type

### Community 1049 - "Community 1049"
Cohesion: 0.40
Nodes (5): source_document_url, name, notNull, primaryKey, type

### Community 1050 - "Community 1050"
Cohesion: 0.40
Nodes (5): source_url_source_id, name, notNull, primaryKey, type

### Community 1051 - "Community 1051"
Cohesion: 0.40
Nodes (5): source_url_title, name, notNull, primaryKey, type

### Community 1052 - "Community 1052"
Cohesion: 0.40
Nodes (5): source_url_url, name, notNull, primaryKey, type

### Community 1053 - "Community 1053"
Cohesion: 0.40
Nodes (5): text_text, name, notNull, primaryKey, type

### Community 1054 - "Community 1054"
Cohesion: 0.40
Nodes (5): tool_dynamic_input, name, notNull, primaryKey, type

### Community 1055 - "Community 1055"
Cohesion: 0.40
Nodes (5): tool_dynamic_name, name, notNull, primaryKey, type

### Community 1056 - "Community 1056"
Cohesion: 0.40
Nodes (5): tool_dynamic_output, name, notNull, primaryKey, type

### Community 1057 - "Community 1057"
Cohesion: 0.40
Nodes (5): tool_dynamic_type, name, notNull, primaryKey, type

### Community 1058 - "Community 1058"
Cohesion: 0.40
Nodes (5): tool_error_text, name, notNull, primaryKey, type

### Community 1059 - "Community 1059"
Cohesion: 0.40
Nodes (5): tool_fetch_input, name, notNull, primaryKey, type

### Community 1060 - "Community 1060"
Cohesion: 0.40
Nodes (5): tool_fetch_output, name, notNull, primaryKey, type

### Community 1061 - "Community 1061"
Cohesion: 0.40
Nodes (5): tool_question_input, name, notNull, primaryKey, type

### Community 1062 - "Community 1062"
Cohesion: 0.40
Nodes (5): tool_question_output, name, notNull, primaryKey, type

### Community 1063 - "Community 1063"
Cohesion: 0.40
Nodes (5): tool_search_input, name, notNull, primaryKey, type

### Community 1064 - "Community 1064"
Cohesion: 0.40
Nodes (5): tool_search_output, name, notNull, primaryKey, type

### Community 1065 - "Community 1065"
Cohesion: 0.40
Nodes (5): tool_state, name, notNull, primaryKey, type

### Community 1066 - "Community 1066"
Cohesion: 0.40
Nodes (5): tool_todoRead_input, name, notNull, primaryKey, type

### Community 1067 - "Community 1067"
Cohesion: 0.40
Nodes (5): tool_todoRead_output, name, notNull, primaryKey, type

### Community 1068 - "Community 1068"
Cohesion: 0.40
Nodes (5): tool_todoWrite_input, name, notNull, primaryKey, type

### Community 1069 - "Community 1069"
Cohesion: 0.40
Nodes (5): tool_todoWrite_output, name, notNull, primaryKey, type

### Community 1070 - "Community 1070"
Cohesion: 0.40
Nodes (5): tool_tool_call_id, name, notNull, primaryKey, type

### Community 1071 - "Community 1071"
Cohesion: 0.40
Nodes (5): type, name, notNull, primaryKey, type

### Community 1072 - "Community 1072"
Cohesion: 0.40
Nodes (4): ESLint Conventions, Inline disables, Rule policy, When to disable vs refactor

### Community 1073 - "Community 1073"
Cohesion: 0.40
Nodes (5): API Errors, Authentication Errors, Command Examples for Testing, General Issues, Troubleshooting

### Community 1074 - "Community 1074"
Cohesion: 0.40
Nodes (5): source_document_source_id, name, notNull, primaryKey, type

### Community 1075 - "Community 1075"
Cohesion: 0.50
Nodes (3): Commercial Licensing, Non-Commercial License, Upstream License Notice

### Community 1076 - "Community 1076"
Cohesion: 0.50
Nodes (3): Architecture Authentication Flow, Authentication Flow, Client pattern details

### Community 1077 - "Community 1077"
Cohesion: 0.50
Nodes (3): Chart, Files, Quick check

### Community 1078 - "Community 1078"
Cohesion: 0.50
Nodes (3): Data Table, Files, Quick check

### Community 1079 - "Community 1079"
Cohesion: 0.50
Nodes (3): Files, Link Preview, Quick check

### Community 1080 - "Community 1080"
Cohesion: 0.50
Nodes (3): dialect, entries, version

### Community 1082 - "Community 1082"
Cohesion: 0.50
Nodes (3): Files, Quick check, Timeline

## Knowledge Gaps
- **10761 isolated node(s):** `husky.sh script`, `@railway/mcp-server`, `@arizeai/phoenix-mcp`, `PHOENIX_HOST`, `firecrawl-mcp` (+10756 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **39 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `cn()` connect `Components` to `Utilities`, `Components`, `Server Actions`, `Charts`, `React Hooks`, `Components`, `UI Primitives`, `Canvas UI`, `Evals Library`, `Evals UI`, `Components`, `UI Primitives`, `Tool UI`, `React Hooks`, `Tool UI`, `Tool UI`, `Tool UI`, `Components`, `Charts`, `Charts`, `Tool UI`, `Evals UI`, `Components`, `Tool UI`, `Tools`, `Tool UI`, `UI Primitives`, `Tool UI`, `UI Primitives`, `Tool UI`, `Tool UI`, `Tool UI`, `Tool UI`, `Tools`, `Community 486`, `Evals UI`?**
  _High betweenness centrality (0.011) - this node is a cross-community bridge._
- **Why does `UIMessage` connect `Components` to `Database & Schema`, `Components`, `Server Actions`, `Tools`, `React Hooks`, `Streaming`, `Tools`, `Tool UI`, `Community 655`, `Canvas UI`, `Tool UI`, `Components`, `Agents`, `Tools`, `Types`?**
  _High betweenness centrality (0.002) - this node is a cross-community bridge._
- **Why does `columns` connect `Community 971` to `Community 659`, `Community 425`, `Community 426`, `Community 968`, `Community 969`, `Community 970`, `Community 972`, `Community 973`, `Community 974`, `Community 975`, `Community 976`, `Community 977`, `Community 978`, `Community 979`, `Community 980`, `Community 981`, `Community 982`, `Community 983`, `Community 984`, `Community 985`, `Community 986`, `Community 987`, `Community 988`, `Community 989`, `Community 990`, `Community 991`, `Community 992`, `Community 993`, `Community 994`, `Community 995`, `Community 996`, `Community 997`, `Community 998`, `Community 999`, `Community 1000`, `Community 1001`, `Community 1009`, `Community 373`?**
  _High betweenness centrality (0.001) - this node is a cross-community bridge._
- **What connects `husky.sh script`, `@railway/mcp-server`, `@arizeai/phoenix-mcp` to the rest of the system?**
  _10784 weakly-connected nodes found - possible documentation gaps or missing edges._
- **Should `Project Root` be split into smaller, more focused modules?**
  _Cohesion score 0.020618556701030927 - nodes in this community are weakly interconnected._
- **Should `Utilities` be split into smaller, more focused modules?**
  _Cohesion score 0.05485232067510549 - nodes in this community are weakly interconnected._
- **Should `Components` be split into smaller, more focused modules?**
  _Cohesion score 0.04040404040404041 - nodes in this community are weakly interconnected._