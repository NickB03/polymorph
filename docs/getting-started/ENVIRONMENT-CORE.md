...
| `DATABASE_RESTRICTED_URL` | Optional locally; required in production | Connection string for the restricted `app_user` role (`scripts/provision-app-user.sql`). When set, the runtime query client prefers it over `DATABASE_URL` (`lib/db/index.ts`); the privileged admin client never uses it. Row-Level Security is only enforced through this role — the owner role bypasses it. See [production deployment](../operations/DEPLOYMENT-PRODUCTION.md). |
