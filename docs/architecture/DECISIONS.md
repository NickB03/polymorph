...
- **Image Generation**: `gateway:meta/muse-image-1.0` (Meta Muse Image) via Vercel AI Gateway (`AI_GATEWAY_API_KEY`; see `lib/tools/generate-image/server.ts`). Chosen 2026-09-18 over Gemini 2.5 Flash Image: about a quarter of the price ($0.01 vs $0.039 per image) and well ahead on the Artificial Analysis arenas (generation Elo 1112 vs 985, editing 1116 vs 987). It is a Gateway `image`-type model, so the tool calls `generateImage` rather than `generateText`.
...
- **Railway cron (offline evals)**: `polymorph-evals` runs the single synthetic regression case `reg-research-mode` every Monday at 15:00 UTC (`0 15 * * 1`, managed in Railway). The baseline uses `EVAL_RUN_MODE=regression`, `EVAL_CASE_IDS=reg-research-mode`, and `EVAL_CASE_CONCURRENCY=1`, then pushes results to Phoenix as experiments and persists summaries to `eval_summaries` for the admin `/admin/evals` dashboard. The service code defaults to OpenRouter-backed `google/gemini-3.1-flash-lite-preview`, but Railway can preserve a deployed `JUDGE_MODEL` override. `LOOKBACK_HOURS` and `SAMPLE_SIZE` apply only to optional `traffic-monitor` audits.
