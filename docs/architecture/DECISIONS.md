...
- **Railway cron (offline evals)**: `polymorph-evals` is intended to run on an every-48-hours cadence for the personal-project baseline. The exact live schedule is managed in Railway rather than this repo. The repo defaults use OpenRouter-backed `google/gemini-3.1-flash-lite-preview`, `LOOKBACK_HOURS=48`, `SAMPLE_SIZE=10`, and `EVAL_CASE_CONCURRENCY=1`, then push results to Phoenix as experiments and persist summaries to `eval_summaries` for the admin `/admin/evals` dashboard.
