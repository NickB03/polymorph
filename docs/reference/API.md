...
- `Content-Security-Policy: sandbox allow-scripts` -- sandboxes the document even when opened top-level (e.g. the fullscreen view). The opaque origin prevents model-generated scripts from reading app-origin cookies or storage, matching the isolation of the in-app preview iframe. Must never gain `allow-same-origin`.
